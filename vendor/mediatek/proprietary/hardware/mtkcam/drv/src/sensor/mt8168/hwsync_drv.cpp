...
MINT32 HWSyncDrvImp::init(HW_SYNC_USER_ENUM user,MUINT32 sensorIdx,MINT32 aeCyclePeriod)
{
    ...
    switch(user) {
        ...
        {
            SeninfDrv *const pSeninf_Drv = SeninfDrv::createInstance(HalSensorList::singleton()->querySecureState());
            ...
            if (mSensorIdxList.size() == 1) {
                ...
                SENINF_REG_TOP_N3D_A_CTL TopIO; // 0x40014
                SENINF_REG_N3D_A_CTL RegIO; // 0x40100

                //n3dEn=1, i2c1_en=1, i2c2_en=1, mode=0, dbg_sel default=3 (cnt1 inner reg)
                //SEN1_TIM_EN=1, SEN2_TIM_EN=1
                TopIO.Raw = 0x2200;
                RegIO.Raw = 0x3046;

                if (pSeninf) {
                    SENINF_WRITE_REG(pSeninf, SENINF_TOP_N3D_A_CTL, TopIO.Raw); // 0x1A040014
                    SENINF_WRITE_REG(pSeninf, SENINF_N3D_A_CTL, RegIO.Raw); // 0x1A040100
                } else {
...
MINT32 HWSyncDrvImp::uninit(HW_SYNC_USER_ENUM user,MUINT32 sensorIdx)
{
    ...
    switch(user) {
        case HW_SYNC_USER_HALSENSOR:
        {
            ...
            /*
            /* [4]remove sensor device info if user power off sensor
            /* (need after destroyThread cuz function "revertSenssorFps need senDev info")
             */
            if (uninitIdx <= 1) {
    ...
    switch(cmd) {
        ...
        case HW_SYNC_CMD_DISABLE_SYNC:
        {
            ...
            if (this->mSyncCount == 0) {
                ...
                //m_pIspDrv
                if (this->m_pIspDrv) {
                    int result = this->m_pIspDrv->uninit("Hwsync");
                    if (result < 0) {
                        LOG_ERR("m_pIspDrv->uninit() fail ");
                        return result;
                    }
            ...
            if (mState == HW_SYNC_STATE_SYNCING) {
                ...
                //need wait sync related procedure stop before do AE operation, assume do AE operation after 2 sensors are configured done
                mState = HW_SYNC_STATE_IDLE ;
                pthread_mutex_unlock(&mStateMutex);
            }
        ...
        case HW_SYNC_CMD_SET_PARA:
            if (Dev_temp.size() > 2) {
                ...
                if (Dev_temp.size() != 0) {
                    if (Dev_temp[0] == arg1) {
                        LOG_ERR("cant set the same sen DevID twince(%d)\n",arg1);
                        ret = 1;
                    }
                }
                ...
                for (unsigned int i = 0; i < Dev_temp.size(); i++) {
                    LOG_INF("Dev_temp[%d]=%d", i, Dev_temp[i]);
                }
            ...
            switch(arg2) {//scenario here is dvfs level
                ...
                default:
                    this->m_DVFS = E_SMI_HIGH;
                    LOG_ERR("unsupported DVFS level:%llu\n", (long long)arg2);
                    break;
            }
            break;
        case HW_SYNC_CMD_ENABLE_SYNC:
            ...
            if (this->mSyncCount == 0) {
                //m_pIspDrv creation and initialization
                this->m_pIspDrv = IspDrv::createInstance();
                if (!this->m_pIspDrv) {
                    LOG_ERR("m_pIspDrv::createInstance fail ");
                    break;
                }

                int result = this->m_pIspDrv->init("Hwsync");
                if (result < 0) {
                    LOG_ERR("m_pIspDrv->init() fail ");
                    return result;
                }
            }
            ...
            switch(this->mSyncCount) {
                ...
                case 2:
                {
                    ...
                    for (MUINT32 i = 0; i < this->Dev_temp.size(); i++) {
                        LOG_INF("previous configured_%d devID:%d,shutter:%d\n",i, this->Dev_temp[i],\
                            this->TG_Shutter[i]);
                    }
                    LOG_INF("2nd lunch devID:%llu, TG:%d\n", (long long)arg2, dynamicInfo.TgInfo);
                    //change order
        ...
        case HW_SYNC_CMD_GET_FRAME_TIME:
        {
            if (this->mSyncCount == 2) {
                ...
                LOG_DBG("senDev(%d), expTime[0]=%d, expTime[1]=%d, frmTime[0]=%d, frmTime[1]=%d\n",arg1, *(p_expTime+0),*(p_expTime+1),*(p_frmTime+0),*(p_frmTime+1));
        ...
        case HW_SYNC_CMD_SYNC_TYPE:
            *((MUINTPTR*)arg2) = (MUINT32)HW_SYNC_TYPE_VER_2;
            break;
    }
    if (ret) {
        LOG_ERR("cmd fail_%s\n",str(cmd));
...
MINT32 HWSyncDrvImp::changeMode(MBOOL bTrigInitFlow)
{
    ...
    LOG_INF("Init info: dev_0x%x/dev_0x%x pclk(%d/%d),period(%d,%d/%d,%d),fps(%d/%d us),frmtime_delay(%d/%d)",\
        ...
        mFrameTimeDelayFrame[0], mFrameTimeDelayFrame[1]);
    ...
    *(pp_frmTime+1) = 0;
    ...
    if((0 != pre_aeExp[0]) && (0 != pre_aeExp[1])){
        ...
        if((expTime_MAIN1 > pre_aeExp[0]*2) || (expTime_MAIN1*2 < pre_aeExp[0])){
            LOG_INF("[MAIN1]no adj, pre_exp(%d/%d),exp(%d/%d)\n", pre_aeExp[0], pre_aeExp[1], expTime_MAIN1, expTime_MAIN2);
            pre_aeExp[0] = expTime_MAIN1;
            pre_aeExp[1] = expTime_MAIN2;
            goto EXIT;
        }
        else if((expTime_MAIN2 > pre_aeExp[1]*2) || (expTime_MAIN2*2 < pre_aeExp[1])){
            LOG_INF("[MAIN2]no adj, pre_exp(%d/%d),exp(%d/%d)\n", pre_aeExp[0], pre_aeExp[1], expTime_MAIN1, expTime_MAIN2);
            pre_aeExp[0] = expTime_MAIN1;
            pre_aeExp[1] = expTime_MAIN2;
            goto EXIT;
        }
...
MUINT32 HWSyncDrvImp::calPixCnt2US(MUINT32 n3dCnt)
{
    ...
    if(time > 0xFFFFFFFF){
        LOG_ERR("pix2time over flow:0x%x\n",n3dCnt);
    }

//    LOG_DBG("SMI_CLOCK(this->m_DVFS) = %d, time = %lld", SMI_CLOCK(this->m_DVFS), time);
    return (MUINT32)time;
}
...
MVOID* HWSyncDrvImp::DBG_Thread::DBG_Trig(MVOID* arg)
{
    ...
    waitIrq.UserInfo.UserKey = _this->m_pIspDrv->registerIrq(LOG_TAG);
    if (_this->m_DBG_t.m_working_tg == E_TG_A) {
    ...
    //check sync result
    for (m = 0; m < 2; m++) {
        if (_this->m_pIspDrv->waitIrq(&waitIrq) <= 0) {
            LOG_ERR("[%d]:wait vsync timeout\n",_this->m_DBG_t.m_working_tg);
            goto EXIT;
        }

        n = _this->DeviceInfo(E_GET_VSYNC_CNT,_this->m_DBG_t.m_working_tg);
        LOG_INF("[%d]:vsync counter:%d\n",_this->m_DBG_t.m_working_tg,n);
    }
    ...
    if (_this->mHalSensor->sendCommand(_this->mSensorDevArray[_this->m_DBG_t.m_working_tg],\
        ...
        LOG_ERR("[%d]:query N3D diff fail\n",_this->mSensorDevArray[_this->m_DBG_t.m_working_tg]);
...
MINT32 HWSyncDrvImp::doOneTimeSync()
{
    ...
    for (int i = 0; i < 2; i++) {
        if (dynamicInfo[i].TgInfo == CAM_TG_1) {
            ...
            LOG_ERR("unsupported TG:%d\n",dynamicInfo[i].TgInfo);
        }
    }
...
MUINT32 HWSyncDrvImp::doGeneralSync_AE(MUINT32 arg1, MUINTPTR arg2, MUINTPTR arg3)
{
    ...
    LOG_INF("adjSenIdx(0x%x), adjFrmT(%d) us, PreFrmTime(%d/%d) us, CurFrmTime(%d/%d) us, VD(%d) us, order(%d), SOF(%d/%d)\n",adj_sensor,\
...
MUINT32 HWSyncDrvImp::doGeneralSync2_AE(MUINT32 arg1, MUINTPTR arg2, MUINTPTR arg3)
{
    ...
    LOG_INF("adjSenIdx(0x%x), adjFrmT(%d) us, PreFrmTime(%d/%d) us, CurFrmTime(%d/%d,%d/%d) us, VD(%d) us, order(%d), SOF(%d/%d)\n",adj_sensor,\
        frmTime_1[adj_sensor], \
        this->calPixCnt2US(mN3dInfo.vs_regCnt[0]),this->calPixCnt2US(mN3dInfo.vs_regCnt[1]), \
        this->mCurFrameTime[0], this->mCurFrameTime[1], \
        newFrmT[0],newFrmT[1], \
...
MINT32 HWSyncDrvImp::setSyncFrmTimeMain1()
{
    ...
    waitIrq.UserInfo.Type = ISP_DRV_IRQ_TYPE_INT_P1_ST;
    waitIrq.UserInfo.Status = CAM_CTL_INT_P1_STATUS_VS1_INT_ST;
    waitIrq.UserInfo.UserKey = this->m_pIspDrv->registerIrq(LOG_TAG);
    ...
    if (mState != HW_SYNC_STATE_IDLE && mState != HW_SYNC_STATE_READY2LEAVE) {
        ...
        for (unsigned int i = 0; i < waitSofCnt; i++) {
            if (this->m_pIspDrv->waitIrq(&waitIrq) <= 0) {
                if (mState != HW_SYNC_STATE_IDLE && mState != HW_SYNC_STATE_READY2LEAVE) {
        ...
        if (this->mHalSensor->sendCommand(this->mSensorDevArray[0],\
            ...
            LOG_ERR("[Main1]:set frame rate to sensor failed, m2ndData.FrameTime: %d\n", this->m2ndData.FrameTime[0]);
...
MINT32 HWSyncDrvImp::setSyncFrmTimeMain2()
{
    ...
    if (mState != HW_SYNC_STATE_IDLE && mState != HW_SYNC_STATE_READY2LEAVE) {
        ...
        if (this->mHalSensor->sendCommand(this->mSensorDevArray[1],\
            ...
            LOG_ERR("[Main2]:set frame rate to sensor failed, m2ndData.FrameTime: %d\n", this->m2ndData.FrameTime[1]);
...
MUINT32 HWSyncDrvImp::DeviceInfo(E_CMD cmd, MUINT32 tgIdx)
{
    MUINT32 _cnt = 0;
#if 0   //tony
    E_DEVICE_INFO ispCmd = _GET_SOF_CNT;
    ISP_DRV_PASS1_PATH_ENUM ispPath = ISP_DRV_PASS1_PATH_P1;
    ...
    switch(tgIdx) {
        case E_TG_A:
            ispPath = ISP_DRV_PASS1_PATH_P1;
            break;
        case E_TG_B:
            ispPath = ISP_DRV_PASS1_PATH_P1_D;
            break;
        default:
    ...
    if(!this->m_pIspDrv->getDeviceInfo(ispCmd, ispPath, (MUINT8 *)&_cnt)) {
        LOG_ERR("P1 getDeviceInfo error!, ispCmd = %d ispPath = %d", ispCmd, ispPath);
    }
//    LOG_DBG("_cnt = %d", _cnt);
#endif
    return _cnt;
