...
static INT32 consys_pmic_get_from_dts(struct platform_device *pdev)
{
	...
	if (KERNEL_pmic_get_register_value(PMIC_SWCID) == PMIC_MT6390_CHIP_ID) {
...
static INT32 consys_hw_vcn18_ctrl(MTK_WCN_BOOL enable)
{
	if (enable) {
		...
		/*set vcn18 SW mode*/
		KERNEL_pmic_set_register_value(PMIC_RG_LDO_VCN18_OP_EN_SET, 0x1);
		KERNEL_pmic_set_register_value(PMIC_RG_LDO_VCN18_OP_EN_CLR, 0xE);
		if (reg_VCN18) {
		...
		KERNEL_pmic_set_register_value(PMIC_RG_LDO_VCN33_OP_EN_SET, 0x1);
		KERNEL_pmic_set_register_value(PMIC_RG_LDO_VCN33_OP_EN_CLR, 0xE);
		if (reg_VCN33_BT) {
