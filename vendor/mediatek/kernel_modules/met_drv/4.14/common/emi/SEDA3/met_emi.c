...
int met_check_ddr_type(unsigned int DRAM_TYPE) __attribute__((weak));
int met_check_ddr_type(unsigned int DRAM_TYPE)
{
	return MET_DDRTYPE_UNDEFINE;
}

static unsigned int get_emi_clock_rate(unsigned int dram_data_rate_MHz)
{
	unsigned int DRAM_TYPE;
	int met_ddr_type = MET_DDRTYPE_UNDEFINE;

	if (get_ddr_type_symbol) {
		DRAM_TYPE = get_ddr_type_symbol();
		met_ddr_type = met_check_ddr_type(DRAM_TYPE);

		if (met_ddr_type == MET_DDRTYPE_UNDEFINE) {
			/*
			 *      smart phone project define:
			 *	the ddr type define :
			 *	enum DDRTYPE {
			 *	TYPE_LPDDR3 = 1,
			 *	TYPE_LPDDR4,
			...
			if ((DRAM_TYPE == 2) || (DRAM_TYPE == 3))
				met_ddr_type = MET_DDRTYPE_DDR4;
			else
				met_ddr_type = MET_DDRTYPE_DDR3;
		} else if (met_ddr_type == MET_DDRTYPE_UNKNOWN) {
			METERROR("[%s][%d] Unknown ddr_type = %u, use the TYPE_LPDDR3 setting\n",
				__func__, __LINE__, DRAM_TYPE);
		}
		if (met_ddr_type == MET_DDRTYPE_DDR4)
