...
noinline void ms_mali(struct timeval ts, unsigned int freq,const int cnt, unsigned int *value)
{
	char *SOB, *EOB;
	int len;

	MET_TRACE_GETBUF(&SOB, &EOB);
	len = sprintf(EOB, "%x,%x,%x,", (unsigned int)ts.tv_sec, (unsigned int)ts.tv_usec, freq);
	// coverity[var_deref_op : FALSE]
	EOB = ms_formatH_EOL(EOB + len, cnt, value);
	MET_TRACE_PUTBUF(SOB, EOB);
}
...
noinline void MET_DBG_TEMP(void)
{
	return;
}
