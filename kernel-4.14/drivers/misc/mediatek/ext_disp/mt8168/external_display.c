...
struct ext_disp_path_context {
	...
	struct cmdqRecStruct *cmdq_rdma_handle_config;
...
atomic_t g_extd_release_ticket = ATOMIC_INIT(0);
...
{
	ext_disp_mode = EXTD_DECOUPLE_MODE;	/*mode; */
...
int _is_hdmi_decouple_mode(enum EXT_DISP_PATH_MODE mode)
{
...
static bool _is_hdmi_interlace(void)
{
	/*get from hdmi*/
	return false;
}

static uint32_t _get_wdma_normal_buffer_mva(void)
{
...
static uint32_t _set_rdma_normal_buffer_id(void)
{
	pgc->dc_rdma_buf_id = pgc->dc_buf_id;

	return 0;
}

static uint32_t _get_rdma_normal_buffer_mva(void)
{
	uint32_t rdma_mva;

	rdma_mva = pgc->dc_buf[pgc->dc_buf_id];
...
void hdmi_config_rdma(void)
{
	...
	if (pgc->state == EXTD_DEINIT || pgc->state == EXTD_SUSPEND) {
		DISPMSG("EXTD_DEINIT : exit %s!\n", __func__);
		goto done;
	}
	{
		cmdq_handle = pgc->cmdq_rdma_handle_config;
		if (cmdq_handle == NULL)
			DISPERR("%s : cmdq_handle is NULL!\n", __func__);
		...
		rdma_mva = _get_rdma_normal_buffer_mva();

		if (rdma_mva)
			pconfig->rdma_config.address =
				(unsigned int)rdma_mva;
		else
			DISPERR("rdma input address is null!!\n");
		pconfig->rdma_config.width = ext_disp_get_width(pgc->session);
		pconfig->rdma_config.height = ext_disp_get_height(pgc->session);
		pconfig->rdma_config.inputFormat = UFMT_RGB888;
		pconfig->rdma_config.pitch =
			ext_disp_get_width(pgc->session) * 3;
		pconfig->rdma_config.dst_x = 0;
		pconfig->rdma_config.dst_y = 0;
		pconfig->rdma_config.dst_h = pconfig->dst_h;
		pconfig->rdma_config.dst_w = pconfig->dst_w;
		if (_is_hdmi_interlace() && _is_hdmi_decouple_mode(pgc->mode)) {
			pconfig->rdma_config.height /= 2;
			pconfig->rdma_config.pitch *= 2;
			/*is top filed, set buffer offset*/
			/*get top info from DPI*/
			if (0)
				pconfig->rdma_config.buf_offset +=
					ext_disp_get_width(pgc->session) * 3;
		}
...
struct task_struct *hdmi_config_rdma_task;
wait_queue_head_t hdmi_config_rdma_wq;
atomic_t hdmi_config_rdma_event = ATOMIC_INIT(0);

static void _hdmi_config_rdma_irq_handler(enum DISP_MODULE_ENUM module,
					  unsigned int param)
{
	if (pgc->state == EXTD_DEINIT || pgc->state == EXTD_SUSPEND) {
		DISPCHECK("hdmi is not plugin, exit rdma_irq_handler\n");
		return;
	}

	if (module == DISP_MODULE_RDMA1 && _is_hdmi_interlace()) {
		/* i timing , config rdma address based on
		 * top/bottom filed info
		 */
...
static int _hdmi_config_rdma_kthread(void *data)
{
	...
	while (1) {
		wait_event_interruptible(hdmi_config_rdma_wq,
					 atomic_read(&hdmi_config_rdma_event));
		atomic_set(&hdmi_config_rdma_event, 0);

		/* hdmi config rdma indepenent, instead of config wdma and rdma
		 * in the same cmdq task
		 */
		hdmi_config_rdma();
...
static struct disp_internal_buffer_info *
_allocate_decouple_buffer(unsigned int size)
{
	...
	client = ion_client_create(g_ion_device, "ext_disp_decouple");
...
static void _init_decouple_buffers(void)
{
	unsigned int i = 0;
	unsigned int height = HDMI_MAX_HEIGHT;
	unsigned int width = HDMI_MAX_WIDTH;
	unsigned int bpp = ufmt_get_bpp(UFMT_RGB888);
	...
	for (i = 0; i < DISP_INTERNAL_BUFFER_COUNT; i++) {
		decouple_buffer_info[i] =
			_allocate_decouple_buffer(buffer_size);
		if (decouple_buffer_info[i] != NULL) {
			pgc->dc_buf[i] = decouple_buffer_info[i]->mva;
			EXTDMSG("decouple buffer: pgc->dc_buf[%d]= 0x%x\n",
				i, pgc->dc_buf[i]);
		}
	}
}
...
static int _trigger_ovl_to_memory(disp_path_handle disp_handle,
	struct cmdqRecStruct *cmdq_handle, void *callback,
	unsigned int data)
{
	dpmgr_path_trigger(disp_handle, cmdq_handle, CMDQ_ENABLE);
	cmdqRecWait(cmdq_handle, CMDQ_EVENT_DISP_WDMA0_SOF);
...
static int _ext_disp_config_output(void)
{
	int ret = 0;
	struct disp_ddp_path_config *pconfig = NULL;
	...
	{
		writing_mva = _get_wdma_normal_buffer_mva();
	...
	EXTDINFO("config_wdma w %d, h %d hdmi(%d %d)\n",
		    pconfig->wdma_config.srcWidth,
		    pconfig->wdma_config.srcHeight,
			ext_disp_get_width(pgc->session),
		    ext_disp_get_height(pgc->session));
...
static int _build_path_decouple(void)
{
	...
	struct disp_ddp_path_config *pconfig = NULL;
	uint32_t writing_mva = 0;
	unsigned int rdma_bpp = 3;
	enum UNIFIED_COLOR_FMT rdma_fmt = UFMT_RGB888;

	EXTDFUNC();
	pgc->mode = EXTD_DECOUPLE_MODE;

	ret = cmdqRecCreate(CMDQ_SCENARIO_RDMA1_DISP,
	...
	pgc->ovl2mem_path_handle = dpmgr_create_path(
		DDP_SCENARIO_SUB_OVL_MEMOUT, pgc->cmdq_handle_config);
	...
	if (first_build_path_decouple) {
		...
		_init_decouple_buffers();
		first_build_path_decouple = 0;
	}

	/* ovl need dst_dirty to set background color */
	pconfig = dpmgr_path_get_last_config(pgc->ovl2mem_path_handle);
	pconfig->dst_w = ext_disp_get_width(pgc->session);
	pconfig->dst_h = ext_disp_get_height(pgc->session);
	pconfig->dst_dirty = 1;
	...
	pconfig->rdma_config.pitch =
		ext_disp_get_width(pgc->session) * rdma_bpp;
	pconfig->rdma_dirty = 1;

	DISPMSG("build_path_decouple dst(%d %d) rdma(%d %d) hdmi(%d %d)\n",
		pconfig->dst_w, pconfig->dst_h, pconfig->rdma_config.width,
		pconfig->rdma_config.height, ext_disp_get_width(pgc->session),
		ext_disp_get_height(pgc->session));

	init_waitqueue_head(&hdmi_config_rdma_wq);

	disp_register_module_irq_callback(
		DISP_MODULE_RDMA1, _hdmi_config_rdma_irq_handler);

	hdmi_config_rdma_task =
		kthread_create(_hdmi_config_rdma_kthread, NULL,
			       "hdmi_config_rdma_kthread");
...
static void _cmdq_build_trigger_loop(void)
{
	...
	if (pgc->cmdq_handle_trigger == NULL) {
		ret =
		    cmdqRecCreate(CMDQ_SCENARIO_TRIGGER_LOOP_SUB,
...
int ext_disp_deinit(unsigned int session)
{
	...
	if (_is_hdmi_decouple_mode(pgc->mode)) {
		...
		dpmgr_destroy_path(pgc->ovl2mem_path_handle, NULL);
	}
...
int ext_fence_release_callback(unsigned long userdata)
{
	...
	/*update rdma buffer id*/
	_set_rdma_normal_buffer_id();
	if (_is_hdmi_decouple_mode(pgc->mode) && !_is_hdmi_interlace()) {
		/*trigger rdma thread to update rdma buffer*/
		atomic_set(&hdmi_config_rdma_event, 1);
		wake_up_interruptible(&hdmi_config_rdma_wq);
	}
	...
	if (pgc->mode == EXTD_DECOUPLE_MODE)
		ret =
		    _trigger_ovl_to_memory(pgc->ovl2mem_path_handle,
				pgc->cmdq_handle_config,
				callback, atomic_read(&g_extd_trigger_ticket));
...
int ext_disp_frame_cfg_input(struct disp_frame_cfg_t *cfg)
{
	...
	if (_should_config_ovl_input()) {
		...
		gset_arg.dst_mod_type =
		    dpmgr_path_get_dst_module_type(handle);
		gset_arg.is_decouple_mode = 1;

		dpmgr_path_ioctl(handle, pgc->cmdq_handle_config,
	...
	if (_should_wait_path_idle())
		dpmgr_wait_event_timeout(handle,
					 DISP_PATH_EVENT_FRAME_DONE, HZ / 2);
