...
int hdmi_uevent_dev_register(struct notify_dev *sdev)
{
	...
	if (!switch_class) {
		...
		if (ret == 0) {
			EXTDMSG("[hdmi]w-y create_switch_class susesess\n");
		} else {
			EXTDMSG("[hdmi]w-y create_switch_class fail\n");
			return ret;
		}
	}

	sdev->index = atomic_inc_return(&device_count);
	EXTDMSG("[hdmi]w-y index:0x%x\n", sdev->index);

	sdev->dev = device_create(switch_class, NULL,
		 MKDEV(0, sdev->index), NULL, sdev->name);

	if (sdev->dev) {
		EXTDMSG("[hdmi]w-y device create ok,index:0x%x\n",
			sdev->index);
		ret = 0;
	} else {
		EXTDMSG("[hdmi]w-y device create fail,index:0x%x\n",
			sdev->index);
		ret = -1;
	}
	...
	if (ret < 0) {
		...
		EXTDMSG("switch: Failed to register driver %s\n",
			sdev->name);
	}
...
int notify_uevent_user(struct notify_dev *sdev, int state)
{
	...
	EXTDMSG("[hdmi]w-y notify state:0x%x\n", state);
	...
	envp[1] = state_buf;
	envp[2] = NULL;
	EXTDMSG("[hdmi]w-y name:%s ,state:%s\n", envp[0], envp[1]);

	kobject_uevent_env(&sdev->dev->kobj, KOBJ_CHANGE, envp);

	EXTDMSG("[hdmi]w-y kobj_name:%s\n", sdev->dev->kobj.name);
	EXTDMSG("[hdmi]w-y init:0x%x\n", sdev->dev->kobj.state_initialized);
	EXTDMSG("[hdmi]w-y sysfs:0x%x\n",
		sdev->dev->kobj.state_in_sysfs);
	EXTDMSG("[hdmi]w-y uevent sent:0x%x\n",
		sdev->dev->kobj.state_add_uevent_sent);
	EXTDMSG("[hdmi]w-y remove sent:0x%x\n",
		sdev->dev->kobj.state_remove_uevent_sent);
	EXTDMSG("[hdmi]w-y uevent suppress:0x%x\n",
...
void hdmi_force_resolution(int params)
{
	...
	/* EXTDMSG("__func__ params:0x%lx, 3d:%d\n", __func__, force_reschange,
...
{
	struct LCM_PARAMS *disp_config = &(extd_dpi_params.dispif_config);

	EXTDFUNC();

	disp_config->dpi.dsc_enable = 0;
	if (hdmi_drv && hdmi_drv->get_params) {
		...
		if (dst_is_dsi) {
			...
			disp_config->width = p->hdmi_width;
			disp_config->height = p->hdmi_height;

			disp_config->type = LCM_TYPE_DSI;
			...
			if ((arg == HDMI_VIDEO_2160p_DSC_24Hz)
				...
				memset(&(disp_config->dsi.dsc_params),
				0, sizeof(struct LCM_DSC_CONFIG_PARAMS));

				disp_config->dsi.dsc_enable = 1;

				/* width/(slice_mode's slice) */
				disp_config->dsi.dsc_params.slice_width = 1920;
				disp_config->dsi.dsc_params.slice_hight = 8;
				/* 128: 1/3 compress; 192: 1/2 compress */
				disp_config->dsi.dsc_params.bit_per_pixel = 128;
				/* 0: 1 slice; 1: 2 slice; 2: 3 slice */
				disp_config->dsi.dsc_params.slice_mode = 1;
				disp_config->dsi.dsc_params.rgb_swap = 0;
				disp_config->dsi.dsc_params.xmit_delay = 0x200;
				disp_config->dsi.dsc_params.dec_delay = 0x4c0;
				disp_config->dsi.dsc_params.scale_value = 0x20;
				disp_config->dsi.dsc_params.increment_interval =
					0x11e;
				disp_config->dsi.dsc_params.decrement_interval =
					0x1a;
				disp_config->dsi.dsc_params.nfl_bpg_offset =
					0xdb7;
				disp_config->dsi.dsc_params.slice_bpg_offset =
					0x394;
				disp_config->dsi.dsc_params.final_offset =
					0x10f0;
				disp_config->dsi.dsc_params.line_bpg_offset =
					0xc;
				disp_config->dsi.dsc_params.bp_enable = 0x0;
				disp_config->dsi.dsc_params.rct_on = 0x0;
			...
			disp_config->dpi.hsync_back_porch =
			    hdmi_params->hsync_back_porch;
			disp_config->dpi.hsync_front_porch =
			    hdmi_params->hsync_front_porch;

			disp_config->dpi.vsync_pulse_width =
			    hdmi_params->vsync_pulse_width;
			disp_config->dpi.vsync_back_porch =
			    hdmi_params->vsync_back_porch;
			disp_config->dpi.vsync_front_porch =
			    hdmi_params->vsync_front_porch;

			disp_config->dpi.dpi_clock =
			...
			disp_config->dpi.width = p->hdmi_width;
			disp_config->dpi.height =
			    p->hdmi_height;
			if ((arg == HDMI_VIDEO_2160p_DSC_24Hz)
			    || (arg == HDMI_VIDEO_2160p_DSC_30Hz)) {
				memset(&(disp_config->dpi.dsc_params), 0,
					sizeof(struct LCM_DSC_CONFIG_PARAMS));

				disp_config->dpi.dsc_enable = 1;

				disp_config->dpi.width =
				    p->hdmi_width / 3;
				/* width/(slice_mode's slice) */
				disp_config->dpi.dsc_params.slice_width = 1920;
				disp_config->dpi.dsc_params.slice_hight = 8;
				/* 128: 1/3 compress; 192: 1/2 compress */
				disp_config->dpi.dsc_params.bit_per_pixel = 128;
				/* 0: 1 slice; 1: 2 slice; 2: 3 slice */
				disp_config->dpi.dsc_params.slice_mode = 1;
				disp_config->dpi.dsc_params.rgb_swap = 0;
				disp_config->dpi.dsc_params.xmit_delay = 0x200;
				disp_config->dpi.dsc_params.dec_delay = 0x4c0;
				disp_config->dpi.dsc_params.scale_value = 0x20;
				disp_config->dpi.dsc_params.increment_interval =
					0x11e;
				disp_config->dpi.dsc_params.decrement_interval =
					0x1a;
				disp_config->dpi.dsc_params.nfl_bpg_offset =
					0xdb7;
				disp_config->dpi.dsc_params.slice_bpg_offset =
					0x394;
				disp_config->dpi.dsc_params.final_offset =
					0x10f0;
				disp_config->dpi.dsc_params.line_bpg_offset =
					0xc;
				disp_config->dpi.dsc_params.bp_enable = 0x0;
				disp_config->dpi.dsc_params.rct_on = 0x0;

			}

			disp_config->dpi.bg_width =
			    p->bg_width;
			disp_config->dpi.bg_height =
			    p->bg_height;

			disp_config->dpi.format =
			    LCM_DPI_FORMAT_RGB888;
			disp_config->dpi.rgb_order =
			    LCM_COLOR_ORDER_RGB;
			disp_config->dpi.i2x_en = true;
			disp_config->dpi.i2x_edge = 2;
			disp_config->dpi.embsync = false;
		}
	}

	ext_disp_set_lcm_param(disp_config);
...
int hdmi_power_enable(int enable)
{
	...
	if (enable) {
		...
		hdmi_power_off();
		/* switch_set_state(&hdmi_switch_data, HDMI_STATE_NO_DEVICE); */
		notify_uevent_user(&hdmi_notify_data, HDMI_STATE_NO_DEVICE);
	}
	EXTDINFO("%s done\n", __func__);
	return 0;
}
...
int hdmi_set_resolution(int res)
{
	...
	up(&hdmi_update_mutex);
	if (enable_ut != 1) {
		/* switch_set_state(&hdmires_switch_data,
		 *hdmi_reschange + 1);
		 */
...
int hdmi_init(void)
{
	...
	hdmi_notify_data.state = 0;
	ret = hdmi_uevent_dev_register(&hdmi_notify_data);
	if (ret == 0)
		EXTDMSG("[hdmi] hdmi_uevent_dev_register hdmi ok\n");
	else
		EXTDMSG("[hdmi] hdmi_uevent_dev_register hdmi fail\n");
	...
	if (ret == 0)
		EXTDMSG("[hdmi] hdmi_uevent_dev_register res_hdmi ok\n");
	else
		EXTDMSG("[hdmi] hdmi_uevent_dev_register res_hdmi fail\n");
