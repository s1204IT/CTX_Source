...
struct _JpegDecSOF {
	...
	enum _JpegDecFormat eJpegFormat;
	...
	struct _JpegDecSOFComp arSofComp[MAX_JPEG_COMPONENT];
};
...
struct _JpegDecDHT {
	...
	struct _JpegDecDhtHuffTbl arDcTbl[4];
	struct _JpegDecDhtHuffTbl arAcTbl[4];
};
...
struct _JpegDrvDecFlow {
	...
	/* no use in full frame mode,
	 * only use in PauseResume/DirectCouple mode
	 */
...
struct _JPEG_DEC_DRV_OUT {
	long timeout;
	unsigned int *result;
};

struct _JPEG_DEC_CONFIG_ROW {
	unsigned int decRowBuf[3];	/* OK */
	unsigned int pauseMCU;	/* OK */
};

struct _JPEG_DEC_CONFIG_CMDQ {
	unsigned int goNum;
...
struct _JpegDrvDecResult {
	unsigned int *pChksum;
};
/* ========================================= */

/* JPEG Encoder Structure */
struct _JPEG_ENC_DRV_IN {
...
struct _compat_JPEG_DEC_DRV_OUT {
	compat_long_t timeout;
	compat_uptr_t result;
};

struct _compat_JpegDrvDecResult {
	compat_uptr_t pChksum;
};

struct _compat_JPEG_ENC_DRV_OUT {
	compat_long_t timeout;
...
#define JPEG_DEC_IOCTL_CONFIG \
	_IOW(JPEG_IOCTL_MAGIC,  2, struct _JPEG_DEC_DRV_IN)
#define JPEG_DEC_IOCTL_FLOW   \
	_IOW(JPEG_IOCTL_MAGIC,  3, struct _JpegDrvDecFlow)
#define JPEG_DEC_IOCTL_START  _IO(JPEG_IOCTL_MAGIC,  4)
#define JPEG_DEC_IOCTL_WAIT   \
	_IOWR(JPEG_IOCTL_MAGIC,  5, struct _JPEG_DEC_DRV_OUT)
...
#define JPEG_DEC_IOCTL_CHKSUM \
	_IOWR(JPEG_IOCTL_MAGIC,  8, struct _JpegDrvDecResult)
...
#define JPEG_DEC_IOCTL_RESUME \
	_IOW(JPEG_IOCTL_MAGIC, 11, struct _JPEG_DEC_CONFIG_ROW)
#define JPEG_DEC_IOCTL_FLUSH_CMDQ \
	_IOW(JPEG_IOCTL_MAGIC, 17, struct _JPEG_DEC_CONFIG_CMDQ)
...
#define JPEG_ENC_IOCTL_CONFIG    \
	_IOW(JPEG_IOCTL_MAGIC, 12, struct _JPEG_ENC_DRV_IN)
#define JPEG_ENC_IOCTL_WAIT     \
	_IOWR(JPEG_IOCTL_MAGIC, 13, struct _JPEG_ENC_DRV_OUT)
...
#define COMPAT_JPEG_DEC_IOCTL_WAIT \
	_IOWR(JPEG_IOCTL_MAGIC,  5, struct _compat_JPEG_DEC_DRV_OUT)
#define COMPAT_JPEG_DEC_IOCTL_CHKSUM \
	_IOWR(JPEG_IOCTL_MAGIC,  8, struct _compat_JpegDrvDecResult)
#define COMPAT_JPEG_ENC_IOCTL_WAIT \
	_IOWR(JPEG_IOCTL_MAGIC, 13, struct _compat_JPEG_ENC_DRV_OUT)
#endif
#endif //#ifndef __JPEG_DRV_H__
