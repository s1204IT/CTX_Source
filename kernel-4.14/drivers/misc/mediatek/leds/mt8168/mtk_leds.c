...
struct cust_mt65xx_led *get_cust_led_dtsi(void)
{
	...
	if (pled_dtsi) {
		LEDS_DEBUG("[LED] %s pled_dtsi not null\n", __func__);
		goto out;
	}
	...
	for (i = 0; i < TYPE_TOTAL; i++) {
		...
		ret =
		    of_property_read_u32(led_node, "maxlevel",
					 &data);
		if (!ret) {
			pled_dtsi[i].maxlevel = data;
			LEDS_DEBUG("The %s's led maxlevel is : %d\n",
			     pled_dtsi[i].name, pled_dtsi[i].maxlevel);
		} else {
			LEDS_DEBUG("led dts can not get %s led maxlevel\n",
			    pled_dtsi[i].name);
			pled_dtsi[i].maxlevel = 1023;
		}
