...
{
	/* 1.UVLO off */
	kernel_output_reg(MT6390_TOP_RST_STATUS);
	kernel_output_reg(MT6390_PONSTS);
	kernel_output_reg(MT6390_POFFSTS);
	/* 2.thermal shutdown 150 */
	kernel_output_reg(MT6390_THERMALSTATUS);
	/* 3.power not good */
	kernel_output_reg(MT6390_PG_SDN_STS0);
	/* 4.BUCK OC status */
	kernel_output_reg(MT6390_OC_SDN_STS0);
	kernel_output_reg(MT6390_BUCK_TOP_OC_CON0);
	/* 4.5 BUCK OC shutdown control */
	kernel_output_reg(MT6390_BUCK_TOP_ELR0);
	/* 5.long press shutdown */
	kernel_output_reg(MT6390_STRUP_CON4);
	/* 6.WDTRST */
	kernel_output_reg(MT6390_TOP_RST_MISC);
	/* 7.CLK TRIM */
	kernel_output_reg(MT6390_TOP_CLK_TRIM);
}
...
{
	/* 1.UVLO off */
	both_output_reg(MT6390_TOP_RST_STATUS);
	both_output_reg(MT6390_PONSTS);
	both_output_reg(MT6390_POFFSTS);
	/* 2.thermal shutdown 150 */
	both_output_reg(MT6390_THERMALSTATUS);
	/* 3.power not good */
	both_output_reg(MT6390_PG_SDN_STS0);
	/* 4.BUCK OC */
	both_output_reg(MT6390_OC_SDN_STS0);
	both_output_reg(MT6390_BUCK_TOP_OC_CON0);
	/* 4.5 BUCK OC shutdown control */
	both_output_reg(MT6390_BUCK_TOP_ELR0);
	/* 5.long press shutdown */
	both_output_reg(MT6390_STRUP_CON4);
	/* 6.WDTRST */
	both_output_reg(MT6390_TOP_RST_MISC);
	/* 7.CLK TRIM */
	both_output_reg(MT6390_TOP_CLK_TRIM);
}
...
int pmic_dump_exception_reg(void)
{
	...
	/* clear BUCK OC */
	ret_val = pmic_config_interface(MT6390_BUCK_TOP_OC_CON0, 0xFF, 0xFF, 0);
	udelay(200);
