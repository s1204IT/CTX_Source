...
static struct mt6390_charger_desc mt6390_chg_default_desc = {
	.ichg = 500000,
	.cv = 4350000,
...
static int mt6390_charger_parse_dt(struct mt6390_charger *info,
				struct device *dev)
{
	...
	info->desc = &mt6390_chg_default_desc;

	desc = devm_kzalloc(dev, sizeof(*desc), GFP_KERNEL);
	if (!desc)
		return -ENOMEM;

	memcpy(desc, &mt6390_chg_default_desc, sizeof(*desc));
	...
	if (of_property_read_string(np, "alias_name",
		...
		info->charger_prop.alias_name = "mt6390";
	}
...
static int mt6390_plug_in(struct charger_device *chg_dev)
{
	int ret = 0;
...
static int mt6390_enable_charging(struct charger_device *chg_dev, bool en)
{
	int ret = 0;
	/* struct mt6390_charger *info = dev_get_drvdata(&chg_dev->dev); */
...
static int mt6390_is_charging_enabled(struct charger_device *chg_dev, bool *en)
{
	unsigned short val;
...
static int mt6390_get_ichg(struct charger_device *chg_dev, u32 *ichg)
{
	int ret = 0;
	u32 array_size;
	u32 val;
...
static int mt6390_set_ichg(struct charger_device *chg_dev, u32 ichg)
{
	int ret = 0;
	u32 array_size;
...
static int mt6390_get_min_ichg(struct charger_device *chg_dev, u32 *uA)
{
	u32 array_size;
...
static int mt6390_get_cv(struct charger_device *chg_dev, u32 *cv)
{
	int ret = 0;
	u32 array_size;
	u32 val;
...
static int mt6390_set_cv(struct charger_device *chg_dev, u32 cv)
{
	int ret = 0;
	u32 array_size;
...
static int mt6390_kick_wdt(struct charger_device *chg_dev)
{
	int ret = 0;
...
static int mt6390_dump_register(struct charger_device *chg_dev)
{
	int ret = 0;
	u32 i = 0;
	u32 ichg = 0, cv = 0;
	...
	ret = mt6390_get_ichg(chg_dev, &ichg);
	ret = mt6390_get_cv(chg_dev, &cv);
	ret = mt6390_is_charging_enabled(chg_dev, &chg_en);

	for (i = MT6390_CHR_TOP_CON0; i <= MT6390_PCHR_ELR1; i += 2)
		chr_debug("[0x%x]=0x%x\t", i, upmu_get_reg_value(i));
	chr_debug("\n");

	for (i = MT6390_CHR_CON0; i <= MT6390_CHR_CON9; i += 2)
		chr_debug("[0x%x]=0x%x\t", i, upmu_get_reg_value(i));
	chr_debug("\n");
...
static int mt6390_do_event(struct charger_device *chg_dev, u32 event, u32 args)
{
	switch (event) {
	case EVENT_EOC:
...
static struct charger_ops mt6390_charger_ops = {
	/* normal charging */
	.plug_in = mt6390_plug_in,
	/* .plug_out = mt6390_plug_out, */
	.enable = mt6390_enable_charging,
	.is_enabled = mt6390_is_charging_enabled,
	.get_charging_current = mt6390_get_ichg,
	.set_charging_current = mt6390_set_ichg,
	.get_min_charging_current = mt6390_get_min_ichg,
	.get_constant_voltage = mt6390_get_cv,
	.set_constant_voltage = mt6390_set_cv,
	.kick_wdt = mt6390_kick_wdt,
	.dump_registers = mt6390_dump_register,

	/* Event */
	.event = mt6390_do_event,
};

static void watchdog_int_handler(void)
{
	pr_notice("mt6390 CHRWDT IRQ\n");
}

static int mt6390_charger_init_setting(struct mt6390_charger *info)
{
	int ret = 0;
	unsigned short val;
	...
	/* 32s */
	pmic_set_register_value(PMIC_RG_CHRWDT_TD, 3);
...
static int mt6390_charger_probe(struct platform_device *pdev)
{
	int ret = 0;
	...
	mt6390_charger_parse_dt(info, &pdev->dev);
	platform_set_drvdata(pdev, info);

	/* Register charger device */
	info->charger_dev = charger_device_register(info->charger_dev_name,
		&pdev->dev, info, &mt6390_charger_ops, &info->charger_prop);
	...
	mt6390_charger_init_setting(info);
...
static int mt6390_charger_remove(struct platform_device *pdev)
{
	struct mt6390_charger *info = platform_get_drvdata(pdev);
...
static const struct of_device_id mt6390_charger_of_match[] = {
	{.compatible = "mediatek,mt6357_charger",},
	{.compatible = "mediatek,mt6390_charger",},
	{},
};

MODULE_DEVICE_TABLE(of, mt6390_charger_of_match);

static struct platform_driver mt6390_charger_driver = {
	.probe = mt6390_charger_probe,
	.remove = mt6390_charger_remove,
	.driver = {
		   .name = "mt6390_charger",
		   .of_match_table = mt6390_charger_of_match,
		   },
};

static int __init mt6390_charger_init(void)
{
	return platform_driver_register(&mt6390_charger_driver);
}
module_init(mt6390_charger_init);

static void __exit mt6390_charger_exit(void)
{
	platform_driver_unregister(&mt6390_charger_driver);
}
module_exit(mt6390_charger_exit);
