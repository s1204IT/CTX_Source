...
int PMIC_check_battery(void)
{
	...
	mt6390_upmu_set_rg_baton_en(1);
	/*PMIC_upmu_set_baton_tdet_en(1);*/
	val = mt6390_upmu_get_rgs_baton_undet();
	if (val == 0) {
...
int PMIC_POWER_HOLD(unsigned int hold)
{
	...
	/* MT6390 must keep power hold */
	pmic_config_interface_nolock(PMIC_RG_PWRHOLD_ADDR, hold
				     , PMIC_RG_PWRHOLD_MASK
				     , PMIC_RG_PWRHOLD_SHIFT);
	PMICLOG("[PMIC_KERNEL] MT6390 PowerHold = 0x%x\n"
		, upmu_get_reg_value(MT6390_PPCCTL0));
...
void PMIC_LP_INIT_SETTING(void)
{
	...
	/* Workaround setting for MT6390 MRV */
	if (is_pmic_mrv()) {
		/* Suspend */
		...
		for (i = 0; i < pmic_regulator_ldo_matches_size; i++) {
			if (strncmp(mt_ldos[i].desc.name,
					"vsram_others", 12) == 0) {
				mt_ldos[i].en_cb =
					mt6390_upmu_set_rg_ldo_vsram_proc_en;
				mt_ldos[i].vol_cb =
					mt6390_upmu_set_rg_ldo_vsram_proc_vosel;
				mt_ldos[i].da_en_cb =
					mt6390_upmu_get_da_vsram_proc_en;
				mt_ldos[i].da_vol_cb =
					mt6390_upmu_get_da_vsram_proc_vosel;
			} else if (strncmp(mt_ldos[i].desc.name,
					"vsram_proc", 10) == 0) {
				mt_ldos[i].en_cb =
					mt6390_upmu_set_rg_ldo_vsram_others_en;
				mt_ldos[i].vol_cb =
				mt6390_upmu_set_rg_ldo_vsram_others_vosel;
				mt_ldos[i].da_en_cb =
					mt6390_upmu_get_da_vsram_others_en;
				mt_ldos[i].da_vol_cb =
					mt6390_upmu_get_da_vsram_others_vosel;
			}
		}
	} else {
		/* Suspend */
