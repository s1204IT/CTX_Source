...
/* mt6390 0.0625 , need to * 10000 and / 10000 */
#define UNIT_TIME          (50)
/* mt6390: 19.646 * 1000, need to divide 1000 */
...
static void fgauge_read_RTC_boot_status(struct gauge_device *gauge_dev)
{
	...
	"[%s] rtc_invalid %d plugout %d plugout_time %d spare3 0x%x\n",
			...
			is_bat_plugout, bat_plug_out_time,
			spare3_reg);
}
	...
	struct gauge_device *gauge_dev, bool *fg_is_charging, int *data)
{
	 ...
	 *(1)    Set READ command
	 */
	ret = pmic_config_interface(MT6390_FGADC_CON1, 0x0001, 0x000F, 0x0);
	 ...
	 *(6)    Clear status to 0
	 */
	ret = pmic_config_interface(MT6390_FGADC_CON1, 0x0008, 0x000F, 0x0);
	/*
	...
	ret = pmic_config_interface(MT6390_FGADC_CON1, 0x0000, 0x000F, 0x0);
 ...
 */
	...
	if (reset == 0)
		ret = pmic_config_interface(
			MT6390_FGADC_CON1, 0x0001, 0x1F05, 0x0);
	else {
		ret = pmic_config_interface(
			MT6390_FGADC_CON1, 0x0705, 0x1F05, 0x0);
...
static int fgauge_reset_hw(struct gauge_device *gauge_dev)
{
	...
	while (val_car != 0x0) {
		ret = pmic_config_interface(
			MT6390_FGADC_CON1, 0x0600, 0x1F00, 0x0);
...
static int read_hw_ocv_6390_plug_in(void)
{
...
static int read_hw_ocv_6390_power_on(void)
{
...
static int read_hw_ocv_6390_power_on_rdy(void)
{
...
int read_hw_ocv(struct gauge_device *gauge_dev, int *data)
{
	...
	_hw_ocv_35_pon_rdy = read_hw_ocv_6390_power_on_rdy();
	_hw_ocv_35_pon = read_hw_ocv_6390_power_on();
	_hw_ocv_35_plugin = read_hw_ocv_6390_plug_in();
	...
	_hw_ocv_src = FROM_6390_PON_ON;
	_prev_hw_ocv = _hw_ocv;
	_prev_hw_ocv_src = FROM_6390_PON_ON;
	_flag_unreliable = 0;
 ...
 */
	if (reset == 0) {
		ret = pmic_config_interface(
				MT6390_FGADC_CON1, 0x0001, 0x1F0F, 0x0);
	} else {
		ret = pmic_config_interface(
				MT6390_FGADC_CON1, 0x1F05, 0xFF0F, 0x0);
	...
	struct gauge_device *gauge_dev, int *is_plugout, int *plutout_time)
{
	...
	*plutout_time = bat_plug_out_time;
	bm_err(
		...
		__func__, rtc_invalid, is_bat_plugout,
	...
	int fg_zcv_car_th)
{
	...
	/* mt6390 set UNIT_FGCAR_ZCV to 19646 */
...
static int fgauge_set_rtc_ui_soc(struct gauge_device *gauge_dev, int rtc_ui_soc)
{
	...
	bm_notice("[%s] rtc_ui_soc %d spare3_reg 0x%x new_spare3_reg 0x%x\n",
		__func__, rtc_ui_soc, spare3_reg, new_spare3_reg);
...
static struct gauge_ops mt6390_gauge_ops = {
...
static int mt6390_parse_dt(struct mt6390_gauge *info, struct device *dev)
{
...
static int mt6390_gauge_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct mt6390_gauge *info;

	bm_debug("%s: starts\n", __func__);

	info = devm_kzalloc(
		&pdev->dev,
		sizeof(struct mt6390_gauge),
		GFP_KERNEL);
	if (!info)
		return -ENOMEM;

	mt6390_parse_dt(info, &pdev->dev);
	platform_set_drvdata(pdev, info);

	/* Register charger device */
	info->gauge_dev = gauge_device_register(info->gauge_dev_name,
		&pdev->dev, info, &mt6390_gauge_ops, &info->gauge_prop);
...
static int mt6390_gauge_remove(struct platform_device *pdev)
{
	struct mt6390_gauge *mt = platform_get_drvdata(pdev);
...
static void mt6390_gauge_shutdown(struct platform_device *dev)
{
}


static const struct of_device_id mt6390_gauge_of_match[] = {
	{.compatible = "mediatek,mt6357_gauge",},
	{.compatible = "mediatek,mt6390_gauge",},
	{},
};

MODULE_DEVICE_TABLE(of, mt6390_gauge_of_match);

static struct platform_driver mt6390_gauge_driver = {
	.probe = mt6390_gauge_probe,
	.remove = mt6390_gauge_remove,
	.shutdown = mt6390_gauge_shutdown,
	.driver = {
		   .name = "mt6390_gauge",
		   .of_match_table = mt6390_gauge_of_match,
		   },
};

static int __init mt6390_gauge_init(void)
{
	return platform_driver_register(&mt6390_gauge_driver);
}
device_initcall(mt6390_gauge_init);

static void __exit mt6390_gauge_exit(void)
{
	platform_driver_unregister(&mt6390_gauge_driver);
}
module_exit(mt6390_gauge_exit);
