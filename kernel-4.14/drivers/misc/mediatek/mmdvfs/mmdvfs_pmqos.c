...
void mm_qos_update_larb_bwl(u32 larb_update, bool bw_change)
{
	...
	for (i = 0; i < SMI_COMM_MASTER_NUM; i++) {
		if (!(larb_update & (1 << i)))
			continue;
		...
		if (larb_bw) {
			...
			smi_bwl_update(i, default_bwl, true, "MMDVFS");
	...
	u32 hrt_value, u32 comp_type)
{
	...
	if (req->hrt_value) {
		...
		if (larb < MAX_LARB_COUNT &&
			larb_req[larb].comm_port < SMI_COMM_MASTER_NUM)
...
void mm_qos_update_all_request(struct plist_head *owner_list)
{
	...
	s32 cam_bw;
#ifdef MMDVFS_MMP
	u32 larb_count = 0, larb_port_id = 0, larb_port_bw = 0;
#endif
	u32 larb_id = 0;
	u32 port_id = 0;
	...
	for (i = 0; i < MAX_LARB_COUNT; i++)
		mm_bw += larb_req[i].total_bw_data;
...
static void init_virtual_larbs(void)
{
	...
	larb_req[SMI_PMQOS_LARB_DEC(PORT_VIRTUAL_MD)].total_hrt_data = 1600;
}
#endif
	...
	struct mm_freq_limit_config *limit_config)
{
	...
	for (i = 0; i < limit_size; i++) {
		limit_config->limit_steps[i] = kzalloc(MAX_FREQ_STEP,
