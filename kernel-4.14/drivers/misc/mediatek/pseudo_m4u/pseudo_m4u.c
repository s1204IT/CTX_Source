...
struct sg_table *m4u_create_sgtable(unsigned long va, unsigned int size)
{
	...
	m4u_pr_debug("%s va=0x%lx, PG OFF=0x%lx, VM START~END=0x%lx~0x%lx\n",
	...
	if (ret < 0) {
		m4u_pr_err("%s get frmvec pg fail\n", __func__);
	...
	if (ret) {
		m4u_pr_err("%s set tbl pages fail\n", __func__);
		goto m4u_create_sgtbl_out;
	}
...
#ifdef M4U_TEE_SERVICE_ENABLE
/*
 * Init tz m4u and reserve iova regions in arm32/arm64 evb are helped via
 * mtk_iommu and iommu framework.
 */
unsigned int mtk_init_tz_m4u(void)
{
	...
	static unsigned int tz_meu_inited;
