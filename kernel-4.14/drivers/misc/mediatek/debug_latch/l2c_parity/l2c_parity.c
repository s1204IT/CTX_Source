...
static int enable_l2c_parity_interrupt(void)
{
	struct device_node *node;
	int ret = -EINVAL;

	node = of_find_compatible_node(NULL, NULL, MCUCFG);
	if (node) {
		void __iomem *mcusys_base;

		mcusys_base = of_iomap(node, 0);
		if (mcusys_base) {
			writel(MP0_L2C_PARITY_MASK, mcusys_base +
				L2C_PARITY_INTERRUPT_MASK);
			iounmap(mcusys_base);
			ret = 0;
		}
	}
...
static int l2c_parity_probe(struct platform_device *pdev)
{
	...
	return enable_l2c_parity_interrupt();
}
