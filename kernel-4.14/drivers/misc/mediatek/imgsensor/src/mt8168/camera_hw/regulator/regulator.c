...
static void imgsensor_oc_handler1(void)
{
	...
	cam_pr_debug("[regulator]%s enter vcama oc %d\n",
		__func__,
...
{
	cam_pr_debug("[regulator]%s enter vcamd oc %d\n",
		__func__,
		gimgsensor.status.oc);
	gimgsensor.status.oc = 1;
	aee_kernel_warning("Imgsensor OC", "Over current");
...
{
	cam_pr_debug("[regulator]%s enter vcamio oc %d\n",
		__func__,
		gimgsensor.status.oc);
	gimgsensor.status.oc = 1;
	aee_kernel_warning("Imgsensor OC", "Over current");
		...
		if (sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN) {
			...
			if (preg && regulator_is_enabled(preg)) {
				pmic_enable_interrupt(
					...
					"camera");
				regulator_put(preg);
				cam_pr_debug("[regulator] %s INT_VCAMA_OC %d\n",
					__func__, enable);
			}
		...
		if (sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN2) {
			...
			if (preg && regulator_is_enabled(preg)) {
				pmic_enable_interrupt(
					...
					"camera");
				regulator_put(preg);
				cam_pr_debug("[regulator] %s INT_VCAMD_OC %d\n",
					__func__, enable);
			}
		}
		...
		if (sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN ||
			...
			if (preg && regulator_is_enabled(preg)) {
				pmic_enable_interrupt(
					...
					"camera");
				regulator_put(preg);
				cam_pr_debug("[regulator] %s INT_VCAMIO_OC %d\n",
					__func__, enable);
			}
		}
		rcu_read_lock();
...
static enum IMGSENSOR_RETURN regulator_init(void *pinstance)
{
	...
	if (pdevice->of_node == NULL) {
		cam_pr_err("regulator get cust camera node failed!\n");
	...
	if (pregulator) {
		if (pin_state != IMGSENSOR_HW_PIN_STATE_LEVEL_0) {
			...
			if (regulator_set_voltage(
				...
				cam_pr_err("[regulator]fail to regulator_set_voltage, powertype:%d powerId:%d\n",
					pin,
					regulator_voltage[
					pin_state -
			...
			if (regulator_enable(pregulator)) {
				cam_pr_err("[regulator]fail to regulator_enable, powertype:%d powerId:%d\n",
					pin,
					regulator_voltage[
					pin_state -
					IMGSENSOR_HW_PIN_STATE_LEVEL_0]);
			...
			if (regulator_is_enabled(pregulator)) {
				cam_pr_debug("[regulator]%d is enabled\n", pin);
		...
		cam_pr_err("regulator == NULL %d %d %d\n",
		    reg_type_offset,
		    pin,
