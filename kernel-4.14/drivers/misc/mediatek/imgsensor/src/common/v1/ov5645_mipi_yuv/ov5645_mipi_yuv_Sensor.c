...
static enum MSDK_SCENARIO_ID_ENUM CurrentScenarioId =
...
static kal_uint16 OV5645MIPI_read_cmos_sensor(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, OV5645MIPI_WRITE_ID);
	return get_byte;
}

static void OV5645MIPI_write_cmos_sensor(
	kal_uint32 addr,
	kal_uint32 para)
{
	...
	iWriteRegI2C(pusendcmd, 3, OV5645MIPI_WRITE_ID);
}
...
void printAE_2_ARRAY(void)
{
	...
	for (i = 0; i < AE_VERTICAL_BLOCKS; i++)
		cam_pr_debug("      line[%2d]", i);
	cam_pr_debug("\n");
	for (j = 0; j < AE_HORIZONTAL_BLOCKS; j++) {
		cam_pr_debug("\trow[%2d]", j);
		for (i = 0; i < AE_VERTICAL_BLOCKS; i++)
			// SENSORDB("AE_2_ARRAY[%2d][%2d]=%d\n",
			// j,i,AE_2_ARRAY[j][i]);
			cam_pr_debug("  %7d", AE_2_ARRAY[j][i]);
		cam_pr_debug("\n");
	}
}
...
{				/* line[5] */
	...
	for (i = 0; i <= AE_VERTICAL_BLOCKS; i++) {
		*(&line_coordinate[0] + i) = step * i;
		cam_pr_debug("line[%d]=%d\t", i, *(&line_coordinate[0] + i));
	}
	cam_pr_debug("\n");
}
...
{				/* row[5] */
	...
	for (i = 0; i <= AE_HORIZONTAL_BLOCKS; i++) {
		*(&row_coordinate[0] + i) = step * i;
		cam_pr_debug("row[%d]=%d\t", i, *(&row_coordinate[0] + i));
	}
	cam_pr_debug("\n");
}
	...
	UINT32 *linenum, UINT32 *rownum)
{
	...
	cam_pr_debug("PV point [%d, %d] to section line coordinate[%d] row[%d]\n",
		x, y, *linenum, *rownum);
}
	...
	UINT32 *sectionlinenum, UINT32 *sectionrownum)
{
	...
	*sectionrownum = clampSection(rownum - 1, 0, AE_HORIZONTAL_BLOCKS - 1);
	cam_pr_debug("%s from[%d][%d] to[%d][%d]\n", __func__,
...
void OV5645_FOCUS_Set_AE_Window(UINT32 *zone_addr)
{
	...
	cam_pr_debug("AE x0=%d,y0=%d,x1=%d,y1=%d,FD_XS=%d,FD_YS=%d\n",
...
static kal_uint32 OV5645MIPIReadShutter(void)
{
	...
	cam_pr_debug("enter %s function:\n ", __func__);
	/* AEC[b19~b16] */
	temp_reg1 = OV5645MIPI_read_cmos_sensor(0x3500);
	/* AEC[b15~b8] */
	temp_reg2 = OV5645MIPI_read_cmos_sensor(0x3501);
	/* AEC[b7~b0] */
	temp_reg3 = OV5645MIPI_read_cmos_sensor(0x3502);
...
static kal_uint32 OV5645MIPIReadExtraShutter(void)
{
	...
	cam_pr_debug("enter %s function:\n ", __func__);
	/* AEC[b15~b8] */
	temp_reg1 = OV5645MIPI_read_cmos_sensor(0x350c);
	/* AEC[b7~b0] */
	temp_reg2 = OV5645MIPI_read_cmos_sensor(0x350d);
	spin_lock(&ov5645mipi_drv_lock);
...
static kal_uint32 OV5645MIPIReadSensorGain(void)
{
	...
	cam_pr_debug("enter %s function:\n ", __func__);
	sensor_gain = (OV5645MIPI_read_cmos_sensor(0x350B) & 0xFF);
	cam_pr_debug("exit %s function:\n ", __func__);
	return sensor_gain;
}				/* OV5645MIPIReadSensorGain */
...
static void OV5645MIPI_set_AE_mode(kal_bool AE_enable)
{
	...
	cam_pr_debug("enter %s function:\n ", __func__);
	AeTemp = OV5645MIPI_read_cmos_sensor(0x3503);
...
static void OV5645MIPI_set_AWB_mode(kal_bool AWB_enable)
{
	...
	cam_pr_debug("enter %s function:\n ", __func__);
	AwbTemp = OV5645MIPI_read_cmos_sensor(0x3406);
...
void OV5645MIPI_night_mode(kal_bool enable)
{
	...
	kal_uint16 night = OV5645MIPI_read_cmos_sensor(0x3A00);
...
static kal_uint32 OV5645MIPI_GetSensorID(kal_uint32 *sensorID)
{
	...
	for (i = 0; i < 3; i++) {
		*sensorID =
		    (OV5645MIPI_read_cmos_sensor(0x300A) << 8) |
		    OV5645MIPI_read_cmos_sensor(0x300B);
		if (*sensorID != OV5645MIPI_SENSOR_ID) {
			cam_pr_debug("Read Sensor ID Fail = 0x%04x\n",
				*sensorID);
			*sensorID = 0xFFFFFFFF;
...
static void OV5645MIPISetHVMirror(kal_uint8 Mirror, kal_uint8 Mode)
{
	...
	cam_pr_debug("enter %s %d %d:\n ", __func__, Mode, Mirror);
	flip = OV5645MIPI_read_cmos_sensor(0x3820);
	mirror = OV5645MIPI_read_cmos_sensor(0x3821);
	...
	UINT32 *pFeatureReturnPara32)
{
	*pFeatureReturnPara32 = 1;
	cam_pr_debug(" *pFeatureReturnPara32 = %d\n", *pFeatureReturnPara32);
}
...
static void OV5645_FOCUS_OVT_AFC_Touch_AF(UINT32 x, UINT32 y)
{
	...
	cam_pr_debug("AF x_view=%d,y_view=%d\n", x_view, y_view);
	OV5645MIPI_write_cmos_sensor(0x3024, x_view);
	OV5645MIPI_write_cmos_sensor(0x3025, y_view);
	x_tmp = OV5645MIPI_read_cmos_sensor(0x3024);
	y_tmp = OV5645MIPI_read_cmos_sensor(0x3025);
	cam_pr_debug("AF x_tmp1=%d,y_tmp1=%d\n", x_tmp, y_tmp);
	cam_pr_debug("exit %s function:\n ", __func__);
}
...
static void OV5645_FOCUS_Set_AF_Window(UINT32 *zone_addr)
{
	...
	mapMiddlewaresizePointToPreviewsizePoint(x1, y1, FD_XS, FD_YS,
		&pvx1, &pvy1, PRV_W, PRV_H);
	cam_pr_debug("AF pvx0=%d,pvy0=%d\n", pvx0, pvy0);
	cam_pr_debug("AF pvx0=%d,pvy0=%d\n", pvx1, pvy1);
	AF_pvx = (pvx0 + pvx1) / 32;
	AF_pvy = (pvy0 + pvy1) / 32;
	cam_pr_debug("AF AF_pvx=%d,AF_pvy=%d\n", AF_pvx, AF_pvy);
...
static void OV5645_FOCUS_OVT_AFC_Get_AF_Status(UINT32 *pFeatureReturnPara32)
{
	...
	*pFeatureReturnPara32 = SENSOR_AF_IDLE;
	state_3028 = OV5645MIPI_read_cmos_sensor(0x3028);
	state_3029 = OV5645MIPI_read_cmos_sensor(0x3029);
	mDELAY(1);
	cam_pr_debug("enter %s function:state_3028=%d,state_3029=%d\n",
		__func__, state_3028, state_3029);
	if (state_3028 == 0) {
	...
	cam_pr_debug("exit %s function:state_3028=%d,state_3029=%d\n", __func__,
	...
	kal_uint32 color_b_gain)
{
	...
	kal_uint8 temp = OV5645MIPI_read_cmos_sensor(0x350b);
...
UINT32 OV5645MIPIOpen(void)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (sensor_id != OV5645MIPI_SENSOR_ID) {
			cam_pr_debug("Read Sensor ID Fail[open] = 0x%x\n",
					   sensor_id);
			return ERROR_SENSOR_CONNECT_FAIL;
		}
	}
	cam_pr_debug("Sensor Read ID OK\n");
			 ...
			 MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	cam_pr_debug("----Aaron------read register 0x3501=[%x]\n",
			   OV5645MIPI_read_cmos_sensor(0x3501));
	cam_pr_debug("----Aaron------read register 0x3502=[%x]\n",
			   OV5645MIPI_read_cmos_sensor(0x3502));
	cam_pr_debug("----Aaron------read register 0x350c=[%x]\n",
			   OV5645MIPI_read_cmos_sensor(0x350c));
	cam_pr_debug("----Aaron------read register 0x350d=[%x]\n",
			   OV5645MIPI_read_cmos_sensor(0x350d));
	cam_pr_debug("----Aaron------read register 0x350b=[%x]\n",
			   OV5645MIPI_read_cmos_sensor(0x350b));
	cam_pr_debug("exit %s function:\n ", __func__);
	return ERROR_NONE;
			 ...
			 MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (OV5645MIPISensor.SensorMode == SENSOR_MODE_PREVIEW) {
		...
		color_r_gain =
		    ((OV5645MIPI_read_cmos_sensor(0x3401) & 0xFF) +
		     ((OV5645MIPI_read_cmos_sensor(0x3400) & 0xFF) * 256));
		color_b_gain =
		    ((OV5645MIPI_read_cmos_sensor(0x3405) & 0xFF) +
		     ((OV5645MIPI_read_cmos_sensor(0x3404) & 0xFF) * 256));
		OV5645MIPIFullSizeCaptureSetting();
...
UINT32 OV5645MIPIGetInfo(enum MSDK_SCENARIO_ID_ENUM ScenarioId,
...
UINT32 OV5645MIPIControl(enum MSDK_SCENARIO_ID_ENUM ScenarioId,
...
#endif
UINT32 OV5645MIPIYUVSensorSetting(enum FEATURE_ID iCmd, UINT32 iPara)
{
	cam_pr_debug("enter %s function:\n", __func__);
	switch (iCmd) {
	case FID_SCENE_MODE:
		cam_pr_debug("Night Mode:%d\n", iPara);
	...
	case FID_ZOOM_FACTOR:
		cam_pr_debug("FID_ZOOM_FACTOR:%d\n", iPara);
		spin_lock(&ov5645mipi_drv_lock);
		zoom_factor = iPara;
		spin_unlock(&ov5645mipi_drv_lock);
		break;
...
UINT32 OV5645MIPIYUVSetVideoMode(UINT16 u2FrameRate)
{
	...
	if (u2FrameRate == 30) {
		/* ;OV5645MIPI 1280x960,30fps */
		/* 56Mhz, 224Mbps/Lane, 2Lane. */
		cam_pr_debug("%s enter u2FrameRate == 30 setting.\n",
			__func__);
		/* fix30 */
		OV5645MIPI_write_cmos_sensor(0x3A00, 0x38);
		/* ; MIPI 2 lane */
		...
		cam_pr_debug("%s exit u2FrameRate == 30 setting:\n", __func__);
		...
		OV5645MIPI_write_cmos_sensor(0x350c, 0x00);	/*  */
		OV5645MIPI_write_cmos_sensor(0x350d, 0x00);	/*  */
		cam_pr_debug("%s exit u2FrameRate == 20 setting.\n",
			__func__);
	} else if (u2FrameRate == 15) {
		/* ;OV5645MIPI 1280x960,15fps */
		/* 28Mhz, 112Mbps/Lane, 2Lane. */
		cam_pr_debug("%s enter u2FrameRate == 15 setting.\n",
			__func__);
		...
		OV5645MIPI_write_cmos_sensor(0x350c, 0x00);	/*  */
		OV5645MIPI_write_cmos_sensor(0x350d, 0x00);	/*  */
		cam_pr_debug("%s exit u2FrameRate == 15 setting:\n", __func__);
	} else {
		cam_pr_debug("Wrong frame rate setting\n");
	}
	cam_pr_debug("exit %s function:\n ", __func__);
	return TRUE;
}
...
static void OV5645MIPIGetCurAeAwbInfo(UINT32 pSensorAEAWBCurStruct)
{
	...
	Info->SensorAwbGainCur.AwbCurRgain =
			OV5645MIPI_read_cmos_sensor(0x504c);
	Info->SensorAwbGainCur.AwbCurBgain =
			OV5645MIPI_read_cmos_sensor(0x504e);
#endif
}
	...
	enum MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate)
{
	...
	cam_pr_debug("%s: scenarioId = %d, frame rate = %d\n", __func__,
...
int flag_torch;
static void OV5645MIPI_FlashTriggerCheck(
	unsigned int *pFeatureReturnPara32)
{
	unsigned int NormBr;

	flag_torch = 1;
	NormBr = OV5645MIPI_read_cmos_sensor(0x56A1);
	cam_pr_debug("################NormBr is %d\n", NormBr);
...
static UINT32 streaming_control(kal_bool enable)
{
...
UINT32 OV5645MIPIFeatureControl(
	...
	UINT32 *pFeatureParaLen)
{
	...
	case SENSOR_FEATURE_SET_ESHUTTER:
		// cam_pr_debug("OV5645MIPIFeatureControl:
	...
	case SENSOR_FEATURE_CHECK_SENSOR_ID:
		cam_pr_debug("SENSOR_FEATURE_CHECK_SENSOR_ID\n");
		OV5645MIPI_GetSensorID(pFeatureData32);
		break;
	case SENSOR_FEATURE_SET_YUV_CMD:
		// cam_pr_debug("OV5645MIPIFeatureControl:
		//	SENSOR_FEATURE_SET_YUV_CMD\n");
		OV5645MIPIYUVSensorSetting((enum FEATURE_ID) *pFeatureData32,
					*(pFeatureData32 + 1));
		break;
	...
	case SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO:
		OV5645MIPIMaxFramerateByScenario(
			(enum MSDK_SCENARIO_ID_ENUM) *pFeatureData32,
			 *(pFeatureData32 + 1));
		break;
	...
	case SENSOR_FEATURE_INITIALIZE_AF:
		cam_pr_debug("SENSOR_FEATURE_INITIALIZE_AF\n");
		OV5645_FOCUS_OVT_AFC_Init();
		break;
	case SENSOR_FEATURE_MOVE_FOCUS_LENS:
		cam_pr_debug("SENSOR_FEATURE_MOVE_FOCUS_LENS\n");
		OV5645_FOCUS_Move_to(*pFeatureData16);
		break;
	case SENSOR_FEATURE_GET_AF_STATUS:
		cam_pr_debug("SENSOR_FEATURE_GET_AF_STATUS\n");
		OV5645_FOCUS_OVT_AFC_Get_AF_Status(pFeatureReturnPara32);
		*pFeatureParaLen = 4;
		break;
	case SENSOR_FEATURE_SINGLE_FOCUS_MODE:
		cam_pr_debug("SENSOR_FEATURE_SINGLE_FOCUS_MODE\n");
		OV5645_FOCUS_OVT_AFC_Single_Focus();
		break;
	case SENSOR_FEATURE_CONSTANT_AF:
		cam_pr_debug("SENSOR_FEATURE_CONSTANT_AF\n");
		OV5645_FOCUS_OVT_AFC_Constant_Focus();
		break;
	case SENSOR_FEATURE_CANCEL_AF:
		cam_pr_debug("SENSOR_FEATURE_CANCEL_AF\n");
		OV5645_FOCUS_OVT_AFC_Cancel_Focus();
		break;
	case SENSOR_FEATURE_GET_AF_INF:
		cam_pr_debug("SENSOR_FEATURE_GET_AF_INF\n");
		OV5645_FOCUS_Get_AF_Inf(pFeatureReturnPara32);
		*pFeatureParaLen = 4;
		break;
	case SENSOR_FEATURE_GET_AF_MACRO:
		cam_pr_debug("SENSOR_FEATURE_GET_AF_MACRO\n");
		OV5645_FOCUS_Get_AF_Macro(pFeatureReturnPara32);
		*pFeatureParaLen = 4;
		break;
	case SENSOR_FEATURE_SET_AF_WINDOW:
		cam_pr_debug("SENSOR_FEATURE_SET_AF_WINDOW\n");
		OV5645_FOCUS_Set_AF_Window(pFeatureData32);
		break;
	case SENSOR_FEATURE_GET_AF_MAX_NUM_FOCUS_AREAS:
		cam_pr_debug("SENSOR_FEATURE_GET_AF_MAX_NUM_FOCUS_AREAS\n");
	...
	case SENSOR_FEATURE_GET_AE_MAX_NUM_METERING_AREAS:
		cam_pr_debug("AE zone addr = 0x%x\n", *pFeatureData32);
...
struct SENSOR_FUNCTION_STRUCT SensorFuncOV5645MIPI = {
	OV5645MIPIOpen,
	OV5645MIPIGetInfo,
	OV5645MIPIGetResolution,
	OV5645MIPIFeatureControl,
