...
enum IMGSENSOR_RETURN imgsensor_i2c_buffer_mode(int enable)
{
	...
	cam_pr_debug("i2c_buf_mode_en %d\n", enable);

	if (pinst->pi2c_client == NULL) {
		cam_pr_err("pi2c_client is NULL!\n");
		return IMGSENSOR_RETURN_ERROR;
	}
	...
	int speed)
{
	...
	if (i2c_transfer(
		...
		pinst->msg,
		IMGSENSOR_I2C_MSG_SIZE_READ) < 0) {
		static DEFINE_RATELIMIT_STATE(ratelimit, 1 * HZ, 30);

		if (__ratelimit(&ratelimit))
			cam_pr_err(
			"I2C read failed (0x%x)! speed(0=%d) (0x%x), i2c_transfer %d\n",
	...
	int speed)
{
	...
	if (i != i2c_transfer(
		...
		if (__ratelimit(&ratelimit))
			cam_pr_err(
				"I2C write failed (0x%x)! speed(0=%d) (0x%02x%02x), i2c_transfer %d\n",
				ret,
				speed,
				*pwrite_data, *(pwrite_data+1),
				i2c_transfer(
					pinst->pi2c_client->adapter,
					pinst->msg,
					i));
