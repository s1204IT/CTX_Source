...
static unsigned int rdma_index(enum mtk_ddp_comp_id module)
{
	int idx = 0;
				   ...
				   struct drm_crtc *crtc, void *handle)
{
	...
	MTK_DRM_DEBUG_DRIVER("rdma %d crtc 0x%p\n",
			     rdma_index(rdma->ddp_comp.id), crtc);
			    ...
			    unsigned int bpc, void *handle)
{
	...
	mtk_ddp_write_mask(comp, width,
		DISP_REG_RDMA_SIZE_CON_0, 0xfff, handle);
	...
	MTK_DRM_DEBUG(comp->larb_dev,
		      "comp %d rdma %d config %ux%u@%uHZ fifo 0x%X\n",
		      comp->id, rdma_index(comp->id),
		      width, height, vrefresh, reg);
				  ...
				  void *handle)
{
	...
	if (fmt == DRM_FORMAT_UYVY || fmt == DRM_FORMAT_YUYV) {
		...
		mtk_ddp_write_mask(comp, 0, DISP_REG_RDMA_SIZE_CON_0,
				 RDMA_MATRIX_ENABLE, handle);
	}
	...
	mtk_ddp_write_mask(comp, RDMA_MODE_MEMORY, DISP_REG_RDMA_GLOBAL_CON,
...
static const struct mtk_disp_rdma_data mt8168_rdma_driver_data = {
	.fifo_size = 6 * SZ_1K,
};
