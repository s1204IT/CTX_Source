...
#define MAX_REC_COMP 12
void __iomem *gdrm_disp_base[MAX_REC_COMP];
void __iomem *gdrm_hdmi_base[MAX_REC_COMP];
struct mtk_drm_debugfs_table gdrm_disp_table[MAX_REC_COMP] = {
	{ DDP_COMPONENT_OVL0, "OVL0 ", {0, 0xf40}, {0x260, 0x80} },
	{ DDP_COMPONENT_COLOR0, "COLOR0 ", {0x400, 0xc00}, {0x100, 0x100} },
	{ DDP_COMPONENT_CCORR, "CCORR0 ", {0x400, 0xc00}, {0x100, 0x100} },
	{ DDP_COMPONENT_AAL, "AAL0 ", {0, 0}, {0x100, 0} },
	{ DDP_COMPONENT_GAMMA, "GAMMA0 ", {0, 0}, {0x100, 0} },
	{ DDP_COMPONENT_DITHER, "DITHER0 ", {0, 0}, {0x100, 0} },
	{ DDP_COMPONENT_RDMA0, "RDMA0 ", {0, 0}, {0x100, 0} },
	{ -1, "CONFIG ", {0, 0}, {0x120, 0} },
	{ -1, "MUTEX ", {0, 0}, {0x100, 0} }
};

struct mtk_drm_debugfs_table gdrm_hdmi_table[MAX_REC_COMP] = {
			  ...
			  loff_t *ppos)
{
	if (strncmp(dis_cmd_buf, "regr:", 5) == 0) {
		...
		for (i = 0; i < ARRAY_SIZE(gdrm_disp_table); i++) {
			if (addr >= gdrm_disp_table[i].reg_base &&
			    addr < gdrm_disp_table[i].reg_base +
			    0x1000UL) {
		...
		for (i = 0; i < ARRAY_SIZE(gdrm_hdmi_table); i++) {
			if (addr >= gdrm_hdmi_table[i].reg_base &&
			    addr < gdrm_hdmi_table[i].reg_base +
			    0x1000UL) {
				ret = sprintf(read_buf,
					"%8s Read register 0x%08lX: 0x%08X\n",
			  ...
			  struct mtk_drm_private *priv)
{
	...
	mtkdrm_dbgfs = debugfs_create_file("mtkdrm", 0664,
...
void mtk_drm_ut_debug_printk(const char *function_name, const char *format, ...)
{
	...
	pr_err("[" DRM_NAME ":%s] %pV", function_name, &vaf);
