...
struct mtk_ddp_comp *mtk_wb_find_wdma(struct mtk_drm_crtc *mtk_crtc)
{
...
void mtk_wb_add_comp_to_path(struct mtk_drm_crtc *mtk_crtc)
{
	...
	int hdisplay = mtk_crtc->base.mode.hdisplay;
	int vdisplay = mtk_crtc->base.mode.vdisplay;
	int wb_buf_bpp = 3; /* RGB */
	size_t wb_buf_size = hdisplay*vdisplay*wb_buf_bpp;

	wdma = mtk_wb_find_wdma(mtk_crtc);
	if (wdma == NULL) {
		MTK_DRM_ERROR(drm_dev?(drm_dev->dev):0,
			"cannot find out wdma\n");
		return;
	}

	wdma->mtk_gem = mtk_drm_gem_create(drm_dev, wb_buf_size, true);
...
#endif
{
	...
	if (conn_state->writeback_job != NULL &&
		...
		MTK_DRM_DEBUG_DRIVER(
		"mtk_wb_gem cookie 0x%p kvaddr 0x%p dma_addr 0x%p\n",
		mtk_wb_gem->cookie,
		mtk_wb_gem->kvaddr,
		(void *)mtk_wb_gem->dma_addr);
