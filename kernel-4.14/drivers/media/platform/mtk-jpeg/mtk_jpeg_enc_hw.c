...
{
	*fileSize = mtk_jpeg_enc_get_file_size(base);//for dst size
...
static void mtk_jpeg_enc_set_img_size(void __iomem *base,
...
static void mtk_jpeg_enc_set_src_img(void __iomem *base, u32 width,
		u32 height, u32 yuv_format, u32 totalEncDU)
{
...
static void mtk_jpeg_enc_set_image_stride(void __iomem *base,
	u32 img_stride)
{
	writel((img_stride), base + JPGENC_IMG_STRIDE);
}

static void mtk_jpeg_enc_set_memory_stride(void __iomem *base,
	u32 mem_stride)
{
	writel((mem_stride), base + JPGENC_STRIDE);
}

static void mtk_jpeg_enc_set_luma_addr(void __iomem *base,
	u32 src_luma_addr)
{
...
static void mtk_jpeg_enc_set_dst_buf(void __iomem *base,
	u32 dst_addr, u32 stall_size,
	u32 init_offset, u32 offset_mask)
{
...
static void mtk_jpeg_enc_set_ctrl_cfg(void __iomem *base,
	u32 exif_en, u32 quality,
	u32 restart_interval)
{
...
{
	mtk_jpeg_enc_set_src_img(base, config->enc_w,
		config->enc_h, config->enc_format,
		config->total_encdu);
	mtk_jpeg_enc_set_src_buf(base, config->img_stride,
		config->mem_stride, fb->fb_addr[0].dma_addr,
		fb->fb_addr[1].dma_addr);
	mtk_jpeg_enc_set_dst_buf(base, bs->dma_addr,
		bs->size, bs->dma_addr_offset,
