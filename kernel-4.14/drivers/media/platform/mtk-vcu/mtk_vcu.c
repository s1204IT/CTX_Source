...
struct mtk_vcu *vcu_ptr[MTK_VCU_NR_MAX];
...
static int vcu_check_reg_base(struct mtk_vcu *vcu,
				u64 addr, u64 length)
{
	int i;
...
static int vcu_init_ipi_handler(void *data, unsigned int len, void *priv)
{
	...
	if (vcu->run.signaled == 1u && run->signaled == 0u) {
		if (vcu->fuse_bypass) {
			...
			while (
			atomic_read(
			&vcu_ptr[vcuid]->gce_job_cnt[VCU_VDEC]) > 0 ||
			atomic_read(
			&vcu_ptr[vcuid]->gce_job_cnt[VCU_VENC]) > 0) {
				wait_cnt++;
				if (wait_cnt > 5) {
					pr_info("[VCU] Vpud killed gce status %d %d\n",
					atomic_read(
					&vcu_ptr[vcuid]->gce_job_cnt[VCU_VDEC]),
					atomic_read(
					&vcu_ptr[vcuid]->gce_job_cnt[VCU_VENC])
					);
					break;
				}
				usleep_range(10000, 20000);
...
static int mtk_vcu_open(struct inode *inode, struct file *file)
{
	...
	if (strcmp(current->comm, "camd") == 0 ||
		strcmp(current->comm, "v4l2d") == 0 ||
		strcmp(current->comm, "camera-main") == 0)
		vcuid = MTK_VCU_CAMERA;
...
{
	long ret = -1;
	struct mtk_vcu *vcu = vcu_ptr[MTK_VCU_VCODEC];


	if (vcu != NULL &&
		vcu->vdec_log_info != NULL &&
		val != NULL &&
		...
		memcpy(vcu->vdec_log_info->log_info,
			val, strlen(val));
	} else
		return -EFAULT;
	pr_info("[log wakeup VPUD] log_info %p vcu_ptr %p val %p: %s %zu\n",
		(char *)vcu->vdec_log_info->log_info,
		vcu, val, val, strlen(val));
...
static int mtk_vcu_suspend(struct device *pDev)
{
	...
	if (atomic_read(&vcu->ipi_done[VCU_VDEC]) == 0 ||
		atomic_read(&vcu->ipi_done[VCU_VENC]) == 0 ||
		atomic_read(&vcu->gce_job_cnt[VCU_VDEC]) > 0 ||
		atomic_read(&vcu->gce_job_cnt[VCU_VENC]) > 0) {
					...
					unsigned long action, void *data)
{
	...
	case PM_SUSPEND_PREPARE:
		vcu->is_entering_suspend = 1;
		while (atomic_read(&vcu->ipi_done[VCU_VDEC]) == 0 ||
			atomic_read(&vcu->ipi_done[VCU_VENC]) == 0 ||
			atomic_read(&vcu->gce_job_cnt[VCU_VDEC]) > 0 ||
			atomic_read(&vcu->gce_job_cnt[VCU_VENC]) > 0) {
			wait_cnt++;
			if (wait_cnt > 5) {
				pr_info("vcodec_pm_suspend waiting %d %d %d %d\n",
				  atomic_read(&vcu->ipi_done[VCU_VDEC]),
				  atomic_read(&vcu->ipi_done[VCU_VENC]),
				  atomic_read(&vcu->gce_job_cnt[VCU_VDEC]),
				  atomic_read(&vcu->gce_job_cnt[VCU_VENC]));
