...
static struct mtk_video_fmt mtk_video_formats[MTK_MAX_ENC_CODECS_SUPPORT] = {
	{0} };
	...
	do {
		...
		if (src_vb2_v4l2 != NULL && dst_vb2_v4l2 != NULL) {
			...
			if (src_vb2_v4l2 == NULL)
				mtk_v4l2_debug(1, "%s NULL enc src buffer\n",
					       __func__);

			if (dst_vb2_v4l2 == NULL)
				mtk_v4l2_debug(1, "%s NULL enc dst buffer\n",
					       __func__);
		}
						  ...
						  unsigned int t)
{
	...
	for (k = 0;
	     k < MTK_MAX_ENC_CODECS_SUPPORT && mtk_video_formats[k].fourcc != 0;
	     k++) {
...
static int vidioc_venc_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	mtk_v4l2_debug(0, "[%d] id %d val %d array[0] %d array[1] %d",
	...
	case V4L2_CID_MPEG_VIDEO_H265_TIER_LEVEL:
		mtk_v4l2_debug(2,
			       "V4L2_CID_MPEG_VIDEO_H265_TIER_LEVEL val = %d",
			       ctrl->val);
		if (!vidioc_venc_check_supported_profile_level(
...
static int vidioc_enum_fmt(struct v4l2_fmtdesc *f, bool output_queue)
{
	...
	for (i = 0;
	     i < MTK_MAX_ENC_CODECS_SUPPORT && mtk_video_formats[i].fourcc != 0;
	     ++i) {
			  ...
			  struct mtk_vcodec_ctx *ctx)
{
	...
	int org_w, org_h, i;
	__u32 bs_fourcc;
	__u32 saligned, width_align, height_align;
	...
	if (f->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		...
		if (pix_fmt_mp->num_planes == 1U) {
			...
			pix_fmt_mp->pixelformat == V4L2_PIX_FMT_BGR32) {
				pix_fmt_mp->plane_fmt[0].sizeimage =
				    pix_fmt_mp->width * 4 * pix_fmt_mp->height;
				pix_fmt_mp->plane_fmt[0].bytesperline =
				 ...
				 pix_fmt_mp->pixelformat ==
					V4L2_PIX_FMT_RGB24) {
				pix_fmt_mp->plane_fmt[0].sizeimage =
				    pix_fmt_mp->width * 3 * pix_fmt_mp->height;
				pix_fmt_mp->plane_fmt[0].bytesperline =
				    pix_fmt_mp->width * 3;
			} else {
				pix_fmt_mp->plane_fmt[0].sizeimage =
		...
		mtk_v4l2_debug(
			0, "w/h (%d, %d) -> (%d,%d), sizeimage[%d,%d,%d]",
			org_w, org_h, pix_fmt_mp->width, pix_fmt_mp->height,
			pix_fmt_mp->plane_fmt[0].sizeimage,
			pix_fmt_mp->plane_fmt[1].sizeimage,
			pix_fmt_mp->plane_fmt[2].sizeimage);
	}
			       ...
			       struct venc_enc_param *param)
{
	...
	param->nonrefp = enc_params->nonrefp;
	mtk_v4l2_debug(0,
	"fmt 0x%x, P/L %d/%d, w/h %d/%d, buf %d/%d, fps/bps %d/%d(%d), gop %d, i_period %d opr %d smvr %d,nonrefp %d",
	...
	param->operationrate, ctx->slowmotion, param->nonrefp);
}
				 ...
				 struct v4l2_format *f)
{
	...
	if (ctx->state == MTK_STATE_FREE) {
		...
		mtk_v4l2_debug(0, "[%d]ctx->state is %d ", ctx->id, ctx->state);
	}
	...
	struct v4l2_encoder_cmd *cmd)
{
	...
	case V4L2_ENC_CMD_STOP:
		...
		if (!vb2_is_streaming(src_vq) || !vb2_is_streaming(dst_vq)) {
			mtk_v4l2_debug(1, "stream is off. No need to flush.");
			return 0;
		}
				   ...
				   struct device *alloc_devs[])
{
	...
	mtk_v4l2_debug(2, "[%d] nplanes %d sizeimage %d %d %d", ctx->id,
		       *nplanes, q_data->sizeimage[0], q_data->sizeimage[1],
		       q_data->sizeimage[2]);
...
static void vb2ops_venc_buf_queue(struct vb2_buffer *vb)
{
	...
	if ((vb->vb2_queue->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) &&
		...
		mtk_v4l2_debug(
			1, "[%d] Before id=%d encode parameter change %x",
			ctx->id, mtk_buf->vb.vb2_buf.index, ctx->param_change);
...
static int mtk_venc_encode_header(void *priv)
{
	...
	mtk_v4l2_debug(1, "[%d] buf id=%d va=0x%p dma_addr=0x%llx size=%zu",
...
static void mtk_venc_worker(struct work_struct *work)
{
	...
	mtk_v4l2_debug(
		...
		pfrm_buf->fb_addr[0].va, (u64)pfrm_buf->fb_addr[0].dma_addr,
		pfrm_buf->fb_addr[0].size, src_buf->planes[0].data_offset,
		pfrm_buf->fb_addr[1].va, (u64)pfrm_buf->fb_addr[1].dma_addr,
		pfrm_buf->fb_addr[1].size, src_buf->planes[1].data_offset,
		pfrm_buf->fb_addr[2].va, (u64)pfrm_buf->fb_addr[2].dma_addr,
		pfrm_buf->fb_addr[2].size, src_buf->planes[2].data_offset);
...
int mtk_vcodec_enc_ctrls_setup(struct mtk_vcodec_ctx *ctx)
{
	...
	v4l2_ctrl_new_std_menu(handler, ops,
			       ...
			       V4L2_MPEG_VIDEO_H265_LEVEL_MAIN_TIER_LEVEL_4, 0,
	...
	cfg.type = V4L2_CTRL_TYPE_INTEGER;
	cfg.flags = V4L2_CTRL_FLAG_VOLATILE;
	cfg.name = "Video encode refp frame num";
	cfg.min = 0;
	cfg.max = 0x7ffffff;
	cfg.step = 1;
	cfg.def = 0;
