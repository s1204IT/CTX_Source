...
static void handle_init_ack_msg(struct vdec_vcu_ipi_init_ack *msg)
{
	struct vdec_vcu_inst *vcu =
		(struct vdec_vcu_inst *)(unsigned long)msg->ap_inst_addr;
...
static void handle_query_cap_ack_msg(struct vdec_vcu_ipi_query_cap_ack *msg)
{
	...
	if (vcu == NULL)
		return;
	mtk_vcodec_debug(
		vcu, "+ ap_inst_addr = 0x%lx, vcu_data_addr = 0x%x, id = %d",
...
int vcu_dec_ipi_handler(void *data, unsigned int len, void *priv)
{
	...
	if (msg->status == 0) {
		...
		case VCU_IPIMSG_DEC_WAITISR:
			...
			do_gettimeofday(&t_e);
			mtk_vcodec_perf_log(
				"irq:%ld", (t_e.tv_sec - t_s.tv_sec) * 1000000 +
...
static int vcodec_vcu_send_msg(struct vdec_vcu_inst *vcu, void *msg, int len)
{
	...
	vcu_get_task(&task, &f, 0);
	vcu_put_file_lock();
	if (task == NULL || vcu->daemon_pid != task->tgid) {
		if (task)
		      ...
		      unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		mtk_vcodec_debug(vcu, "msg.id = 0x%X, msg.data[%d]=%d", msg.id,
				 i, msg.data[i]);
	}
