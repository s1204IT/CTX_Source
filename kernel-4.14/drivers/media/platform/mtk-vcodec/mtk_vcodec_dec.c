...
static struct mtk_video_fmt mtk_video_formats[MTK_MAX_DEC_CODECS_SUPPORT] = {
	{0} };
						  ...
						  unsigned int t)
{
	...
	for (k = 0;
	     k < MTK_MAX_DEC_CODECS_SUPPORT && mtk_video_formats[k].fourcc != 0;
	     k++) {
	...
	bool got_early_eos)
{
	...
	if (vdec_if_get_param(ctx, GET_PARAM_DISP_FRAME_BUFFER,
			      &disp_frame_buffer)) {
		mtk_v4l2_err(
			"[%d]Cannot get param : GET_PARAM_DISP_FRAME_BUFFER",
			ctx->id);
		return NULL;
	}
...
static struct vb2_buffer *get_free_buffer(struct mtk_vcodec_ctx *ctx)
{
	...
	mtk_v4l2_debug(4, "[%d] tmp_frame_addr = 0x%p", ctx->id,
		       free_frame_buffer);
	...
	if (dstbuf->used) {
		if ((dstbuf->queued_in_vb2) && (dstbuf->queued_in_v4l2) &&
			 ...
			 */
			mtk_v4l2_debug(
				2, "[%d]status=%x queue id=%d to rdy_queue %d",
					     ...
					     struct mtk_vcodec_mem *current_bs)
{
	...
	if (vdec_if_get_param(ctx, GET_PARAM_FREE_BITSTREAM_BUFFER,
			      &free_bs_buffer) != 0) {
		mtk_v4l2_err(
			"[%d] Cannot get param : GET_PARAM_FREE_BITSTREAM_BUFFER",
			ctx->id);
		return NULL;
	}
	...
	srcbuf = container_of(free_bs_buffer, struct mtk_video_dec_buf,
			      bs_buffer);
	mtk_v4l2_debug(2,
...
static void mtk_vdec_worker(struct work_struct *work)
{
	...
	if (ret < 0 || mtk_vcodec_unsupport) {
		mtk_v4l2_err(
			...
			buf->size, src_buf_info->vb.vb2_buf.timestamp,
			dst_buf->index, ret, src_chg);
		src_buf = v4l2_m2m_src_buf_remove(ctx->m2m_ctx);
		...
		if (mtk_vcodec_unsupport) {
			 ...
			 */
			mtk_v4l2_err(
				" <=== [%d] vcodec not support the source!===>",
				ctx->id);
		 ...
		 * when decode success without resolution change except
		 * rv30/rv40.
		 */
		src_buf = v4l2_m2m_src_buf_remove(ctx->m2m_ctx);
		clean_free_bs_buffer(ctx, NULL);
	...
	if (ret == 0 && res_chg) {
		if ((fourcc == V4L2_PIX_FMT_RV40) ||
			 ...
			 * needs to refer to the previous frame,so driver should
			 * not flush decode,
...
void mtk_vcodec_dec_set_default_params(struct mtk_vcodec_ctx *ctx)
{
	...
	v4l_bound_align_image(&q_data->coded_width, MTK_VDEC_MIN_W,
...
static int mtk_vdec_set_param(struct mtk_vcodec_ctx *ctx)
{
	...
	if (ctx->dec_param_change & MTK_DEC_PARAM_FRAME_SIZE) {
		...
		if (in[0] != 0 && in[1] != 0) {
			if (vdec_if_set_param(ctx, SET_PARAM_FRAME_SIZE, in) !=
			    0) {
	...
	if (ctx->dec_param_change & MTK_DEC_PARAM_FIXED_MAX_FRAME_SIZE) {
		...
		if (in[0] != 0 && in[1] != 0) {
			if (vdec_if_set_param(
				    ctx, SET_PARAM_SET_FIXED_MAX_OUTPUT_BUFFER,
				    in) != 0) {
				mtk_v4l2_err("[%d] Error!! Cannot set param",
					     ctx->id);
				return -EINVAL;
	...
	if (ctx->dec_param_change & MTK_DEC_PARAM_NAL_SIZE_LENGTH) {
		in[0] = (unsigned long)ctx->dec_params.wait_key_frame;
		if (vdec_if_set_param(ctx, SET_PARAM_NAL_SIZE_LENGTH, in) !=
		    0) {
			mtk_v4l2_err("[%d] Error!! Cannot set param", ctx->id);
			return -EINVAL;
		}
...
static int vidioc_try_fmt(struct v4l2_format *f, struct mtk_video_fmt *fmt)
{
	...
	if (f->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		...
		pix_fmt_mp->height = clamp(pix_fmt_mp->height, MTK_VDEC_MIN_H,
					   MTK_VDEC_MAX_H);
		pix_fmt_mp->width = clamp(pix_fmt_mp->width, MTK_VDEC_MIN_W,
					  MTK_VDEC_MAX_W);
		...
		v4l_bound_align_image(&pix_fmt_mp->width, MTK_VDEC_MIN_W,
				      MTK_VDEC_MAX_W, 6, &pix_fmt_mp->height,
				      MTK_VDEC_MIN_H, MTK_VDEC_MAX_H, 6, 9);
		...
		for (i = 0; i < pix_fmt_mp->num_planes; i++) {
			pix_fmt_mp->plane_fmt[i].sizeimage =
				(pix_fmt_mp->width * pix_fmt_mp->height) >> i;
			     ...
			     struct v4l2_format *f)
{
	...
	fmt = mtk_vdec_find_format(
		ctx, f, (f->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE)
				? MTK_FMT_DEC
				: MTK_FMT_FRAME);
	if (fmt == NULL) {
		if (f->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
				  ...
				  struct v4l2_frmsizeenum *fsize)
{
	...
	for (i = 0; i < MTK_MAX_DEC_CODECS_SUPPORT &&
		    mtk_vdec_framesizes[i].fourcc != 0;
	     ++i) {
		...
		mtk_v4l2_debug(
			...
			fsize->stepwise.step_height, fsize->reserved[0],
			fsize->reserved[1]);
		return 0;
	}
			   ...
			   bool output_queue)
{
	...
	for (i = 0;
	     i < MTK_MAX_DEC_CODECS_SUPPORT && mtk_video_formats[i].fourcc != 0;
	     i++) {
			     ...
			     struct v4l2_format *f)
{
	...
	if ((f->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) &&
		...
		mtk_v4l2_debug(
			1,
			"fourcc:(%d %d),bytesperline:%d,sizeimage:%d,%d,%d\n",
			ctx->q_data[MTK_Q_DATA_SRC].fmt->fourcc,
			q_data->fmt->fourcc, pix_mp->plane_fmt[0].bytesperline,
...
static int vb2ops_vdec_buf_prepare(struct vb2_buffer *vb)
{
	...
	mtk_v4l2_debug(4, "[%d] (%d) id=%d", ctx->id, vb->vb2_queue->type,
		       vb->index);
	...
	if (!(mtkbuf->flags & NO_CAHCE_CLEAN) &&
		...
		if (vb->vb2_queue->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
			...
			mtk_v4l2_debug(
				4, "[%d] Cache sync- TD for %p sz=%d dev %p",
				ctx->id, (void *)src_mem.dma_addr,
			...
			for (plane = 0; plane < vb->num_planes; plane++) {
				...
				mtk_v4l2_debug(
					...
					(unsigned int)dst_mem.fb_base[plane]
						.size,
					&ctx->dev->plat_dev->dev);
			}
		}
...
static void vb2ops_vdec_buf_queue(struct vb2_buffer *vb)
{
	...
	mtk_v4l2_debug(4, "[%d] (%d) id=%d, vb=%p", ctx->id,
	...
	if (ctx->state != MTK_STATE_INIT) {
		mtk_v4l2_debug(4, "[%d] already init driver %d", ctx->id,
			       ctx->state);
		return;
	}
	...
	mtk_v4l2_debug(
		2, "[%d] buf id=%d va=%p dma=%p size=%zx length=%zu dmabuf=%p",
		ctx->id, src_buf->index, src_mem.va, &src_mem.dma_addr,
		src_mem.size, src_mem.length, src_mem.dmabuf);

	if (src_mem.va != NULL) {
		mtk_v4l2_debug(0, "[%d] %x %x %x %x %x %x %x %x %x\n", ctx->id,
			       ((char *)src_mem.va)[0], ((char *)src_mem.va)[1],
			       ((char *)src_mem.va)[2], ((char *)src_mem.va)[3],
			       ((char *)src_mem.va)[4], ((char *)src_mem.va)[5],
			       ((char *)src_mem.va)[6], ((char *)src_mem.va)[7],
			       ((char *)src_mem.va)[8]);
	}
	...
	mtk_vcodec_unsupport =
		((src_chg & VDEC_HW_NOT_SUPPORT) != 0) ? true : false;
	wait_seq_header =
		((src_chg & VDEC_NEED_SEQ_HEADER) != 0U) ? true : false;
	if (ret || !res_chg || mtk_vcodec_unsupport || wait_seq_header) {
		...
		mtk_v4l2_debug(
			...
			ctx->id, src_buf->index, src_mem.size, ret, res_chg,
			mtk_vcodec_unsupport);

		/* If not support the source, eg: w/h, bitdepth, level, */
		/* we need to stop to play it */
		if (wait_seq_header) {
			...
			mtk_v4l2_err(
				"[%d]Error!! Codec driver not support the file!",
				ctx->id);
			mtk_vdec_queue_error_event(ctx);
		}
		return;
	...
	mtk_v4l2_debug(
		2, "[%d] fmt %d, num_planes %d, fb_sz[0] %d, fb_sz[1] %d",
		ctx->id, dst_q_data->fmt->fourcc, dst_q_data->fmt->num_planes,
	...
	if (!(mtkbuf->flags & NO_CAHCE_INVALIDATE) &&
		...
		for (plane = 0; plane < buf->frame_buffer.num_planes; plane++) {
			...
			mtk_v4l2_debug(
				...
				&ctx->dev->plat_dev->dev, &buf->frame_buffer);
		}
	}
}
...
static int vb2ops_vdec_buf_init(struct vb2_buffer *vb)
{
	...
	struct mtk_video_dec_buf *buf =
		container_of(vb2_v4l2, struct mtk_video_dec_buf, vb);

	if (vb->vb2_queue->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		mtk_v4l2_debug(
			4,
			"[%d] pfb=%p used %d ready_to_display %d queued_in_v4l2 %d",
			vb->vb2_queue->type, &buf->frame_buffer, buf->used,
			buf->ready_to_display, buf->queued_in_v4l2);
...
static void vb2ops_vdec_stop_streaming(struct vb2_queue *q)
{
	...
	if (ctx->state >= MTK_STATE_HEADER) {
		...
		mtk_v4l2_debug(
			...
			ctx->last_decoded_picinfo.buf_h, ctx->picinfo.bitdepth);
...
static int mtk_vdec_g_v_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_MPEG_MTK_FRAME_INTERVAL:
		if (vdec_if_get_param(ctx, GET_PARAM_FRAME_INTERVAL, &value) !=
		    0) {
			mtk_v4l2_err("[%d] Error!! Cannot get param", ctx->id);
			ret = -EINVAL;
		}
	...
	case V4L2_CID_MPEG_MTK_ASPECT_RATIO:
		if (vdec_if_get_param(ctx, GET_PARAM_ASPECT_RATIO,
				      &ctrl->val) != 0) {
			mtk_v4l2_err("[%d] Error!! Cannot get param", ctx->id);
			ret = -EINVAL;
		}
	...
	case V4L2_CID_MPEG_MTK_FIX_BUFFERS_SVP:
		if (vdec_if_get_param(
			    ctx, GET_PARAM_PLATFORM_SUPPORTED_FIX_BUFFERS_SVP,
			    &ctrl->val) != 0) {
			mtk_v4l2_err("[%d] Error!! Cannot get param", ctx->id);
			ret = -EINVAL;
		}
...
#endif
		mtk_v4l2_debug(0,
			       "[%d] V4L2_CID_MPEG_MTK_SEC_DECODE id %d val %d",
...
int mtk_vcodec_dec_ctrls_setup(struct mtk_vcodec_ctx *ctx)
{
	...
	ctrl = v4l2_ctrl_new_std(&ctx->ctrl_hdl, &mtk_vcodec_dec_ctrl_ops,
				 V4L2_CID_MPEG_MTK_FRAME_INTERVAL, 16666, 41719,
				 1, 33333);
	ctrl->flags |= V4L2_CTRL_FLAG_VOLATILE;

	ctrl = v4l2_ctrl_new_std(&ctx->ctrl_hdl, &mtk_vcodec_dec_ctrl_ops,
				 V4L2_CID_MPEG_MTK_ASPECT_RATIO, 0, 0xF000F, 1,
				 0x10001);
	ctrl->flags |= V4L2_CTRL_FLAG_VOLATILE;

	ctrl = v4l2_ctrl_new_std(&ctx->ctrl_hdl, &mtk_vcodec_dec_ctrl_ops,
				 V4L2_CID_MPEG_MTK_FIX_BUFFERS, 0, 0xF, 1, 0);
	ctrl->flags |= V4L2_CTRL_FLAG_VOLATILE;

	ctrl = v4l2_ctrl_new_std(&ctx->ctrl_hdl, &mtk_vcodec_dec_ctrl_ops,
				 V4L2_CID_MPEG_MTK_FIX_BUFFERS_SVP, 0, 0xF, 1,
				 0);
	ctrl->flags |= V4L2_CTRL_FLAG_VOLATILE;

	ctrl = v4l2_ctrl_new_custom(&ctx->ctrl_hdl, &mtk_interlacing_ctrl,
				    NULL);
	ctrl->flags |= V4L2_CTRL_FLAG_VOLATILE;
	ctrl = v4l2_ctrl_new_custom(&ctx->ctrl_hdl, &mtk_codec_type_ctrl, NULL);
	ctrl = v4l2_ctrl_new_custom(&ctx->ctrl_hdl, &mtk_color_desc_ctrl, NULL);
	ctrl->flags |= V4L2_CTRL_FLAG_VOLATILE;

	/* s_ctrl */
	ctrl = v4l2_ctrl_new_std(&ctx->ctrl_hdl, &mtk_vcodec_dec_ctrl_ops,
				 V4L2_CID_MPEG_MTK_DECODE_MODE, 0, 32, 1, 0);

	ctrl = v4l2_ctrl_new_std(&ctx->ctrl_hdl, &mtk_vcodec_dec_ctrl_ops,
				 V4L2_CID_MPEG_MTK_SEC_DECODE, 0, 32, 1, 0);

	ctrl = v4l2_ctrl_new_std(&ctx->ctrl_hdl, &mtk_vcodec_dec_ctrl_ops,
				 V4L2_CID_MPEG_MTK_FRAME_SIZE, 0, 65535, 1, 0);

	ctrl = v4l2_ctrl_new_std(&ctx->ctrl_hdl, &mtk_vcodec_dec_ctrl_ops,
				 V4L2_CID_MPEG_MTK_FIXED_MAX_FRAME_BUFFER, 0,
				 65535, 1, 0);

	ctrl = v4l2_ctrl_new_std(&ctx->ctrl_hdl, &mtk_vcodec_dec_ctrl_ops,
				 V4L2_CID_MPEG_MTK_CRC_PATH, 0, 255, 1, 0);

	ctrl = v4l2_ctrl_new_std(&ctx->ctrl_hdl, &mtk_vcodec_dec_ctrl_ops,
				 V4L2_CID_MPEG_MTK_GOLDEN_PATH, 0, 255, 1, 0);

	ctrl = v4l2_ctrl_new_std(&ctx->ctrl_hdl, &mtk_vcodec_dec_ctrl_ops,
				 V4L2_CID_MPEG_MTK_SET_WAIT_KEY_FRAME, 0, 255,
				 1, 0);
