...
static void get_pic_info(struct vdec_inst *inst, struct vdec_pic_info *pic)
{
	...
	mtk_vcodec_debug(
		inst, "pic(%d, %d), buf(%d, %d), bitdepth = %d, fourcc = %d\n",
		pic->pic_w, pic->pic_h, pic->buf_w, pic->buf_h, pic->bitdepth,
		pic->fourcc);
...
static void get_crop_info(struct vdec_inst *inst, struct v4l2_crop *cr)
{
	...
	mtk_vcodec_debug(inst, "l=%d, t=%d, w=%d, h=%d", cr->c.left, cr->c.top,
...
static int vdec_init(struct mtk_vcodec_ctx *ctx, unsigned long *h_vdec)
{
	...
	case V4L2_PIX_FMT_DIVX4:
		inst->vcu.id = IPI_VDEC_DIVX4;
		break;
	case V4L2_PIX_FMT_DIVX5:
	case V4L2_PIX_FMT_DIVX6:
		inst->vcu.id = IPI_VDEC_DIVX5;
		break;
		       ...
		       struct vdec_fb *fb, unsigned int *src_chg)
{
	...
	mtk_vcodec_debug(inst,
			 "+ [%d] FB y_dma=%llx c_dma=%llx va=%p num_planes %d",
	...
	mtk_vcodec_debug(
		inst, "+ FB y_fd=%llx c_fd=%llx BS fd=%llx format=%c%c%c%c",
		inst->vsi->dec.fb_fd[0], inst->vsi->dec.fb_fd[1],
		inst->vsi->dec.bs_fd, fm_fourcc & 0xFF, (fm_fourcc >> 8) & 0xFF,
...
static void vdec_get_bs(struct vdec_inst *inst, struct ring_bs_list *list,
			struct mtk_vcodec_mem **out_bs)
{
	...
	*out_bs = bs;
	...
	list->read_idx =
		(list->read_idx == DEC_MAX_BS_NUM - 1) ? 0 : list->read_idx + 1;
	list->count--;
}

static void vdec_get_fb(struct vdec_inst *inst, struct ring_fb_list *list,
				 ...
				 struct mtk_video_fmt *video_fmt)
{
	...
	for (i = 0; i < MTK_MAX_DEC_CODECS_SUPPORT; i++) {
		if (video_fmt[i].fourcc != 0) {
			mtk_vcodec_debug(
				inst,
				"video_formats[%d] fourcc %d type %d num_planes %d\n",
...
static int vdec_get_param(unsigned long h_vdec, enum vdec_get_param_type type,
			  void *out)
{
	struct vdec_inst *inst = (struct vdec_inst *)h_vdec;
	int ret = 0;
...
static int vdec_set_param(unsigned long h_vdec, enum vdec_set_param_type type,
			  void *in)
{
	struct vdec_inst *inst = (struct vdec_inst *)h_vdec;
	uint64_t size;
