...
static int  star_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (star_prv->opened) {
		if (star_prv->wol == WOL_NONE) {
			...
			regulator_disable(star_prv->phy_regulator_18);
			regulator_disable(star_prv->phy_regulator_33);
#endif
...
static int star_resume(struct platform_device *pdev)
{
	...
	if (star_prv->opened) {
		if (star_prv->wol == WOL_NONE) {
			STAR_PR_INFO("Not support wol.\n");
#ifdef PHY_POWER_BY_PMIC_REG
			ret = regulator_enable(star_prv->phy_regulator_33);
			if (ret != 0)
				STAR_PR_ERR("failed to regulator33(%d)\n",
					    ret);
			mdelay(1000);
			ret = regulator_enable(star_prv->phy_regulator_18);
			if (ret != 0)
				STAR_PR_ERR("failed to regulator18(%d)\n",
					    ret);
#endif
...
static int star_probe(struct platform_device *pdev)
{
...
#ifdef PHY_POWER_BY_PMIC_REG
	star_prv->phy_regulator_33 = devm_regulator_get(&pdev->dev,
							"eth-regulator-33");
	ret = regulator_set_voltage(star_prv->phy_regulator_33,
				    3300000, 3300000);
	if (ret != 0) {
	...
	star_prv->phy_regulator_18 = devm_regulator_get(&pdev->dev,
							"eth-regulator-18");
	ret = regulator_set_voltage(star_prv->phy_regulator_18,
				    1800000, 1800000);
	if (ret != 0) {
		STAR_PR_ERR("failed to regulator_set_voltage(%d)\n",
			    ret);
