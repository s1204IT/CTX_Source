...
static const char * const mt8133_m1_pin_str[PIN_STATE_MAX] = {
	"aud_mosi_on",
	"aud_mosi_off",
	"aud_miso_on",
	"aud_miso_off",
...
struct mt8133_m1_priv {
	...
	struct mt8133_m1_dmic_ctrl_data dmic_data;
	struct mt8133_m1_tdm_ctrl_data tdm_data;
};

struct mt8133_dai_link_prop {
	char *name;
...
static void mt8133_m1_ext_hp_amp1_turn_on(struct snd_soc_card *card)
{
	struct mt8133_m1_priv *card_data = snd_soc_card_get_drvdata(card);
...
static void mt8133_m1_ext_hp_amp1_turn_off(struct snd_soc_card *card)
{
	struct mt8133_m1_priv *card_data = snd_soc_card_get_drvdata(card);
...
static void mt8133_m1_ext_hp_amp2_turn_on(struct snd_soc_card *card)
{
	struct mt8133_m1_priv *card_data = snd_soc_card_get_drvdata(card);
...
static void mt8133_m1_ext_hp_amp2_turn_off(struct snd_soc_card *card)
{
	struct mt8133_m1_priv *card_data = snd_soc_card_get_drvdata(card);
...
/* HP Spk Amp1 */
static int mt8133_m1_hp_spk_amp1_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		mt8133_m1_ext_hp_amp1_turn_on(card);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		mt8133_m1_ext_hp_amp1_turn_off(card);
		break;
	default:
		break;
	}
...
/* HP Spk Amp2 */
static int mt8133_m1_hp_spk_amp2_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_dapm_context *dapm = w->dapm;
	struct snd_soc_card *card = dapm->card;
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		mt8133_m1_ext_hp_amp2_turn_on(card);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		mt8133_m1_ext_hp_amp2_turn_off(card);
		break;
	default:
		break;
	}
...
static int mt8133_m1_lol_spk_amp_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_dapm_context *dapm = w->dapm;
	struct snd_soc_card *card = dapm->card;
...
static const struct snd_kcontrol_new mt8133_m1_hpl_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8133_m1_hpr_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8133_m1_lol_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8133_m1_hpl_output_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8133_m1_hpr_output_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8133_m1_hpl_spk_output_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8133_m1_hpr_spk_output_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);

static const struct snd_soc_dapm_widget mt8133_m1_widgets[] = {
	SND_SOC_DAPM_SWITCH("AU_HPL",
		SND_SOC_NOPM, 0, 0, &mt8133_m1_hpl_switch_ctrl),
	SND_SOC_DAPM_SWITCH("AU_HPR",
		SND_SOC_NOPM, 0, 0, &mt8133_m1_hpr_switch_ctrl),
	SND_SOC_DAPM_SWITCH("AU_LOL",
		SND_SOC_NOPM, 0, 0, &mt8133_m1_lol_switch_ctrl),
	SND_SOC_DAPM_SWITCH("HP_L Output",
		SND_SOC_NOPM, 0, 0, &mt8133_m1_hpl_output_switch_ctrl),
	SND_SOC_DAPM_SWITCH("HP_R Output",
		SND_SOC_NOPM, 0, 0, &mt8133_m1_hpr_output_switch_ctrl),
	SND_SOC_DAPM_SWITCH("HP_L Spk Output",
		SND_SOC_NOPM, 0, 0, &mt8133_m1_hpl_spk_output_switch_ctrl),
	SND_SOC_DAPM_SWITCH("HP_R Spk Output",
		SND_SOC_NOPM, 0, 0, &mt8133_m1_hpr_spk_output_switch_ctrl),

	SND_SOC_DAPM_SPK("HP Spk Amp1", mt8133_m1_hp_spk_amp1_event),
	SND_SOC_DAPM_SPK("HP Spk Amp2", mt8133_m1_hp_spk_amp2_event),
	SND_SOC_DAPM_SPK("LOL Spk Amp", mt8133_m1_lol_spk_amp_event),
...
static const struct snd_soc_dapm_route mt8133_m1_routes[] = {
...
static int mt8133_be_hw_params_fixup(struct snd_soc_pcm_runtime *rtd,
	struct snd_pcm_hw_params *params)
{
	struct mt8133_m1_priv *priv = snd_soc_card_get_drvdata(rtd->card);
	int id = rtd->dai_link->id;
	struct mt8133_m1_tdm_ctrl_data *tdm;
	struct mt8133_m1_dmic_ctrl_data *dmic;
...
#ifdef CONFIG_MTK_HIFIXDSP_SUPPORT
static int mt8133_adsp_hostless_va_init(struct snd_soc_pcm_runtime *rtd)
{
...
static int mt8133_adsp_hostless_va_startup(struct snd_pcm_substream *substream)
{
...
static void mt8133_adsp_hostless_va_shutdown(
...
static struct snd_soc_ops adsp_hostless_va_ops = {
	.startup = mt8133_adsp_hostless_va_startup,
	.shutdown = mt8133_adsp_hostless_va_shutdown,
};
#endif

static int mt8133_m1_int_adda_startup(struct snd_pcm_substream *substream)
{
...
static void mt8133_m1_int_adda_shutdown(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mt8133_m1_priv *priv = snd_soc_card_get_drvdata(rtd->card);
	int ret = 0;
...
static struct snd_soc_ops mt8133_m1_int_adda_ops = {
	.startup = mt8133_m1_int_adda_startup,
	.shutdown = mt8133_m1_int_adda_shutdown,
};

/* Digital audio interface glue - connects codec <---> CPU */
static struct snd_soc_dai_link mt8133_m1_dais[] = {
	/* Front End DAI links */
	...
	[DAI_LINK_FE_HOSTLESS_VA] = {
		...
		.dpcm_capture = 1,
		.ignore_suspend = 1,
		.init = mt8133_adsp_hostless_va_init,
...
static int mt8133_m1_gpio_probe(struct snd_soc_card *card)
{
	struct mt8133_m1_priv *priv = snd_soc_card_get_drvdata(card);
	...
	for (i = 0 ; i < PIN_STATE_MAX ; i++) {
		...
		if (IS_ERR(priv->pin_states[i])) {
			...
			dev_info(card->dev, "%s Can't find pin state %s %d\n",
				 __func__, mt8133_m1_pin_str[i], ret);
		}
	}
...
static void mt8133_m1_parse_of(struct snd_soc_card *card,
	struct device_node *np)
{
	...
	static const struct mt8133_dai_link_prop of_dai_links_io[] = {
		{ "dmic",	DAI_LINK_DMIC },
	...
	for (i = 0; i < ARRAY_SIZE(of_dai_links_io); i++) {
		...
		struct snd_soc_dai_link *dai_link = &mt8133_m1_dais[link_id];
		struct mt8133_m1_dmic_ctrl_data *dmic;
		struct mt8133_m1_tdm_ctrl_data *tdm;
...
static struct snd_soc_card mt8133_m1_card = {
	.name = "mt-snd-card",
	.owner = THIS_MODULE,
	.dai_link = mt8133_m1_dais,
	.num_links = ARRAY_SIZE(mt8133_m1_dais),
	.dapm_widgets = mt8133_m1_widgets,
	.num_dapm_widgets = ARRAY_SIZE(mt8133_m1_widgets),
	.dapm_routes = mt8133_m1_routes,
	.num_dapm_routes = ARRAY_SIZE(mt8133_m1_routes),
};

static int mt8133_m1_dev_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card = &mt8133_m1_card;
	struct device *dev = &pdev->dev;
	struct device_node *np = dev->of_node;
	struct device_node *platform_node;
	struct mt8133_m1_priv *priv;
	int i, ret;
	...
	for (i = 0; i < card->num_links; i++) {
		...
		if (is_adsp_dai_link(i))
			continue;
#endif
		if (mt8133_m1_dais[i].platform_name)
			continue;
		mt8133_m1_dais[i].platform_of_node = platform_node;
	}
	...
	for (i = 0; i < ARRAY_SIZE(adsp_dai_links); i++) {
		...
		if (mt8133_m1_dais[dai].platform_name)
			continue;
		mt8133_m1_dais[dai].platform_of_node = platform_node;
	}
#endif

	card->dev = dev;

	priv = devm_kzalloc(dev, sizeof(struct mt8133_m1_priv),
			    GFP_KERNEL);
	if (!priv) {
		ret = -ENOMEM;
	...
	mt8133_m1_gpio_probe(card);

	mt8133_m1_parse_of(card, np);
...
static const struct of_device_id mt8133_m1_dt_match[] = {
	{ .compatible = "mediatek,mt8133-m1", },
	{ }
};
MODULE_DEVICE_TABLE(of, mt8133_m1_dt_match);

static struct platform_driver mt8133_m1_driver = {
	.driver = {
		   .name = "mt8133-m1",
		   .of_match_table = mt8133_m1_dt_match,
#ifdef CONFIG_PM
		   .pm = &snd_soc_pm_ops,
#endif
	},
	.probe = mt8133_m1_dev_probe,
};

module_platform_driver(mt8133_m1_driver);

/* Module information */
MODULE_DESCRIPTION("MT8133 M1 SoC machine driver");
MODULE_AUTHOR("Jiaxin Yu <jiaxin.yu@mediatek.com>");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:mt8133-m1");
