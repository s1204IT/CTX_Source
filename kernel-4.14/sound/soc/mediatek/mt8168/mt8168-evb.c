/*
 * mt8168-evb.c  --  MT8168 machine driver
 *
...
static void mt8168_evb_ext_lol_amp_turn_on(struct snd_soc_card *card)
{
	struct mt8168_evb_priv *card_data = snd_soc_card_get_drvdata(card);

	/* enable spk amp */
	if (gpio_is_valid(card_data->spk_amp_gpio))
		gpio_set_value(card_data->spk_amp_gpio, 1);
...
static void mt8168_evb_ext_lol_amp_turn_off(struct snd_soc_card *card)
{
	struct mt8168_evb_priv *card_data = snd_soc_card_get_drvdata(card);

	/* disable spk amp */
	if (gpio_is_valid(card_data->spk_amp_gpio))
		gpio_set_value(card_data->spk_amp_gpio, 0);
...
/* HP Spk Amp1 */
static int mt8168_evb_hp_spk_amp1_event(struct snd_soc_dapm_widget *w,
...
/* HP Spk Amp2 */
static int mt8168_evb_hp_spk_amp2_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_dapm_context *dapm = w->dapm;
	struct snd_soc_card *card = dapm->card;
...
/* LOL Spk Amp */
static int mt8168_evb_lol_spk_amp_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_dapm_context *dapm = w->dapm;
	struct snd_soc_card *card = dapm->card;
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		mt8168_evb_ext_lol_amp_turn_on(card);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		mt8168_evb_ext_lol_amp_turn_off(card);
		break;
	default:
		break;
	}
...
static const struct snd_kcontrol_new mt8168_evb_hpl_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8168_evb_hpr_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8168_evb_lol_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8168_evb_hpl_output_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8168_evb_hpr_output_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8168_evb_hpl_spk_output_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8168_evb_hpr_spk_output_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);

static const struct snd_soc_dapm_widget mt8168_evb_widgets[] = {
	SND_SOC_DAPM_SWITCH("AU_HPL",
		SND_SOC_NOPM, 0, 0, &mt8168_evb_hpl_switch_ctrl),
	SND_SOC_DAPM_SWITCH("AU_HPR",
		SND_SOC_NOPM, 0, 0, &mt8168_evb_hpr_switch_ctrl),
	SND_SOC_DAPM_SWITCH("AU_LOL",
		SND_SOC_NOPM, 0, 0, &mt8168_evb_lol_switch_ctrl),
	SND_SOC_DAPM_SWITCH("HP_L Output",
		SND_SOC_NOPM, 0, 0, &mt8168_evb_hpl_output_switch_ctrl),
	SND_SOC_DAPM_SWITCH("HP_R Output",
		SND_SOC_NOPM, 0, 0, &mt8168_evb_hpr_output_switch_ctrl),
	SND_SOC_DAPM_SWITCH("HP_L Spk Output",
		SND_SOC_NOPM, 0, 0, &mt8168_evb_hpl_spk_output_switch_ctrl),
	SND_SOC_DAPM_SWITCH("HP_R Spk Output",
		SND_SOC_NOPM, 0, 0, &mt8168_evb_hpr_spk_output_switch_ctrl),
	SND_SOC_DAPM_SPK("HP Spk Amp1", mt8168_evb_hp_spk_amp1_event),
	SND_SOC_DAPM_SPK("HP Spk Amp2", mt8168_evb_hp_spk_amp2_event),
	SND_SOC_DAPM_SPK("LOL Spk Amp", mt8168_evb_lol_spk_amp_event),
...
#endif
static int mt8168_evb_int_adda_startup(struct snd_pcm_substream *substream)
{
...
static void mt8168_evb_int_adda_shutdown(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mt8168_evb_priv *priv = snd_soc_card_get_drvdata(rtd->card);
	int ret = 0;
...
static struct snd_soc_ops mt8168_evb_int_adda_ops = {
	.startup = mt8168_evb_int_adda_startup,
	.shutdown = mt8168_evb_int_adda_shutdown,
};

static int mt8168_evb_tmd_out_startup(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mt8168_evb_priv *priv = snd_soc_card_get_drvdata(rtd->card);
	int ret = 0;
	...
	if (ret)
		dev_info(rtd->card->dev, "%s failed to select state %d\n",
			__func__, ret);

	return 0;
}

static void mt8168_evb_tmd_out_shutdown(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mt8168_evb_priv *priv = snd_soc_card_get_drvdata(rtd->card);
	int ret = 0;
...
static struct snd_soc_ops mt8168_evb_tmd_out_ops = {
	.startup = mt8168_evb_tmd_out_startup,
	.shutdown = mt8168_evb_tmd_out_shutdown,
};
...
static int mt8168_evb_gpio_probe(struct snd_soc_card *card)
{
	...
	priv->spk_amp_gpio = of_get_named_gpio(np, "spk-amp-gpio", 0);
	if (!gpio_is_valid(priv->spk_amp_gpio))
		dev_warn(card->dev, "%s get invalid spk-amp-gpio %d\n",
				__func__, priv->spk_amp_gpio);

	if (gpio_is_valid(priv->spk_amp_gpio)) {
		ret = devm_gpio_request_one(card->dev, priv->spk_amp_gpio,
			GPIOF_OUT_INIT_LOW, "spk amp enable");
		if (ret < 0)
			dev_err(card->dev, "%s failed to init spk amp enable gpio %d\n",
				__func__, ret);
	}
...
/* Module information */
MODULE_DESCRIPTION("MT8168 EVB SoC machine driver");
MODULE_AUTHOR("Jia Zeng <jia.zeng@mediatek.com>");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:mt8168-evb");
