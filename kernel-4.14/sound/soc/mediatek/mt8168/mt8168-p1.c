...
static int mt8168_p1_gpio_probe(struct snd_soc_card *card)
{
	...
	amp1en = of_get_named_gpio(np, "ext-amp1en-gpio", 0);
	if (!gpio_is_valid(priv->ext_amp1_gpio))
		dev_warn(card->dev, "%s get invalid ext-amp1en-gpio %d\n",
				__func__, priv->ext_amp1_gpio);
	if (gpio_is_valid(amp1en)) {
		ret = devm_gpio_request_one(card->dev, amp1en,
			GPIOF_OUT_INIT_HIGH, "ext amp1en enable");
		if (ret < 0)
