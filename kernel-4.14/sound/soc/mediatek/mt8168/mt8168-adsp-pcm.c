...
struct adsp_ioctl_model_data {
	unsigned char *addr_from_user;
	uint32_t size_from_user;
};
...
static int load_model_to_dsp(struct file *fp, unsigned long arg)
{
	...
	ret = prepare_model_to_dsp(fp, model_data.size_from_user);
	if (ret) {
		pr_info("prepare_model_to_dsp fail! ret:%d, model size:%d\n",
			ret, model_data.size_from_user);
		return ret;
	}
	ret = write_model_to_dsp(fp, model_data.addr_from_user);
	if (ret) {
		pr_info("write_model_to_dsp fail! ret:%d, model addr:%p\n", ret,
			model_data.addr_from_user);
		return ret;
	}
