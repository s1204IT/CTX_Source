/*
 * mt8168-afe-pcm.c  --  Mediatek 8168 ALSA SoC AFE platform driver
 *
...
#include "mt8168-afe-debug.h"
#include "mt8168-afe-controls.h"
#include "../common/mtk-base-afe.h"
#include "../common/mtk-afe-platform-driver.h"
#include "../common/mtk-afe-fe-dai.h"

#define MT8168_I2S0_MCLK_MULTIPLIER 256
#define MT8168_I2S1_MCLK_MULTIPLIER 256
#define MT8168_I2S2_MCLK_MULTIPLIER 256
#define MT8168_I2S3_MCLK_MULTIPLIER 256
#define MT8168_TDM_OUT_MCLK_MULTIPLIER 256
#define MT8168_TDM_IN_MCLK_MULTIPLIER 256
...
static const struct snd_pcm_hardware mt8168_afe_hardware = {
...
struct mt8168_afe_rate {
	unsigned int rate;
	unsigned int reg_val;
};

static const struct mt8168_afe_rate mt8168_afe_fs_rates[] = {
	{ .rate = 8000, .reg_val = MT8168_FS_8K },
	...
	{ .rate = 48000, .reg_val = MT8168_FS_48K },
	{ .rate = 88200, .reg_val = MT8168_FS_88D2K },
	{ .rate = 96000, .reg_val = MT8168_FS_96K },
	{ .rate = 176400, .reg_val = MT8168_FS_176D4K },
	{ .rate = 192000, .reg_val = MT8168_FS_192K },
};

static int mt8168_afe_fs_timing(unsigned int rate)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(mt8168_afe_fs_rates); i++)
		if (mt8168_afe_fs_rates[i].rate == rate)
			return mt8168_afe_fs_rates[i].reg_val;

	return -EINVAL;
}

bool mt8168_afe_rate_supported(unsigned int rate, unsigned int id)
{
	switch (id) {
	case MT8168_AFE_IO_TDM_IN:
		if (rate >= 8000 && rate <= 192000)
			return true;
		break;
	case MT8168_AFE_IO_DMIC:
...
bool mt8168_afe_channel_supported(unsigned int channel, unsigned int id)
{
	switch (id) {
	case MT8168_AFE_IO_TDM_IN:
...
bool mt8168_afe_clk_group_44k(int sample_rate)
{
...
bool mt8168_afe_clk_group_48k(int sample_rate)
{
	return (!mt8168_afe_clk_group_44k(sample_rate));
}

static int mt8168_afe_irq_direction_enable(struct mtk_base_afe *afe,
	int irq_id,
	int direction)
{
	struct mtk_base_afe_irq *irq;

	if (irq_id >= MT8168_AFE_IRQ_NUM)
		return -1;

	irq = &afe->irqs[irq_id];

	if (direction == MT8168_AFE_IRQ_DIR_MCU) {
		...
		regmap_update_bits(afe->regmap, AFE_IRQ_MCU_EN,
		       (1 << irq->irq_data->irq_clr_shift),
		       (1 << irq->irq_data->irq_clr_shift));
	} else if (direction == MT8168_AFE_IRQ_DIR_DSP) {
		regmap_update_bits(afe->regmap, AFE_IRQ_MCU_DSP_EN,
		       (1 << irq->irq_data->irq_clr_shift),
		       (1 << irq->irq_data->irq_clr_shift));
		regmap_update_bits(afe->regmap, AFE_IRQ_MCU_EN,
...
static int mt8168_afe_set_i2s_out(struct mtk_base_afe *afe, unsigned int rate,
	int bit_width)
{
...
static int mt8168_afe_set_2nd_i2s_out(struct mtk_base_afe *afe,
...
static int mt8168_afe_set_i2s_in(struct mtk_base_afe *afe, unsigned int rate,
	int bit_width)
{
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
	unsigned int val;
...
static void mt8168_afe_set_i2s_out_enable(struct mtk_base_afe *afe, bool enable)
{
	unsigned long flags;
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
...
static void mt8168_afe_set_2nd_i2s_out_enable(struct mtk_base_afe *afe,
	bool enable)
{
	regmap_update_bits(afe->regmap, AFE_I2S_CON3, 0x1, enable);
}

static void mt8168_afe_set_i2s_in_enable(struct mtk_base_afe *afe,
	bool enable)
{
	regmap_update_bits(afe->regmap, AFE_I2S_CON2, 0x1, enable);
}

static void mt8168_afe_set_2nd_i2s_in_enable(struct mtk_base_afe *afe,
	bool enable)
{
	regmap_update_bits(afe->regmap, AFE_I2S_CON, 0x1, enable);
}

static int mt8168_afe_set_adda_out(struct mtk_base_afe *afe, unsigned int rate)
{
	unsigned int val = 0;
...
static int mt8168_afe_set_adda_in(struct mtk_base_afe *afe, unsigned int rate)
{
	unsigned int val = 0;
...
static int mt8168_afe_enable_adda_on(struct mtk_base_afe *afe)
{
	unsigned long flags;
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
...
static int mt8168_afe_disable_adda_on(struct mtk_base_afe *afe)
{
	unsigned long flags;
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
...
static void mt8168_afe_set_adda_out_enable(struct mtk_base_afe *afe,
	bool enable)
{
	regmap_update_bits(afe->regmap, AFE_ADDA_DL_SRC2_CON0, 0x1, enable);

	if (enable)
		mt8168_afe_enable_adda_on(afe);
	else
		mt8168_afe_disable_adda_on(afe);
}

static void mt8168_afe_set_adda_in_enable(struct mtk_base_afe *afe,
	bool enable)
{
	if (enable) {
...
static unsigned int mt8168_afe_tdm_ch_fixup(unsigned int channels)
{
	if (channels > 4)
		return 8;
	else if (channels > 2)
...
static unsigned int mt8168_afe_tdm_out_ch_per_sdata(unsigned int mode,
	unsigned int channels)
{
	if (mode == MT8168_AFE_TDM_OUT_TDM)
		return mt8168_afe_tdm_ch_fixup(channels);
	else
		return 2;
}

static int mt8168_afe_tdm_out_bitwidth_fixup(unsigned int mode,
	int bitwidth)
{
	if (mode == MT8168_AFE_TDM_OUT_I2S_32BITS)
		return 32;
	else
		return bitwidth;
}

static int mt8168_afe_tdm_out_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	...
	mt8168_afe_enable_top_cg(afe, MT8168_TOP_CG_TDM_OUT);
	mt8168_afe_enable_clk(afe,
		afe_priv->clocks[MT8168_CLK_AUD_TDMOUT_M]);

	mt8168_afe_enable_clk(afe,
		afe_priv->clocks[MT8168_CLK_AUD_TDMOUT_B]);

	return 0;
}

static void mt8168_afe_tdm_out_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	mt8168_afe_disable_top_cg(afe, MT8168_TOP_CG_TDM_OUT);
	mt8168_afe_disable_main_clk(afe);

}

static int mt8168_afe_tdm_out_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	const unsigned int out_channels_per_sdata =
		mt8168_afe_tdm_out_ch_per_sdata(tdm_out_mode,
		runtime->channels);
	const int bit_width = snd_pcm_format_width(runtime->format);
	const int out_bit_width =
		mt8168_afe_tdm_out_bitwidth_fixup(tdm_out_mode,
		bit_width);
	const unsigned int stream = substream->stream;
	unsigned int val;
	...
	if (rate % 8000) {
		mt8168_afe_enable_apll_associated_cfg(afe, MT8168_AFE_APLL1);
		mt8168_afe_set_clk_parent(afe,
			afe_priv->clocks[MT8168_CLK_TDMOUT_M_SEL],
			afe_priv->clocks[MT8168_CLK_AUD1]);
	} else {
		mt8168_afe_enable_apll_associated_cfg(afe, MT8168_AFE_APLL2);
		mt8168_afe_set_clk_parent(afe,
			afe_priv->clocks[MT8168_CLK_TDMOUT_M_SEL],
			afe_priv->clocks[MT8168_CLK_AUD2]);
	}

	mt8168_afe_set_clk_rate(afe, afe_priv->clocks[MT8168_CLK_AUD_TDMOUT_M],
		rate * MT8168_TDM_OUT_MCLK_MULTIPLIER);

	mt8168_afe_set_clk_rate(afe, afe_priv->clocks[MT8168_CLK_AUD_TDMOUT_B],
		rate * out_channels_per_sdata * out_bit_width);

	val = AFE_TDM_CON1_MSB_ALIGNED;

	if ((tdm_out_mode == MT8168_AFE_TDM_OUT_I2S ||
	     tdm_out_mode == MT8168_AFE_TDM_OUT_I2S_32BITS) &&
	...
	} else if (tdm_out_mode == MT8168_AFE_TDM_OUT_TDM) {
		val |= AFE_TDM_CON1_1_BCK_DELAY;
		bck_inverse = AUD_TCON3_HDMI_BCK_INV;
	}
...
static int mt8168_afe_tdm_out_trigger(struct snd_pcm_substream *substream,
...
static int mt8168_afe_tdm_out_set_fmt(struct snd_soc_dai *dai,
				unsigned int fmt)
{
...
static int mt8168_afe_tdm_in_startup(struct snd_pcm_substream *substream,
				struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	mt8168_afe_enable_top_cg(afe, MT8168_TOP_CG_TDM_IN);
	mt8168_afe_enable_clk(afe,
		afe_priv->clocks[MT8168_CLK_AUD_TDMIN_M]);

	mt8168_afe_enable_clk(afe,
		afe_priv->clocks[MT8168_CLK_AUD_TDMIN_B]);

	return 0;
}

static void mt8168_afe_tdm_in_shutdown(struct snd_pcm_substream *substream,
				  struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	mt8168_afe_disable_top_cg(afe, MT8168_TOP_CG_TDM_IN);
	mt8168_afe_disable_main_clk(afe);
}

static int mt8168_afe_tdm_in_prepare(struct snd_pcm_substream *substream,
				struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	if (rate % 8000) {
		mt8168_afe_enable_apll_associated_cfg(afe, MT8168_AFE_APLL1);
		mt8168_afe_set_clk_parent(afe,
			afe_priv->clocks[MT8168_CLK_TDMIN_M_SEL],
			afe_priv->clocks[MT8168_CLK_AUD1]);
	} else {
		mt8168_afe_enable_apll_associated_cfg(afe, MT8168_AFE_APLL2);
		mt8168_afe_set_clk_parent(afe,
	...
	bck = mt8168_afe_tdm_ch_fixup(channels) * rate * bit_width;

	mt8168_afe_set_clk_rate(afe, afe_priv->clocks[MT8168_CLK_AUD_TDMIN_M],
		rate * MT8168_TDM_IN_MCLK_MULTIPLIER);

	mt8168_afe_set_clk_rate(afe, afe_priv->clocks[MT8168_CLK_AUD_TDMIN_B],
		bck);
...
static int mt8168_afe_tdm_in_trigger(struct snd_pcm_substream *substream,
	int cmd, struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
...
static int mt8168_afe_tdm_in_set_fmt(struct snd_soc_dai *dai,
				unsigned int fmt)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
...
static int mt8168_memif_fs(struct snd_pcm_substream *substream,
	unsigned int rate)
{
	return mt8168_afe_fs_timing(rate);
}

static int mt8168_irq_fs(struct snd_pcm_substream *substream,
	unsigned int rate)
{
	return mt8168_memif_fs(substream, rate);
}

static const struct mt8168_cm_ctrl_reg cm_ctrl_reg[MT8168_CM_NUM] = {
	[MT8168_CM1] = {
		.con0 = AFE_CM1_CON0,
		.con1 = AFE_CM1_CON1,
		.con2 = AFE_CM1_CON2,
...
static int mt8168_afe_cm2_mux_conn(struct mtk_base_afe *afe)
{
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
	...
	} else if (input == MT8168_FROM_GASRC2) {
		regmap_update_bits(afe->regmap, AFE_CM2_CONN1,
			CM2_AFE_CM2_CONN_CFG9_MASK,
	...
	} else if (input == MT8168_FROM_TDM_ASRC) {
		regmap_update_bits(afe->regmap, AFE_CM2_CONN1,
			CM2_AFE_CM2_CONN_CFG9_MASK,
...
static int mt8168_afe_get_cm_update_cnt(struct mtk_base_afe *afe,
	enum mt8168_cm_num cmNum, unsigned int rate, unsigned int channel)
{
	...
	unsigned int ch_update_cnt[MT8168_CM_UPDATA_CNT_SET];
	int i;

	if (cmNum == MT8168_CM1) {
		total_cnt = MT8168_CLK_26M / rate;
	} else if (cmNum == MT8168_CM2) {
		if (mt8168_afe_clk_group_48k(rate))
			total_cnt = MT8168_CLK_24M / rate;
		else
			total_cnt = MT8168_CLK_22M / rate;
	} else {
...
static int mt8168_afe_configure_cm(struct mtk_base_afe *afe,
	enum mt8168_cm_num cmNum,
	unsigned int channels,
	unsigned int rate)
{
	unsigned int val, mask;
	unsigned int fs = mt8168_afe_fs_timing(rate);
	...
	mt8168_afe_get_cm_update_cnt(afe, cmNum, rate, channels);

	return 0;
}

int mt8168_afe_fe_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
...
static void mt8168_afe_fe_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
...
static int mt8168_afe_fe_hw_params(struct snd_pcm_substream *substream,
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	struct mt8168_fe_dai_data *fe_data = &afe_priv->fe_data[dai_id];
	...
	if (dai_id == MT8168_AFE_MEMIF_VUL2) {
		if (!ctrl_data->bypass_cm1)
			/* configure cm1 */
			mt8168_afe_configure_cm(afe,
				MT8168_CM1, channels, rate);
		else
			regmap_update_bits(afe->regmap, AFE_CM1_CON0,
				CM_AFE_CM1_VUL_SEL,
				CM_AFE_CM1_VUL_SEL_O17_O18);
	} else if (dai_id == MT8168_AFE_MEMIF_TDM_IN) {
		if (!ctrl_data->bypass_cm2)
			/* configure cm2 */
			mt8168_afe_configure_cm(afe,
				MT8168_CM2, channels, rate);
		else
...
static int mt8168_afe_fe_hw_free(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	if (fe_data->use_sram) {
		...
		mt8168_afe_emi_clk_off(afe);
	}

	return ret;
}

static int mt8168_afe_fe_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
...
int mt8168_afe_fe_trigger(struct snd_pcm_substream *substream, int cmd,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
...
static int mt8168_afe_i2s_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	const unsigned int clk_mode =
		afe_priv->i2s_clk_modes[MT8168_AFE_1ST_I2S];

	dev_dbg(afe->dev, "%s '%s'\n",
		__func__, snd_pcm_stream_str(substream));

	if (clk_mode == MT8168_AFE_I2S_SHARED_CLOCK && dai->active)
		return 0;
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK ||
		...
		mt8168_afe_enable_clk(afe,
		afe_priv->clocks[MT8168_CLK_AUD_I2S1_M]);

	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE ||
	    clk_mode == MT8168_AFE_I2S_SHARED_CLOCK)
		mt8168_afe_enable_clk(afe,
		afe_priv->clocks[MT8168_CLK_AUD_I2S2_M]);

	return 0;
}

static void mt8168_afe_i2s_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	if (be->prepared[stream]) {
		...
		if (reset_i2s_in_change)
			mt8168_afe_set_i2s_in_enable(afe, false);
...
static int mt8168_afe_i2s_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	if ((clk_mode == MT8168_AFE_I2S_SHARED_CLOCK) &&
	...
	if (apply_i2s_out_change) {
		ret = mt8168_afe_set_i2s_out(afe, rate, bit_width);
		if (ret)
			return ret;
	}

	if (apply_i2s_in_change) {
		ret = mt8168_afe_set_i2s_in(afe, rate, bit_width);
		if (ret)
			return ret;
	}
	...
	if (apply_i2s_out_change) {
		mt8168_afe_set_clk_parent(afe,
			afe_priv->clocks[MT8168_CLK_I2S1_M_SEL],
			((rate % 8000) ?
			afe_priv->clocks[MT8168_CLK_AUD1] :
			afe_priv->clocks[MT8168_CLK_AUD2]));

		mt8168_afe_set_clk_rate(afe,
			afe_priv->clocks[MT8168_CLK_AUD_I2S1_M],
			rate * MT8168_I2S1_MCLK_MULTIPLIER);
	...
	if (apply_i2s_in_change) {
		mt8168_afe_set_clk_parent(afe,
			afe_priv->clocks[MT8168_CLK_I2S2_M_SEL],
			((rate % 8000) ?
			afe_priv->clocks[MT8168_CLK_AUD1] :
			afe_priv->clocks[MT8168_CLK_AUD2]));
		...
		mt8168_afe_set_clk_rate(afe,
			afe_priv->clocks[MT8168_CLK_AUD_I2S2_M],
			rate * MT8168_I2S2_MCLK_MULTIPLIER);

		mt8168_afe_set_i2s_in_enable(afe, true);
...
static void mt8168_afe_2nd_i2s_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
...
static int mt8168_afe_2nd_i2s_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	if (apply_i2s_out_change) {
		mt8168_afe_set_clk_parent(afe,
			afe_priv->clocks[MT8168_CLK_I2S3_M_SEL],
			((rate % 8000) ?
			afe_priv->clocks[MT8168_CLK_AUD1] :
			afe_priv->clocks[MT8168_CLK_AUD2]));
		...
		mt8168_afe_set_clk_rate(afe,
			afe_priv->clocks[MT8168_CLK_AUD_I2S3_M],
			rate * MT8168_I2S3_MCLK_MULTIPLIER);

		mt8168_afe_set_2nd_i2s_out_enable(afe, true);
		be->prepared[SNDRV_PCM_STREAM_PLAYBACK] = true;
	}

	if (apply_i2s_in_change) {
		mt8168_afe_set_clk_parent(afe,
			afe_priv->clocks[MT8168_CLK_I2S0_M_SEL],
			((rate % 8000) ?
			afe_priv->clocks[MT8168_CLK_AUD1] :
			afe_priv->clocks[MT8168_CLK_AUD2]));
...
static void mt8168_afe_enable_pcm1(struct mtk_base_afe *afe)
{
	regmap_update_bits(afe->regmap, PCM_INTF_CON1,
			   PCM_INTF_CON1_EN, PCM_INTF_CON1_EN);
}

static void mt8168_afe_disable_pcm1(struct mtk_base_afe *afe)
{
	regmap_update_bits(afe->regmap, PCM_INTF_CON1,
			   PCM_INTF_CON1_EN, 0x0);
}

static int mt8168_afe_configure_pcm1(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_pcm_runtime * const runtime = substream->runtime;
	...
	if (fmt == MT8168_PCM_FORMAT_PCMA ||
	    fmt == MT8168_PCM_FORMAT_PCMB)
...
static int mt8168_afe_pcm1_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
...
static void mt8168_afe_pcm1_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	mt8168_afe_disable_pcm1(afe);
	mt8168_afe_disable_main_clk(afe);
}

static int mt8168_afe_pcm1_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	ret = mt8168_afe_configure_pcm1(substream, dai);
	if (ret)
		return ret;

	mt8168_afe_enable_pcm1(afe);

	return 0;
}

static int mt8168_afe_pcm1_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
	...
	switch (fmt & SND_SOC_DAIFMT_FORMAT_MASK) {
	case SND_SOC_DAIFMT_I2S:
		afe_priv->pcm_intf_data.format = MT8168_PCM_FORMAT_I2S;
		break;
	default:
		return -EINVAL;
	}
...
static void mt8168_afe_disable_dmic(struct mtk_base_afe *afe,
	struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
...
static const struct reg_sequence mt8168_afe_dmic_iir_coeff_reg_defaults[] = {
...
static int mt8168_afe_load_dmic_iir_coeff_table(struct mtk_base_afe *afe)
{
	return regmap_multi_reg_write(afe->regmap,
			mt8168_afe_dmic_iir_coeff_reg_defaults,
			ARRAY_SIZE(mt8168_afe_dmic_iir_coeff_reg_defaults));
}

static int mt8168_afe_configure_dmic(struct mtk_base_afe *afe,
	struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
	...
	if (iir_on) {
		if (irr_mode == IIR_MODE0)
			mt8168_afe_load_dmic_iir_coeff_table(afe); /* SW mode */
...
static int mt8168_afe_dmic_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	mt8168_afe_enable_top_cg(afe, MT8168_TOP_CG_DMIC0_ADC);
	mt8168_afe_enable_top_cg(afe, MT8168_TOP_CG_DMIC1_ADC);
	mt8168_afe_enable_top_cg(afe, MT8168_TOP_CG_DMIC2_ADC);
	mt8168_afe_enable_top_cg(afe, MT8168_TOP_CG_DMIC3_ADC);
...
static void mt8168_afe_dmic_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	mt8168_afe_disable_top_cg(afe, MT8168_TOP_CG_DMIC0_ADC);

	mt8168_afe_disable_main_clk(afe);
}

static int mt8168_afe_dmic_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);

	mt8168_afe_configure_dmic(afe, substream, dai);
	mt8168_afe_enable_dmic(afe, substream, dai);

	return 0;
}

static int mt8168_afe_int_adda_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		mt8168_afe_enable_top_cg(afe, MT8168_TOP_CG_DAC);
		mt8168_afe_enable_top_cg(afe, MT8168_TOP_CG_DAC_PREDIS);
	} else if (stream == SNDRV_PCM_STREAM_CAPTURE) {
		mt8168_afe_enable_top_cg(afe, MT8168_TOP_CG_ADC);
	}

	return 0;
}

static void mt8168_afe_int_adda_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	if (be->prepared[stream]) {
		if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			mt8168_afe_set_adda_in_enable(afe, false);

		be->prepared[stream] = false;
	}

	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		mt8168_afe_disable_top_cg(afe, MT8168_TOP_CG_DAC_PREDIS);
		mt8168_afe_disable_top_cg(afe, MT8168_TOP_CG_DAC);
	} else if (stream == SNDRV_PCM_STREAM_CAPTURE) {
		mt8168_afe_disable_top_cg(afe, MT8168_TOP_CG_ADC);
	}

	mt8168_afe_disable_main_clk(afe);
}

static int mt8168_afe_int_adda_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		ret = mt8168_afe_set_adda_out(afe, rate);
		if (ret)
			return ret;
		...
		ret = mt8168_afe_set_adda_in(afe, rate);
		if (ret)
			return ret;

		mt8168_afe_set_adda_in_enable(afe, true);
	}
...
static const struct mt8168_gasrc_ctrl_reg
	gasrc_ctrl_reg[MT8168_TDM_ASRC_NUM] = {
	[MT8168_GASRC1] = {
...
static int mt8168_dai_num_to_gasrc(int num)
{
	int val = num - MT8168_AFE_IO_GASRC1;

	if (val < 0 || val > MT8168_GASRC_NUM)
		return -EINVAL;

	return val;
}

static void mt8168_afe_gasrc_set_input_fs(struct mtk_base_afe *afe,
	struct snd_soc_dai *dai, int fs_timing)
{
	const int gasrc_id = mt8168_dai_num_to_gasrc(dai->id);
	unsigned int val = 0;
	unsigned int mask = 0;
...
static void mt8168_afe_gasrc_set_output_fs(struct mtk_base_afe *afe,
	struct snd_soc_dai *dai, int fs_timing)
{
	const int gasrc_id = mt8168_dai_num_to_gasrc(dai->id);
	unsigned int val = 0;
...
static void mt8168_afe_gasrc_enable_iir(struct mtk_base_afe *afe,
	int gasrc_id)
{
...
static void mt8168_afe_gasrc_disable_iir(struct mtk_base_afe *afe,
	int gasrc_id)
{
	unsigned int ctrl_reg = gasrc_ctrl_reg[gasrc_id].con2;
...
static int mt8168_afe_enable_gasrc(struct mtk_base_afe *afe,
	int gasrc_id)
{
	unsigned int ctrl_reg = 0;
	int ret = 0;

	if (gasrc_id == MT8168_GASRC1 ||  gasrc_id == MT8168_GASRC2)
...
static int mt8168_afe_disable_gasrc(struct mtk_base_afe *afe,
	int gasrc_id)
{
	unsigned int ctrl_reg = 0;
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
	...
	if (afe_priv->gasrc_data[gasrc_id].iir_on)
		mt8168_afe_gasrc_disable_iir(afe, gasrc_id);

	return 0;
}

static void mt8168_afe_clear_gasrc(struct mtk_base_afe *afe,
	int gasrc_id)
{
	unsigned int ctrl_reg = 0;
...
struct mt8168_gasrc_rx_freq {
	unsigned int rate;
	unsigned int freq_val;
};

static const
	struct mt8168_gasrc_rx_freq gasrc_frequency_palette[] = {
...
static const
	struct mt8168_gasrc_tx_freq gasrc_period_palette[] = {
...
static const
	struct mt8168_gasrc_tx_freq gasrc_tx_AutoRstThHi[] = {
...
static const
	struct mt8168_gasrc_tx_freq gasrc_tx_AutoRstThLo[] = {
...
static const
	struct mt8168_gasrc_rx_freq gasrc_rx_AutoRstThHi[] = {
...
static const
	struct mt8168_gasrc_rx_freq gasrc_rx_AutoRstThLo[] = {
...
static unsigned int mt8168_afe_gasrc_get_freq_val(unsigned int rate)
{
	int i;
...
static unsigned int mt8168_afe_gasrc_get_period_val(unsigned int rate,
	unsigned int domain)
{
	int i;
...
static void mt8168_afe_gasrc_rx_frequency_palette(
	...
	int input_rate, int output_rate)
{
	...
	if (mt8168_afe_gasrc_get_freq_val(output_rate) > 0)
		regmap_write(afe->regmap, ctrl_reg,
			mt8168_afe_gasrc_get_freq_val(output_rate));

	ctrl_reg = gasrc_ctrl_reg[gasrc_id].con4;
	if (mt8168_afe_gasrc_get_freq_val(input_rate) > 0)
		regmap_write(afe->regmap, ctrl_reg,
			mt8168_afe_gasrc_get_freq_val(input_rate));
}

static void mt8168_afe_gasrc_tx_frequency_palette(
	struct mtk_base_afe *afe,
	...
	int output_rate, unsigned int domain)
{
	...
	ctrl_reg = gasrc_ctrl_reg[gasrc_id].con3;
	if (mt8168_afe_gasrc_get_period_val(input_rate, domain) > 0)
		regmap_write(afe->regmap, ctrl_reg,
			mt8168_afe_gasrc_get_period_val(input_rate, domain));

	ctrl_reg = gasrc_ctrl_reg[gasrc_id].con4;
	if (mt8168_afe_gasrc_get_period_val(output_rate, domain) > 0)
		regmap_write(afe->regmap, ctrl_reg,
			mt8168_afe_gasrc_get_period_val(output_rate, domain));
}

static unsigned int mt8168_afe_gasrc_get_tx_autorst_th_high(
...
static unsigned int mt8168_afe_gasrc_get_tx_autorst_th_low(
	unsigned int rate, unsigned int domain)
{
	int i;
...
static void mt8168_afe_gasrc_autoRstThHi_for_tx(struct mtk_base_afe *afe,
	int gasrc_id, int output_rate, unsigned int domain)
{
	...
	val = mt8168_afe_gasrc_get_tx_autorst_th_high(output_rate, domain);
	if (val > 0)
		regmap_write(afe->regmap, ctrl_reg, val);
}

static void mt8168_afe_gasrc_autoRstThLo_for_tx(struct mtk_base_afe *afe,
	int gasrc_id, int output_rate, unsigned int domain)
{
	unsigned int ctrl_reg = 0;
	unsigned int val = 0;
	...
	val = mt8168_afe_gasrc_get_tx_autorst_th_low(output_rate, domain);
	if (val > 0)
		regmap_write(afe->regmap, ctrl_reg, val);
}

static unsigned int mt8168_afe_gasrc_get_rx_autorst_th_high(unsigned int rate)
{
	int i;
...
static unsigned int mt8168_afe_gasrc_get_rx_autorst_th_low(unsigned int rate)
{
	int i;
...
static void mt8168_afe_gasrc_autoRstThHi_for_rx(struct mtk_base_afe *afe,
	int gasrc_id, int input_rate)
{
	unsigned int ctrl_reg = 0;

	ctrl_reg = gasrc_ctrl_reg[gasrc_id].con9;
	if (mt8168_afe_gasrc_get_rx_autorst_th_high(input_rate) > 0)
		regmap_write(afe->regmap, ctrl_reg,
			mt8168_afe_gasrc_get_rx_autorst_th_high(input_rate));
}

static void mt8168_afe_gasrc_autoRstThLo_for_rx(struct mtk_base_afe *afe,
	int gasrc_id, int input_rate)
{
	unsigned int ctrl_reg = 0;

	ctrl_reg = gasrc_ctrl_reg[gasrc_id].con10;
	if (mt8168_afe_gasrc_get_rx_autorst_th_low(input_rate) > 0)
		regmap_write(afe->regmap, ctrl_reg,
			mt8168_afe_gasrc_get_rx_autorst_th_low(input_rate));
}

static void mt8168_afe_gasrc_setup_cali_tx(struct mtk_base_afe *afe,
...
static void mt8168_afe_gasrc_enable_cali(struct mtk_base_afe *afe,
	int gasrc_id)
{
	unsigned int ctrl_reg = 0;
...
static void mt8168_afe_gasrc_disable_cali(struct mtk_base_afe *afe,
	int gasrc_id)
{
	unsigned int ctrl_reg = 0;
...
static void mt8168_afe_gasrc_setup_cali_rx(struct mtk_base_afe *afe,
	int gasrc_id)
{
	unsigned int mask = 0;
	unsigned int val = 0;
...
static void mt8168_afe_gasrc_fixed_denominator(struct mtk_base_afe *afe,
	int gasrc_id)
{
	unsigned int ctrl_reg = 0;
...
static const struct mt8168_afe_gasrc_iir_coeff_table_id
	mt8168_afe_gasrc_iir_coeff_table_ids[] = {
	{
		.input_rate = 48000,
		.output_rate = 44100,
		.coeff_count = 30,
		.table_id = MT8168_AFE_GASRC_IIR_COEFF_48_TO_44p1,
	},
	{
		.input_rate = 44100,
		.output_rate = 32000,
		.coeff_count = 42,
		.table_id = MT8168_AFE_GASRC_IIR_COEFF_44p1_TO_32,
	},
	{
		.input_rate = 48000,
		.output_rate = 32000,
		.coeff_count = 42,
		.table_id = MT8168_AFE_GASRC_IIR_COEFF_48_TO_32,
	},
	{
		.input_rate = 32000,
		.output_rate = 16000,
		.coeff_count = 48,
		.table_id = MT8168_AFE_GASRC_IIR_COEFF_32_TO_16,
	},
	{
		.input_rate = 96000,
		.output_rate = 44100,
		.coeff_count = 48,
		.table_id = MT8168_AFE_GASRC_IIR_COEFF_96_TO_44p1,
	},
	{
		.input_rate = 44100,
		.output_rate = 16000,
		.coeff_count = 48,
		.table_id = MT8168_AFE_GASRC_IIR_COEFF_44p1_TO_16,
	},
	{
		.input_rate = 48000,
		.output_rate = 16000,
		.coeff_count = 48,
		.table_id = MT8168_AFE_GASRC_IIR_COEFF_48_TO_16,
	},
	{
		.input_rate = 96000,
		.output_rate = 16000,
		.coeff_count = 48,
		.table_id = MT8168_AFE_GASRC_IIR_COEFF_96_TO_16,
	},
};
...
static bool mt8168_afe_gasrc_fill_iir_coeff_table(struct mtk_base_afe *afe,
	int gasrc_id, int table_id, int coeff_count)
{
	...
	if ((table_id < 0) ||
		(table_id >= MT8168_AFE_GASRC_IIR_TABLES))
		return false;

	dev_dbg(afe->dev, "%s table_id %d coeff_count %d\n",
		__func__, table_id, coeff_count);

	coef = &mt8168_afe_gasrc_iir_coeffs[table_id][0];
...
static bool mt8168_afe_gasrc_found_iir_coeff_table_id(int input_rate,
	int output_rate, int *table_id, int *coeff_count)
{
	int i;
	const struct mt8168_afe_gasrc_iir_coeff_table_id *table =
		mt8168_afe_gasrc_iir_coeff_table_ids;
	...
	for (i = 0; i < ARRAY_SIZE(mt8168_afe_gasrc_iir_coeff_table_ids); i++) {
...
static bool mt8168_afe_load_gasrc_iir_coeff_table(struct mtk_base_afe *afe,
	int gasrc_id, int input_rate, int output_rate)
{
	int table_id;
	int coeff_count;

	if (mt8168_afe_gasrc_found_iir_coeff_table_id(input_rate,
			output_rate, &table_id, &coeff_count)) {
		return mt8168_afe_gasrc_fill_iir_coeff_table(afe,
...
static int mt8168_afe_configure_gasrc(struct mtk_base_afe *afe,
	struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		input_fs = mt8168_afe_fs_timing(runtime->rate);
	...
	if (mt8168_afe_load_gasrc_iir_coeff_table(afe, gasrc_id,
			input_rate, output_rate)) {
		mt8168_afe_gasrc_enable_iir(afe, gasrc_id);
	...
	mt8168_afe_gasrc_set_input_fs(afe, dai, input_fs);
	mt8168_afe_gasrc_set_output_fs(afe, dai, output_fs);

	infs_48k_domain = mt8168_afe_clk_group_48k(input_rate);

	/* config con3/con4 */
	if (afe_priv->gasrc_data[gasrc_id].tx_mode) {
		mt8168_afe_gasrc_tx_frequency_palette(afe,
			gasrc_id, input_rate, output_rate, infs_48k_domain);

		if (afe_priv->gasrc_data[gasrc_id].cali_on) {
			mt8168_afe_gasrc_setup_cali_tx(afe,
				gasrc_id, infs_48k_domain);
			mt8168_afe_gasrc_enable_cali(afe, gasrc_id);
		} else
			mt8168_afe_gasrc_disable_cali(afe, gasrc_id);
	} else {
		mt8168_afe_gasrc_rx_frequency_palette(afe,
			gasrc_id, input_rate, output_rate);

		if (afe_priv->gasrc_data[gasrc_id].cali_on) {
			mt8168_afe_gasrc_setup_cali_rx(afe, gasrc_id);
			mt8168_afe_gasrc_enable_cali(afe, gasrc_id);
		} else
			mt8168_afe_gasrc_disable_cali(afe, gasrc_id);
	}

	mt8168_afe_gasrc_fixed_denominator(afe, gasrc_id);

	if (afe_priv->gasrc_data[gasrc_id].tx_mode) {
		mt8168_afe_gasrc_autoRstThHi_for_tx(afe,
			gasrc_id, output_rate, infs_48k_domain);
		mt8168_afe_gasrc_autoRstThLo_for_tx(afe,
			gasrc_id, output_rate, infs_48k_domain);
	} else {
		mt8168_afe_gasrc_autoRstThHi_for_rx(afe,
			gasrc_id, input_rate);
		mt8168_afe_gasrc_autoRstThLo_for_rx(afe,
			gasrc_id, input_rate);
	}
...
static void mt8168_afe_tdm_asrc_set(struct mtk_base_afe *afe,
	struct snd_soc_dai *dai, int fs_timing)
{
	unsigned int val = 0;
	unsigned int mask = 0;
...
static void mt8168_afe_tdm_asrc_one_heart_mode(struct mtk_base_afe *afe,
	struct snd_soc_dai *dai)
{
	unsigned int ctrl_reg = 0;
	int tdm_asrc_id;

	for (tdm_asrc_id = MT8168_TDM_ASRC2;
			tdm_asrc_id < MT8168_TDM_ASRC_NUM; tdm_asrc_id++) {
...
static void mt8168_afe_tdm_asrc_iir_coeff(struct mtk_base_afe *afe,
	int input_fs, int output_fs)
{
	int tdm_asrc_id;
	struct mt8168_afe_private *afe_priv = afe->platform_priv;

	for (tdm_asrc_id = MT8168_TDM_ASRC1;
			tdm_asrc_id < MT8168_TDM_ASRC_NUM; tdm_asrc_id++) {
		if (mt8168_afe_load_gasrc_iir_coeff_table(afe, tdm_asrc_id,
				input_fs, output_fs)) {
			mt8168_afe_gasrc_enable_iir(afe, tdm_asrc_id);
			afe_priv->gasrc_data[tdm_asrc_id].iir_on = true;
		} else {
			mt8168_afe_gasrc_disable_iir(afe, tdm_asrc_id);
			afe_priv->gasrc_data[tdm_asrc_id].iir_on = false;
		}
	}
}

static void mt8168_afe_tdm_asrc_tx_frequency_palette(struct mtk_base_afe *afe,
	int input_rate, int output_rate, int domain)
{
	...
	for (tdm_asrc_id = MT8168_TDM_ASRC1;
			tdm_asrc_id < MT8168_TDM_ASRC_NUM; tdm_asrc_id++) {
		mt8168_afe_gasrc_tx_frequency_palette(afe, tdm_asrc_id,
			input_rate, output_rate, domain);
	}
}

static void mt8168_afe_tdm_asrc_rx_frequency_palette(struct mtk_base_afe *afe,
	int input_rate, int output_rate)
{
	int tdm_asrc_id;
	...
	for (tdm_asrc_id = MT8168_TDM_ASRC1;
			tdm_asrc_id < MT8168_TDM_ASRC_NUM; tdm_asrc_id++) {
		mt8168_afe_gasrc_rx_frequency_palette(afe, tdm_asrc_id,
			input_rate, output_rate);
	}
}

static void mt8168_afe_tdm_asrc_setup_cali_tx(struct mtk_base_afe *afe,
	int domain, bool cmNum)
{
	...
	for (tdm_asrc_id = MT8168_TDM_ASRC1;
			tdm_asrc_id < MT8168_TDM_ASRC_NUM; tdm_asrc_id++) {
		mt8168_afe_gasrc_setup_cali_tx(afe, tdm_asrc_id, domain);
...
static void mt8168_afe_tdm_asrc_setup_cali_rx(struct mtk_base_afe *afe,
	bool cmNum)
{
	int tdm_asrc_id;
	unsigned int ctrl_reg = 0;
	...
	for (tdm_asrc_id = MT8168_TDM_ASRC1;
			tdm_asrc_id < MT8168_TDM_ASRC_NUM; tdm_asrc_id++) {
		mt8168_afe_gasrc_setup_cali_rx(afe, tdm_asrc_id);
		ctrl_reg = gasrc_ctrl_reg[tdm_asrc_id].con5;
		if (cmNum)
			regmap_update_bits(afe->regmap, ctrl_reg,
				GASRC_CON5_CALI_SEL_MASK,
...
static void mt8168_afe_tdm_asrc_enable_cali(struct mtk_base_afe *afe)
{
	int tdm_asrc_id;

	for (tdm_asrc_id = MT8168_TDM_ASRC1;
			tdm_asrc_id < MT8168_TDM_ASRC_NUM; tdm_asrc_id++)
		mt8168_afe_gasrc_enable_cali(afe, tdm_asrc_id);
}

static void mt8168_afe_tdm_asrc_disable_cali(struct mtk_base_afe *afe)
{
	int tdm_asrc_id;

	for (tdm_asrc_id = MT8168_TDM_ASRC4;
			tdm_asrc_id >= MT8168_TDM_ASRC1; tdm_asrc_id--)
		mt8168_afe_gasrc_disable_cali(afe, tdm_asrc_id);
}

static void mt8168_afe_tdm_asrc_fixed_denominator(struct mtk_base_afe *afe)
{
	int tdm_asrc_id;

	for (tdm_asrc_id = MT8168_TDM_ASRC1;
			tdm_asrc_id < MT8168_TDM_ASRC_NUM; tdm_asrc_id++)
		mt8168_afe_gasrc_fixed_denominator(afe, tdm_asrc_id);
}

static void mt8168_afe_tdm_asrc_autoRstThLo_for_tx(struct mtk_base_afe *afe,
...
static void mt8168_afe_tdm_asrc_autoRstThHi_for_tx(struct mtk_base_afe *afe,
	int output_rate, int domain)
{
	int tdm_asrc_id;
...
static void mt8168_afe_tdm_asrc_autoRstThLo_for_rx(struct mtk_base_afe *afe,
	int output_rate)
{
	int tdm_asrc_id;
...
static void mt8168_afe_tdm_asrc_autoRstThHi_for_rx(struct mtk_base_afe *afe,
	int output_rate)
{
	int tdm_asrc_id;
...
static void mt8168_afe_clear_tdm_asrc(struct mtk_base_afe *afe)
{
	int tdm_asrc_id;

	for (tdm_asrc_id = MT8168_TDM_ASRC1;
			tdm_asrc_id < MT8168_TDM_ASRC_NUM; tdm_asrc_id++)
		mt8168_afe_clear_gasrc(afe, tdm_asrc_id);
}

static void mt8168_afe_enable_tdm_asrc(struct mtk_base_afe *afe)
{
	unsigned int ctrl_reg = 0;

	ctrl_reg = gasrc_ctrl_reg[MT8168_TDM_ASRC1].con0;
	regmap_update_bits(afe->regmap, ctrl_reg,
		GASRC_CON0_ASM_ON, GASRC_CON0_ASM_ON);
}

static void mt8168_afe_disable_tdm_asrc(struct mtk_base_afe *afe)
{
	int tdm_asrc_id;
	unsigned int ctrl_reg = 0;
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
...
static int mt8168_afe_configure_tdm_asrc(struct mtk_base_afe *afe,
	struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
	...
	mt8168_afe_tdm_asrc_iir_coeff(afe, input_rate, output_rate);

	infs_48k_domain = mt8168_afe_clk_group_48k(input_rate);
	outfs_48k_domain = mt8168_afe_clk_group_48k(output_rate);
	...
	if (afe_priv->gasrc_data[gasrc_id].tx_mode) {
		mt8168_afe_tdm_asrc_tx_frequency_palette(afe,
			input_rate, output_rate, infs_48k_domain);
		if (afe_priv->gasrc_data[gasrc_id].cali_on) {
			mt8168_afe_tdm_asrc_setup_cali_tx(afe,
				infs_48k_domain,
				tdm_asrc_out_cm2);
			mt8168_afe_tdm_asrc_enable_cali(afe);
		} else {
			mt8168_afe_tdm_asrc_disable_cali(afe);
		}
	} else {
		mt8168_afe_tdm_asrc_rx_frequency_palette(afe,
			input_rate, output_rate);
		if (afe_priv->gasrc_data[gasrc_id].cali_on) {
			mt8168_afe_tdm_asrc_setup_cali_rx(afe,
				tdm_asrc_out_cm2);
			mt8168_afe_tdm_asrc_enable_cali(afe);
		} else {
			mt8168_afe_tdm_asrc_disable_cali(afe);
	...
	mt8168_afe_tdm_asrc_fixed_denominator(afe);

	if (afe_priv->gasrc_data[gasrc_id].tx_mode) {
		mt8168_afe_tdm_asrc_autoRstThHi_for_tx(afe,
			output_rate, infs_48k_domain);
		mt8168_afe_tdm_asrc_autoRstThLo_for_tx(afe,
			output_rate, infs_48k_domain);
	} else {
		mt8168_afe_tdm_asrc_autoRstThHi_for_rx(afe,
			input_rate);
		mt8168_afe_tdm_asrc_autoRstThLo_for_rx(afe,
			input_rate);
	}

	mt8168_afe_tdm_asrc_set(afe, dai, output_fs);
	mt8168_afe_tdm_asrc_one_heart_mode(afe, dai);

	return 0;
}

static int mt8168_afe_gasrc_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	const int gasrc_id = mt8168_dai_num_to_gasrc(dai->id);
	...
	switch (gasrc_id) {
	case MT8168_GASRC1:
		mt8168_afe_enable_top_cg(afe, MT8168_TOP_CG_GENERAL1_ASRC);
		break;
	case MT8168_GASRC2:
		mt8168_afe_enable_top_cg(afe, MT8168_TOP_CG_GENERAL2_ASRC);
		break;
	default:
		break;
	}
...
static void mt8168_afe_gasrc_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	const int gasrc_id = mt8168_dai_num_to_gasrc(dai->id);
	...
	switch (gasrc_id) {
	case MT8168_GASRC1:
		mt8168_afe_disable_top_cg(afe, MT8168_TOP_CG_GENERAL1_ASRC);
		break;
	case MT8168_GASRC2:
		mt8168_afe_disable_top_cg(afe, MT8168_TOP_CG_GENERAL2_ASRC);
		break;
	default:
		break;
	}
...
static int mt8168_afe_gasrc_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	if (afe_priv->gasrc_data[gasrc_id].duplex)
		mt8168_afe_disable_gasrc(afe, gasrc_id);

	mt8168_afe_configure_gasrc(afe, substream, dai);
	mt8168_afe_clear_gasrc(afe, gasrc_id);

	return 0;
}

static int mt8168_afe_gasrc_trigger(struct snd_pcm_substream *substream,
	int cmd, struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	case SNDRV_PCM_TRIGGER_START:
	case SNDRV_PCM_TRIGGER_RESUME:
		ret = mt8168_afe_enable_gasrc(afe, gasrc_id);
		break;
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
		ret = mt8168_afe_disable_gasrc(afe, gasrc_id);
		break;
	default:
		break;
	}
...
static int mt8168_afe_tdm_asrc_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);

	mt8168_afe_enable_main_clk(afe);
	mt8168_afe_enable_top_cg(afe, MT8168_TOP_CG_TDM_ASRC);

	return 0;
}

static void mt8168_afe_tdm_asrc_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);

	mt8168_afe_disable_top_cg(afe, MT8168_TOP_CG_TDM_ASRC);
	mt8168_afe_disable_main_clk(afe);
}

static int mt8168_afe_tdm_asrc_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	if (afe_priv->gasrc_data[gasrc_id].duplex)
		mt8168_afe_disable_tdm_asrc(afe);

	mt8168_afe_configure_tdm_asrc(afe, substream, dai);
	mt8168_afe_clear_tdm_asrc(afe);

	return 0;
}

static int mt8168_afe_tdm_asrc_trigger(struct snd_pcm_substream *substream,
	int cmd, struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	case SNDRV_PCM_TRIGGER_START:
	case SNDRV_PCM_TRIGGER_RESUME:
		mt8168_afe_enable_tdm_asrc(afe);
		break;
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
		mt8168_afe_disable_tdm_asrc(afe);
...
static int mt8168_afe_dai_suspend(struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dai->dev);
#ifdef CONFIG_MTK_HIFIXDSP_SUPPORT
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
	struct mt8168_adsp_data *adsp_data = &(afe_priv->adsp_data);
#endif
	...
	if (adsp_data->adsp_on && adsp_data->hostless_active()) {
		mt8168_afe_set_clk_parent(afe,
			afe_priv->clocks[MT8168_CLK_AUD_INTBUS_SEL],
			afe_priv->clocks[MT8168_CLK_CLK26M]);
		afe->suspended = true;
		return 0;
	}
#endif
...
static int mt8168_afe_dai_resume(struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dai->dev);
#ifdef CONFIG_MTK_HIFIXDSP_SUPPORT
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
	...
	if (adsp_data->adsp_on && adsp_data->hostless_active()) {
		mt8168_afe_set_clk_parent(afe,
			afe_priv->clocks[MT8168_CLK_AUD_INTBUS_SEL],
			afe_priv->clocks[MT8168_CLK_AUD_SYSPLL1_D4]);
		afe->suspended = false;
		return 0;
	}
#endif
...
static const struct snd_soc_dai_ops mt8168_afe_fe_dai_ops = {
	...
	.shutdown	= mt8168_afe_fe_shutdown,
	.hw_params	= mt8168_afe_fe_hw_params,
...
/* BE DAIs */
static const struct snd_soc_dai_ops mt8168_afe_i2s_ops = {
	.startup	= mt8168_afe_i2s_startup,
	.shutdown	= mt8168_afe_i2s_shutdown,
	.prepare	= mt8168_afe_i2s_prepare,
};
...
static const struct snd_soc_dai_ops mt8168_afe_tdm_out_ops = {
	.startup	= mt8168_afe_tdm_out_startup,
	.shutdown	= mt8168_afe_tdm_out_shutdown,
	.prepare	= mt8168_afe_tdm_out_prepare,
	.trigger	= mt8168_afe_tdm_out_trigger,
	.set_fmt	= mt8168_afe_tdm_out_set_fmt,
};

static const struct snd_soc_dai_ops mt8168_afe_tdm_in_ops = {
	.startup	= mt8168_afe_tdm_in_startup,
	.shutdown	= mt8168_afe_tdm_in_shutdown,
	.prepare	= mt8168_afe_tdm_in_prepare,
	.trigger	= mt8168_afe_tdm_in_trigger,
	.set_fmt	= mt8168_afe_tdm_in_set_fmt,
};

static const struct snd_soc_dai_ops mt8168_afe_pcm1_ops = {
	.startup	= mt8168_afe_pcm1_startup,
	.shutdown	= mt8168_afe_pcm1_shutdown,
	.prepare	= mt8168_afe_pcm1_prepare,
	.set_fmt	= mt8168_afe_pcm1_set_fmt,
};

static const struct snd_soc_dai_ops mt8168_afe_dmic_ops = {
	.startup	= mt8168_afe_dmic_startup,
	.shutdown	= mt8168_afe_dmic_shutdown,
	.prepare	= mt8168_afe_dmic_prepare,
};

static const struct snd_soc_dai_ops mt8168_afe_int_adda_ops = {
	.startup	= mt8168_afe_int_adda_startup,
	.shutdown	= mt8168_afe_int_adda_shutdown,
	.prepare	= mt8168_afe_int_adda_prepare,
};

static const struct snd_soc_dai_ops mt8168_afe_gasrc_ops = {
	.startup	= mt8168_afe_gasrc_startup,
	.shutdown	= mt8168_afe_gasrc_shutdown,
	.prepare	= mt8168_afe_gasrc_prepare,
	.trigger	= mt8168_afe_gasrc_trigger,
};

static const struct snd_soc_dai_ops mt8168_afe_tdm_asrc_ops = {
	.startup	= mt8168_afe_tdm_asrc_startup,
...
static struct snd_soc_dai_driver mt8168_afe_pcm_dais[] = {
	...
	{
		.name = "DL1",
		.id = MT8168_AFE_MEMIF_DL1,
		.suspend = mt8168_afe_dai_suspend,
		.resume = mt8168_afe_dai_resume,
		.playback = {
			.stream_name = "DL1",
			.channels_min = 1,
			.channels_max = 2,
	...
	/* BE DAIs */
		...
		.ops = &mt8168_afe_i2s_ops,
	}, {
		.name = "2ND I2S",
		.id = MT8168_AFE_IO_2ND_I2S,
		.playback = {
		...
		.ops = &mt8168_afe_2nd_i2s_ops,
	}, {
	/* BE DAIs */
		.name = "TDM_OUT_IO",
		.id = MT8168_AFE_IO_TDM_OUT,
		.playback = {
		...
		.ops = &mt8168_afe_tdm_out_ops,
	}, {
	/* BE DAIs */
		.name = "TDM_IN_IO",
		.id = MT8168_AFE_IO_TDM_IN,
		.capture = {
		...
		.ops = &mt8168_afe_tdm_in_ops,
	}, {
	/* BE DAIs */
		.name = "PCM1",
		.id = MT8168_AFE_IO_PCM1,
		.playback = {
		...
		.ops = &mt8168_afe_pcm1_ops,
		.symmetric_rates = 1,
		.symmetric_samplebits = 1,
	}, {
	/* BE DAIs */
		.name = "VIRTUAL_DL_SRC",
		.id = MT8168_AFE_IO_VIRTUAL_DL_SRC,
		.capture = {
	...
	/* BE DAIs */
		...
		.id = MT8168_AFE_IO_VIRTUAL_TDM_OUT_SRC,
		.capture = {
	...
	/* BE DAIs */
		...
		.ops = &mt8168_afe_dmic_ops,
	}, {
	/* BE DAIs */
		.name = "INT ADDA",
		.id = MT8168_AFE_IO_INT_ADDA,
		.playback = {
		...
		.ops = &mt8168_afe_int_adda_ops,
	}, {
	/* BE DAIs */
		.name = "GASRC1",
		.id = MT8168_AFE_IO_GASRC1,
		.playback = {
		...
		.ops = &mt8168_afe_gasrc_ops,
	}, {
	/* BE DAIs */
		.name = "GASRC2",
		.id = MT8168_AFE_IO_GASRC2,
		.playback = {
...
static const struct snd_kcontrol_new mt8168_afe_o00_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I05 Switch", AFE_CONN0, 5, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I07 Switch", AFE_CONN0, 7, 1, 0),
};

static const struct snd_kcontrol_new mt8168_afe_o01_mix[] = {
...
static const struct snd_kcontrol_new mt8168_afe_o08_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I06 Switch", AFE_CONN8, 6, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I08 Switch", AFE_CONN8, 8, 1, 0),
};

static const struct snd_kcontrol_new mt8168_afe_o09_mix[] = {
...
static const struct snd_kcontrol_new mt8168_afe_o10_mix[] = {
...
static const struct snd_kcontrol_new mt8168_afe_o11_mix[] = {
...
static const struct snd_kcontrol_new mt8168_afe_o12_mix[] = {
...
static const struct snd_kcontrol_new mt8168_afe_o16_mix[] = {
};

static const struct snd_kcontrol_new mt8168_afe_o17_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I03 Switch", AFE_CONN17, 3, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I14 Switch", AFE_CONN17, 14, 1, 0),
};

static const struct snd_kcontrol_new mt8168_afe_o18_mix[] = {
...
static const struct snd_kcontrol_new mt8168_afe_o21_mix[] = {
...
static const struct snd_kcontrol_new mt8168_afe_o22_mix[] = {
...
static const struct snd_kcontrol_new mt8168_afe_o23_mix[] = {
...
static const struct snd_kcontrol_new mt8168_afe_o24_mix[] = {
...
static const struct snd_kcontrol_new mt8168_afe_o25_mix[] = {
...
static const struct snd_kcontrol_new mt8168_afe_o26_mix[] = {
...
static int mt8168_afe_cm2_io_input_mux_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
...
static int mt8168_afe_gasrc1_output_mux_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
...
static int mt8168_afe_gasrc1_output_mux_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_context *dapm =
		snd_soc_dapm_kcontrol_dapm(kcontrol);
	...
	/* 0:OPEN 1:CM1 2:CM2 */
	if (mGasrc1Out == 1) {
		afe_priv->gasrc_data[MT8168_GASRC1].tx_mode = false;
		afe_priv->gasrc_data[MT8168_GASRC1].cali_on = false;
		regmap_update_bits(afe->regmap, AFE_CM2_CON0,
			CM_AFE_CM2_GASRC1_OUT_SEL,
			GASRC1_OUT_TO_ENGEN_OUT_EN);
	} else if (mGasrc1Out == 2) {
		afe_priv->gasrc_data[MT8168_GASRC1].tx_mode = true;
		afe_priv->gasrc_data[MT8168_GASRC1].cali_on = true;
		regmap_update_bits(afe->regmap, AFE_CM2_CON0,
			CM_AFE_CM2_GASRC1_OUT_SEL,
...
static int mt8168_afe_gasrc2_output_mux_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
...
static int mt8168_afe_gasrc2_output_mux_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_context *dapm =
		snd_soc_dapm_kcontrol_dapm(kcontrol);
	...
	/* 0:OPEN 1:CM1 2:CM2 */
	if (mGasrc2Out == 1) {
		afe_priv->gasrc_data[MT8168_GASRC2].tx_mode = false;
		afe_priv->gasrc_data[MT8168_GASRC2].cali_on = false;
		regmap_update_bits(afe->regmap, AFE_CM2_CON0,
			CM_AFE_CM2_GASRC2_OUT_SEL,
			GASRC2_OUT_TO_ENGEN_OUT_EN);
	} else if (mGasrc2Out == 2) {
		afe_priv->gasrc_data[MT8168_GASRC2].tx_mode = true;
		afe_priv->gasrc_data[MT8168_GASRC2].cali_on = true;
		regmap_update_bits(afe->regmap, AFE_CM2_CON0,
			CM_AFE_CM2_GASRC2_OUT_SEL,
...
static int mt8168_afe_tdm_asrc_output_mux_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
...
static int mt8168_afe_tdm_asrc_output_mux_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_context *dapm =
		snd_soc_dapm_kcontrol_dapm(kcontrol);
	...
	/* 0:OPEN 1:CM1 2:CM2 */
	if (mTdmGasrcOut == 1) {
		afe_priv->gasrc_data[MT8168_TDM_ASRC1].tx_mode = false;
		afe_priv->gasrc_data[MT8168_TDM_ASRC1].cali_on = true;
		afe_priv->gasrc_data[MT8168_TDM_ASRC1].tdm_asrc_out_cm2 = false;
		regmap_update_bits(afe->regmap, AFE_TDM_ASRC_CON0,
			TDM_ASRC_OUT_SEL,
			TDM_ASRC_OUT_SEL_TO_CM1);
	} else if (mTdmGasrcOut == 2) {
		afe_priv->gasrc_data[MT8168_TDM_ASRC1].tx_mode = true;
		afe_priv->gasrc_data[MT8168_TDM_ASRC1].cali_on = false;
		afe_priv->gasrc_data[MT8168_TDM_ASRC1].tdm_asrc_out_cm2 = true;
		regmap_update_bits(afe->regmap, AFE_TDM_ASRC_CON0,
			TDM_ASRC_OUT_SEL,
...
#ifdef CONFIG_MTK_HIFIXDSP_SUPPORT
static void mt8168_adsp_get_afe_memif_sram(struct mtk_base_afe *afe,
				       int memif_id,
				       unsigned int *paddr,
				       unsigned int *size)
{
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
	struct mt8168_fe_dai_data *fe_data = &afe_priv->fe_data[memif_id];
	*paddr = fe_data->sram_phy_addr;
	*size = fe_data->sram_size;
}

static int mt8168_adsp_set_afe_memif(struct mtk_base_afe *afe,
				       int memif_id,
				       unsigned int rate,
				       unsigned int channels,
				       snd_pcm_format_t format)
{
	...
	struct mt8168_control_data *ctrl_data = &afe_priv->ctrl_data;
	int fs, hd_audio = 0;
	unsigned int base_end_offset = AFE_BASE_END_OFFSET;

	if (memif_id == MT8168_AFE_MEMIF_VUL2) {
		if (!ctrl_data->bypass_cm1)
			/* configure cm1 */
			mt8168_afe_configure_cm(afe,
				MT8168_CM1, channels, rate);
			...
			regmap_update_bits(afe->regmap, AFE_CM1_CON0,
				CM_AFE_CM1_VUL_SEL,
				CM_AFE_CM1_VUL_SEL_O17_O18);
	} else if (memif_id == MT8168_AFE_MEMIF_TDM_IN) {
		if (!ctrl_data->bypass_cm2)
			/* configure cm2 */
			mt8168_afe_configure_cm(afe,
				MT8168_CM2, channels, rate);
	...
	mt8168_afe_irq_direction_enable(afe,
		memif->irq_usage,
		MT8168_AFE_IRQ_DIR_DSP);

	return 0;
}

static int mt8168_adsp_set_afe_memif_enable(struct mtk_base_afe *afe,
				       int memif_id,
				       unsigned int rate,
...
static void mt8168_adsp_set_afe_init(struct mtk_base_afe *afe)
{
	dev_dbg(afe->dev, "%s\n", __func__);
...
static void mt8168_adsp_set_afe_uninit(struct mtk_base_afe *afe)
{
	dev_dbg(afe->dev, "%s\n", __func__);
...
static const char * const mt8168_afe_cm2_mux_text[] = {
...
static const struct snd_kcontrol_new mt8168_afe_cm2_mux_input_mux =
	SOC_DAPM_ENUM_EXT("CM2_MUX Source", mt8168_afe_cm2_mux_input_enum,
		mt8168_afe_cm2_io_input_mux_get,
		mt8168_afe_cm2_io_input_mux_put);

static const struct snd_kcontrol_new gasrc1_out_sel_mux =
	SOC_DAPM_ENUM_EXT("GASRC1 Output", gasrc1_out_sel_enum,
	mt8168_afe_gasrc1_output_mux_get,
	mt8168_afe_gasrc1_output_mux_put);

static const struct snd_kcontrol_new gasrc2_out_sel_mux =
	SOC_DAPM_ENUM_EXT("GASRC2 Output", gasrc2_out_sel_enum,
	mt8168_afe_gasrc2_output_mux_get,
	mt8168_afe_gasrc2_output_mux_put);

static const struct snd_kcontrol_new tdm_asrc_out_sel_mux =
	SOC_DAPM_ENUM_EXT("TDM_ASRC Output", tdm_asrc_out_sel_enum,
	mt8168_afe_tdm_asrc_output_mux_get,
	mt8168_afe_tdm_asrc_output_mux_put);
...
static const struct snd_soc_dapm_widget mt8168_afe_pcm_widgets[] = {
	/* inter-connections */
	...
	SND_SOC_DAPM_MIXER("O01", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o01_mix, ARRAY_SIZE(mt8168_afe_o01_mix)),
	SND_SOC_DAPM_MIXER("O03", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o03_mix, ARRAY_SIZE(mt8168_afe_o03_mix)),
	SND_SOC_DAPM_MIXER("O04", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o04_mix, ARRAY_SIZE(mt8168_afe_o04_mix)),
	SND_SOC_DAPM_MIXER("O05", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o05_mix, ARRAY_SIZE(mt8168_afe_o05_mix)),
	SND_SOC_DAPM_MIXER("O06", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o06_mix, ARRAY_SIZE(mt8168_afe_o06_mix)),
	SND_SOC_DAPM_MIXER("O07", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o07_mix, ARRAY_SIZE(mt8168_afe_o07_mix)),
	SND_SOC_DAPM_MIXER("O08", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o08_mix, ARRAY_SIZE(mt8168_afe_o08_mix)),
	SND_SOC_DAPM_MIXER("O09", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o09_mix, ARRAY_SIZE(mt8168_afe_o09_mix)),
	SND_SOC_DAPM_MIXER("O10", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o10_mix, ARRAY_SIZE(mt8168_afe_o10_mix)),
	SND_SOC_DAPM_MIXER("O11", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o11_mix, ARRAY_SIZE(mt8168_afe_o11_mix)),
	SND_SOC_DAPM_MIXER("O12", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o12_mix, ARRAY_SIZE(mt8168_afe_o12_mix)),
	SND_SOC_DAPM_MIXER("O13", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o13_mix, ARRAY_SIZE(mt8168_afe_o13_mix)),
	SND_SOC_DAPM_MIXER("O14", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o14_mix, ARRAY_SIZE(mt8168_afe_o14_mix)),
	SND_SOC_DAPM_MIXER("O15", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o15_mix, ARRAY_SIZE(mt8168_afe_o15_mix)),
	SND_SOC_DAPM_MIXER("O16", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o16_mix, ARRAY_SIZE(mt8168_afe_o16_mix)),
	SND_SOC_DAPM_MIXER("O17", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o17_mix, ARRAY_SIZE(mt8168_afe_o17_mix)),
	SND_SOC_DAPM_MIXER("O18", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o18_mix, ARRAY_SIZE(mt8168_afe_o18_mix)),
	SND_SOC_DAPM_MIXER("O19", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o19_mix, ARRAY_SIZE(mt8168_afe_o19_mix)),
	SND_SOC_DAPM_MIXER("O20", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o20_mix, ARRAY_SIZE(mt8168_afe_o20_mix)),
	SND_SOC_DAPM_MIXER("O21", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o21_mix, ARRAY_SIZE(mt8168_afe_o21_mix)),
	SND_SOC_DAPM_MIXER("O22", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o22_mix, ARRAY_SIZE(mt8168_afe_o22_mix)),
	SND_SOC_DAPM_MIXER("O23", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o23_mix, ARRAY_SIZE(mt8168_afe_o23_mix)),
	SND_SOC_DAPM_MIXER("O24", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o24_mix, ARRAY_SIZE(mt8168_afe_o24_mix)),
	SND_SOC_DAPM_MIXER("O25", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o25_mix, ARRAY_SIZE(mt8168_afe_o25_mix)),
	SND_SOC_DAPM_MIXER("O26", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o26_mix, ARRAY_SIZE(mt8168_afe_o26_mix)),
	SND_SOC_DAPM_MIXER("O27", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o27_mix, ARRAY_SIZE(mt8168_afe_o27_mix)),
	SND_SOC_DAPM_MIXER("O28", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o28_mix, ARRAY_SIZE(mt8168_afe_o28_mix)),
	SND_SOC_DAPM_MIXER("O29", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o29_mix, ARRAY_SIZE(mt8168_afe_o29_mix)),
	SND_SOC_DAPM_MIXER("O30", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o30_mix, ARRAY_SIZE(mt8168_afe_o30_mix)),
	SND_SOC_DAPM_MIXER("O31", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o31_mix, ARRAY_SIZE(mt8168_afe_o31_mix)),
	SND_SOC_DAPM_MIXER("O32", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o32_mix, ARRAY_SIZE(mt8168_afe_o32_mix)),
	SND_SOC_DAPM_MIXER("O33", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o33_mix, ARRAY_SIZE(mt8168_afe_o33_mix)),
	SND_SOC_DAPM_MIXER("O34", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o34_mix, ARRAY_SIZE(mt8168_afe_o34_mix)),
	SND_SOC_DAPM_MIXER("O35", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o35_mix, ARRAY_SIZE(mt8168_afe_o35_mix)),
	SND_SOC_DAPM_MIXER("O36", SND_SOC_NOPM, 0, 0,
			   mt8168_afe_o36_mix, ARRAY_SIZE(mt8168_afe_o36_mix)),
	...
	SND_SOC_DAPM_MUX("CM2_Mux_IO Input Mux",
		SND_SOC_NOPM, 0, 0, &mt8168_afe_cm2_mux_input_mux),
...
static const struct snd_soc_component_driver mt8168_afe_pcm_dai_component = {
	.name = "mt8168-afe-pcm-dai",
	.dapm_widgets = mt8168_afe_pcm_widgets,
	.num_dapm_widgets = ARRAY_SIZE(mt8168_afe_pcm_widgets),
	.dapm_routes = mt8168_afe_pcm_routes,
	.num_dapm_routes = ARRAY_SIZE(mt8168_afe_pcm_routes),
};

static const struct mtk_base_memif_data memif_data[MT8168_AFE_MEMIF_NUM] = {
	{
		.name = "DL1",
		.id = MT8168_AFE_MEMIF_DL1,
...
static const struct mtk_base_irq_data irq_data[MT8168_AFE_IRQ_NUM] = {
	{
		.id = MT8168_AFE_IRQ1,
...
static int memif_specified_irqs[MT8168_AFE_MEMIF_NUM] = {
	[MT8168_AFE_MEMIF_DL1] = MT8168_AFE_IRQ1,
	[MT8168_AFE_MEMIF_DL2] = MT8168_AFE_IRQ2,
	[MT8168_AFE_MEMIF_TDM_OUT] = MT8168_AFE_IRQ5,
	/* [MT8168_AFE_MEMIF_SPDIF_OUT] = MT8168_AFE_IRQ6,*/
	[MT8168_AFE_MEMIF_AWB] = MT8168_AFE_IRQ3,
	[MT8168_AFE_MEMIF_VUL] = MT8168_AFE_IRQ4,
	[MT8168_AFE_MEMIF_VUL2] = MT8168_AFE_IRQ7,
	[MT8168_AFE_MEMIF_VUL3] = MT8168_AFE_IRQ8,
	/* [MT8168_AFE_MEMIF_SPDIF_IN] = MT8168_AFE_IRQ9, */
	[MT8168_AFE_MEMIF_TDM_IN] = MT8168_AFE_IRQ10,
};

static const struct regmap_config mt8168_afe_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static irqreturn_t mt8168_afe_irq_handler(int irq, void *dev_id)
{
	...
	for (i = 0; i < MT8168_AFE_MEMIF_NUM; i++) {
...
#ifdef CONFIG_MTK_HIFIXDSP_SUPPORT
struct mtk_base_afe *mt8168_afe_pcm_get_info(void)
{
	return g_priv;
}
#endif

static int mt8168_afe_runtime_suspend(struct device *dev)
{
	return 0;
}

static int mt8168_afe_runtime_resume(struct device *dev)
{
	return 0;
}

static int mt8168_afe_dev_runtime_suspend(struct device *dev)
{
...
static int mt8168_afe_dev_runtime_resume(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);
	struct regmap *regmap = afe->regmap;
	int i = 0;
...
static int mt8168_afe_init_registers(struct mtk_base_afe *afe)
{
	size_t i;
...
static bool mt8168_afe_validate_sram(struct mtk_base_afe *afe,
...
static void __iomem *mt8168_afe_get_sram_vir_addr(struct mtk_base_afe *afe,
	u32 phy_addr)
{
	struct mt8168_afe_private *afe_priv = afe->platform_priv;
...
static void mt8168_afe_parse_of(struct mtk_base_afe *afe,
				struct device_node *np)
{
	...
	struct {
		...
		{ "dl1",	MT8168_AFE_MEMIF_DL1 },
		{ "dl2",	MT8168_AFE_MEMIF_DL2 },
		{ "tdmout",	MT8168_AFE_MEMIF_TDM_OUT },
		{ "awb",	MT8168_AFE_MEMIF_AWB },
		{ "vul",	MT8168_AFE_MEMIF_VUL },
		{ "vul2",	MT8168_AFE_MEMIF_VUL2 },
		{ "vul3",	MT8168_AFE_MEMIF_VUL3 },
		{ "tdmin",	MT8168_AFE_MEMIF_TDM_IN },
	};
	...
	if (ret == 0)
		...
		afe_priv->tdm_out_mode = MT8168_AFE_TDM_OUT_TDM;
	...
	for (i = 0; i < ARRAY_SIZE(of_fe_table); i++) {
		...
		valid_sram = mt8168_afe_validate_sram(afe, val[0], val[1]);
		if (!valid_sram) {
		...
		afe_priv->fe_data[i].sram_vir_addr =
			mt8168_afe_get_sram_vir_addr(afe, val[0]);
	}
	...
	if (ret == 0) {
		if (temps[0])
			mt8168_afe_set_clk_parent(afe,
				afe_priv->clocks[MT8168_CLK_TOP_AUD_SEL],
				afe_priv->clocks[MT8168_CLK_AUD_SYSPLL3_D4]);
		else
			mt8168_afe_set_clk_parent(afe,
				afe_priv->clocks[MT8168_CLK_TOP_AUD_SEL],
				afe_priv->clocks[MT8168_CLK_CLK26M]);
...
static int mt8168_afe_pcm_probe(struct snd_soc_platform *platform)
{
	return mt8168_afe_add_controls(platform);
}

const struct snd_soc_platform_driver mt8168_afe_pcm_platform = {
	.probe = mt8168_afe_pcm_probe,
	.pcm_new = mtk_afe_pcm_new,
	.pcm_free = mtk_afe_pcm_free,
	.ops = &mtk_afe_pcm_ops,
};

static int mt8168_afe_pcm_dev_probe(struct platform_device *pdev)
{
	int ret, i, sel_irq;
	unsigned int irq_id;
	struct mtk_base_afe *afe;
	...
	mutex_init(&afe_priv->emi_clk_mutex);
#endif
	...
	ret = devm_request_irq(afe->dev, irq_id, mt8168_afe_irq_handler,
	...
	ret = mt8168_afe_init_audio_clk(afe);
	if (ret) {
		dev_err(afe->dev, "mt8168_afe_init_audio_clk fail\n");
		return ret;
	}

	afe->regmap = devm_regmap_init_mmio_clk(&pdev->dev, "top_audio_sel",
		afe->base_addr,	&mt8168_afe_regmap_config);
	if (IS_ERR(afe->regmap))
		return PTR_ERR(afe->regmap);

	/* memif % irq initialize*/
	afe->memif_size = MT8168_AFE_MEMIF_NUM;
	afe->memif = devm_kcalloc(afe->dev, afe->memif_size,
				  sizeof(*afe->memif), GFP_KERNEL);
	if (!afe->memif)
		return -ENOMEM;

	afe->irqs_size = MT8168_AFE_IRQ_NUM;
	...
	afe->mtk_afe_hardware = &mt8168_afe_hardware;
	afe->memif_fs = mt8168_memif_fs;
	afe->irq_fs = mt8168_irq_fs;
	...
	if (!pm_runtime_enabled(&pdev->dev)) {
		...
		ret = mt8168_afe_runtime_resume(&pdev->dev);
		if (ret)
			goto err_pm_disable;
	}

	pm_runtime_get_sync(&pdev->dev);
	afe->reg_back_up_list = mt8168_afe_backup_list;
	afe->reg_back_up_list_num = ARRAY_SIZE(mt8168_afe_backup_list);
	afe->runtime_resume = mt8168_afe_runtime_resume;
	afe->runtime_suspend = mt8168_afe_runtime_suspend;

#ifdef CONFIG_MTK_HIFIXDSP_SUPPORT
	afe_priv->adsp_data.get_afe_memif_sram =
		mt8168_adsp_get_afe_memif_sram;
	afe_priv->adsp_data.set_afe_memif =
		mt8168_adsp_set_afe_memif;
	afe_priv->adsp_data.set_afe_memif_enable =
		mt8168_adsp_set_afe_memif_enable;
	afe_priv->adsp_data.set_afe_init =
		mt8168_adsp_set_afe_init;
	afe_priv->adsp_data.set_afe_uninit =
		mt8168_adsp_set_afe_uninit;
#endif

	/* open afe pdn for dapm read/write audio register */
	mt8168_afe_enable_top_cg(afe, MT8168_TOP_CG_AFE);
	mt8168_afe_parse_of(afe, pdev->dev.of_node);

	ret = snd_soc_register_platform(&pdev->dev, &mt8168_afe_pcm_platform);
	if (ret)
		goto err_platform;

	ret = snd_soc_register_component(&pdev->dev,
					 &mt8168_afe_pcm_dai_component,
					 mt8168_afe_pcm_dais,
					 ARRAY_SIZE(mt8168_afe_pcm_dais));
	if (ret)
		goto err_component;

	mt8168_afe_init_registers(afe);

	mt8168_afe_init_debugfs(afe);

	dev_info(&pdev->dev, "MT8168 AFE driver initialized.\n");
	return 0;
...
static int mt8168_afe_pcm_dev_remove(struct platform_device *pdev)
{
	struct mtk_base_afe *afe = platform_get_drvdata(pdev);

	mt8168_afe_cleanup_debugfs(afe);
	...
	if (!pm_runtime_status_suspended(&pdev->dev))
		mt8168_afe_runtime_suspend(&pdev->dev);
...
static const struct of_device_id mt8168_afe_pcm_dt_match[] = {
	{ .compatible = "mediatek,mt8168-afe-pcm", },
	{ }
};
MODULE_DEVICE_TABLE(of, mt8168_afe_pcm_dt_match);

static const struct dev_pm_ops mt8168_afe_pm_ops = {
	SET_RUNTIME_PM_OPS(mt8168_afe_dev_runtime_suspend,
			   mt8168_afe_dev_runtime_resume, NULL)
};

static struct platform_driver mt8168_afe_pcm_driver = {
	.driver = {
		   .name = "mt8168-afe-pcm",
		   .of_match_table = mt8168_afe_pcm_dt_match,
		   .pm = &mt8168_afe_pm_ops,
	},
	.probe = mt8168_afe_pcm_dev_probe,
	.remove = mt8168_afe_pcm_dev_remove,
};

module_platform_driver(mt8168_afe_pcm_driver);
