...
/*
 * Map scatter-gather list into contiguous io address space
 */
static int __map_sg_list(struct dma_iommu_mapping *mapping, dma_addr_t handle,
			struct scatterlist *sg, int nents,
			enum dma_data_direction dir, unsigned long attrs,
			bool is_coherent)
{
	dma_addr_t iova, iova_base = handle;
	struct scatterlist *s;
	phys_addr_t phys;
	...
	if (iova_base & (~PAGE_MASK)) {
		iova_base = iova_base & PAGE_MASK;
		pr_warn("map sglist: iova base addr 0x%x force to 0x%x\n",
			handle, iova_base);
	}
	...
	for_each_sg(sg, s, nents, i) {
		...
		if (ret < 0) {
			pr_err("map sglist fail: iova 0x%x, pa 0x%x (+0x%x)\n",
				iova, phys, len);
			ns = i;
			goto map_iova_fail;
		}
		     ...
		     bool is_coherent)
{
	...
	if (!mapping) {
		dev_err(dev, "%s: mapping info error\n", __func__);
		return 0;
	}

	for_each_sg(sg, s, nents, i)
		totle_len += PAGE_ALIGN(s->offset + s->length);
	if (!totle_len)
		return 0;

	iova_base = __alloc_iova(mapping, totle_len);
	if (iova_base == ARM_MAPPING_ERROR) {
		dev_err(dev, "%s: alloc iova fail\n", __func__);
		return 0;
	}

	if (__map_sg_list(mapping, iova_base, sg, nents, dir, attrs,
			is_coherent) < 0)
		goto bad_mapping;

	/* for debug */
	/*
	 * dev_info(dev, "__iommu_map_sg:\n===== dump sg map =====\n");
	 * for_each_sg(sg, s, nents, i) {
...
bad_mapping:
	__free_iova(mapping, iova_base, totle_len);
	return 0;
}
